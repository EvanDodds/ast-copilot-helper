...
describe('ComprehensiveReleaseManager', () => {
  ...
  beforeEach(async () => {
    mockConfig = {
      ...
      rollback: {
        ...
        backupRetention: 30,
        validationSteps: ['version-check', 'dependency-check', 'platform-check']
      }
    };
  ...
  describe('rollback functionality', () => {
    test('should execute rollback successfully', async () => {
      const result = await manager.rollbackRelease('1.2.3', 'Critical bug detected');
  ...
  describe('error handling', () => {
    test('should handle version validation errors', async () => {
      await expect(manager.planRelease('invalid-version', ReleaseType.PATCH))
        .rejects.toThrow();
    });
