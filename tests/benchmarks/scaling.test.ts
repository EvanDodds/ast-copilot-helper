...
describe("Scaling and Load Benchmarks", () => {
  ...
  describe("Repository Size Scaling", () => {
    it("should scale linearly with repository size", async () => {
      ...
      for (const repoSize of repositorySizes) {
        ...
        console.log(
          `Repository size ${repoSize}: ${processingTime.toFixed(2)}ms (${nodesPerMs.toFixed(2)} nodes/ms)`,
        );
      ...
      console.log(
        `  Scaling efficiency: ${((sizeRatio / scalingFactor) * 100).toFixed(1)}%`,
      );
    ...
    it("should handle extremely large repositories within time constraints", async () => {
      ...
      for (let chunk = 0; chunk < chunks; chunk++) {
        const currentChunkSize = Math.min(
          chunkSize,
          massiveRepoSize - totalProcessed,
        );
        ...
        if ((chunk + 1) % 5 === 0) {
          ...
          console.log(
            `  Massive repo progress: ${progress}% (${totalProcessed}/${massiveRepoSize})`,
          );
        }
      }
      ...
      console.log(
        `  Performance: ${((targetTime - totalTime) / 1000).toFixed(2)}s under target`,
      );
  ...
  describe("Concurrent User Simulation", () => {
    it("should handle multiple concurrent users efficiently", async () => {
      ...
      const concurrencyResults: Record<
        number,
        { totalTime: number; avgLatency: number; throughput: number }
      > = {};
      ...
      for (const userCount of userCounts) {
        ...
        const allLatencies = userResults.flatMap(
          (result) => result.queryLatencies,
        );
        const avgLatency =
      ...
      expect(concurrencyResults[10].throughput).toBeGreaterThan(
        singleUser.throughput * 5,
      );
    });
    ...
    it("should maintain MCP and CLI performance under load", async () => {
      ...
      // Start MCP clients
      const mcpPromises = Array.from(
        { length: concurrentMCPClients },
        (_, id) => simulateMCPClientSession(id, queriesPerClient),
      );

      // Start CLI clients (more resource intensive)
      const cliPromises = Array.from(
        { length: concurrentCLIClients },
        (_, id) => simulateCLIClientSession(id, queriesPerClient),
      );
      ...
      const mcpLatencies = mcpResults.flatMap(
        (result) => result.queryLatencies,
      );
      const avgMCPLatency =
        mcpLatencies.reduce((sum, lat) => sum + lat, 0) / mcpLatencies.length;
      const maxMCPLatency = Math.max(...mcpLatencies);

      // Analyze CLI performance
      const cliLatencies = cliResults.flatMap(
        (result) => result.queryLatencies,
      );
      const avgCLILatency =
        cliLatencies.reduce((sum, lat) => sum + lat, 0) / cliLatencies.length;
      const maxCLILatency = Math.max(...cliLatencies);

      console.log(`Mixed Client Load Test (${totalTime.toFixed(2)}ms):`);
      console.log(
        `  MCP - Avg: ${avgMCPLatency.toFixed(2)}ms, Max: ${maxMCPLatency.toFixed(2)}ms`,
      );
      console.log(
        `  CLI - Avg: ${avgCLILatency.toFixed(2)}ms, Max: ${maxCLILatency.toFixed(2)}ms`,
      );

      // Performance requirements under load
      expect(avgMCPLatency).toBeLessThan(500); // Adjusted for CI environment load
      expect(maxMCPLatency).toBeLessThan(10000); // Allow for high spikes under CI load
      expect(avgCLILatency).toBeLessThan(1000); // Adjusted for CI environment load
      expect(maxCLILatency).toBeLessThan(15000); // Allow for high spikes under CI load
  ...
  describe("Memory Pressure Scenarios", () => {
    it("should maintain performance under memory pressure", async () => {
      ...
      for (const scenario of memoryPressureScenarios) {
        ...
        console.log(
          `  Memory used: ${((afterMemory.heapUsed - beforeMemory.heapUsed) / 1024 / 1024).toFixed(2)}MB`,
        );
    ...
    it("should recover gracefully from memory spikes", async () => {
      ...
      const avgSpike =
        spikeResults.reduce((a, b) => a + b, 0) / spikeResults.length;
      const avgRecovery =
        recoveryResults.reduce((a, b) => a + b, 0) / recoveryResults.length;
...
async function processRepositoryOfSize(
  nodeCount: number,
): Promise<{ success: boolean; processedNodes: number }> {
  try {
...
async function simulateMCPClientSession(
  clientId: number,
  queryCount: number,
): Promise<UserSessionResult> {
  const queryLatencies: number[] = [];
...
async function simulateCLIClientSession(
  clientId: number,
  queryCount: number,
): Promise<UserSessionResult> {
  const queryLatencies: number[] = [];
