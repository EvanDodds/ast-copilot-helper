...
function validateSingleWorkflow(workflow, _filename) {
  ...
  if (workflow.on?.pull_request) {
    ...
    const hasReadyForReview =
      prTrigger.types?.includes("ready_for_review") ||
      prTrigger === true ||
  ...
  if (workflow.on?.push) {
    ...
    if (onlyMain) {
      ...
      checks.push(
        `⚠️  Push trigger includes: ${pushBranches.join(", ")} (expected: main only)`,
      );
  ...
  if (jobsWithDraftCheck > 0) {
    checks.push(
      `✅ ${jobsWithDraftCheck}/${totalJobs} jobs have draft PR checks`,
    );
  } else if (totalJobs > 0) {
    checks.push(
      "⚠️  No jobs have draft PR checks (PRs will run on all states)",
    );
