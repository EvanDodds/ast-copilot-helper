...
export class QueryBenchmarkRunner {
      ...
      for (const queryType of config.queryTypes) {
        ...
        for (const nodeCount of config.nodeCounts) {
          ...
          for (let i = 0; i < config.iterations; i++) {
            ...
            const run = await this.runSingleQueryBenchmark(
              queryType,
              singleNodeConfig,
            );
            runs.push(run);
   ...
   */
  async runMCPQueryBenchmarks(
    config: QueryBenchmarkConfig & { nodeCount: NodeCount },
  ): Promise<BenchmarkResult> {
    ...
    for (const queryType of queryTypes) {
      for (let i = 0; i < config.iterations; i++) {
        ...
        try {
          ...
          if (!meetsMCPTarget) {
            console.warn(
              `⚠️  MCP ${queryType} query exceeded 200ms: ${duration}ms`,
            );
          }
    ...
    for (const queryType of queryTypes) {
      for (let i = 0; i < config.iterations; i++) {
        ...
        try {
          ...
          if (!meetsCLITarget) {
            console.warn(
              `⚠️  CLI ${queryType} query exceeded 500ms: ${duration}ms`,
            );
          }
  ...
  async runCachingBenchmarks(
    ...
    for (const queryType of queryTypes) {
      ...
      const cacheImprovement = Math.round(
        ((missDuration - hitDuration) / missDuration) * 100,
      );
   ...
   */
  async runConcurrentQueryBenchmarks(
    config: QueryBenchmarkConfig & { nodeCount: NodeCount },
  ): Promise<BenchmarkResult> {
    ...
    for (const concurrency of concurrencyLevels) {
      ...
      const avgDuration =
        results.reduce((sum, dur) => sum + dur, 0) / results.length;
      const throughput =
        Math.round((concurrency / (totalDuration / 1000)) * 100) / 100;

      console.log(
        `Level ${concurrency}: Avg=${avgDuration.toFixed(1)}ms, Throughput=${throughput} q/s`,
      );
   ...
   */
  private async runSingleQueryBenchmark(
    queryType: QueryType,
    config: QueryBenchmarkConfig & { nodeCount: NodeCount },
  ...
  private generateMCPTestData(
    ...
    const targetSize =
      typeof nodeCount === "number" ? nodeCount : sizes[nodeCount];
   ...
   * Simulate MCP query processing
   */
  private async simulateMCPQuery(
    testData: string,
    queryType: QueryType,
  ): Promise<void> {
    const baseDelay =
      queryType === "file" ? 50 : queryType === "ast" ? 100 : 150;
   ...
   * Simulate CLI query processing
   */
  private async simulateCLIQuery(
    testData: string,
    queryType: QueryType,
  ): Promise<void> {
    const baseDelay =
      queryType === "file" ? 100 : queryType === "ast" ? 200 : 300;
    const complexity = testData.length / 100;
   ...
   */
  private async simulateQueryWithCaching(
    testData: string,
    queryType: QueryType,
    cacheHit: boolean,
  ): Promise<void> {
  ...
  private async simulateQuery(
    ...
    const baseDelay =
      queryType === "file" ? 75 : queryType === "ast" ? 125 : 175;
    const complexity = testData.length / 100;
  ...
  private analyzeBenchmarkResults(
    ...
    const avgThroughput =
      successfulRuns.reduce((sum, run) => sum + run.throughput, 0) /
      successfulRuns.length;
    const avgMemoryUsed =
      successfulRuns.reduce((sum, run) => sum + run.memoryUsed, 0) /
      successfulRuns.length;
    const avgCpuUsage =
      successfulRuns.reduce((sum, run) => sum + run.cpuUsage, 0) /
      successfulRuns.length;
    const peakMemoryUsed = Math.max(
      ...successfulRuns.map((run) => run.memoryUsed),
    );
    const totalNodesProcessed = successfulRuns.reduce(
      (sum, run) => sum + run.nodeCount,
      0,
    );

    // Check performance targets
    const mcpRuns = successfulRuns.filter(
      (run) => run.metadata?.protocol === "mcp",
    );
    const cliRuns = successfulRuns.filter(
      (run) => run.metadata?.protocol === "cli",
    );

    const meetsMCPTarget =
      mcpRuns.length === 0 || mcpRuns.every((run) => run.duration < 200);
    const meetsCLITarget =
      cliRuns.length === 0 || cliRuns.every((run) => run.duration < 500);
    ...
    if (avgDuration > 200) {
      recommendations.push(
        "Optimize query processing algorithms to reduce response times",
      );
    }
    if (avgThroughput < 20) {
      recommendations.push(
        "Consider implementing query result caching for better throughput",
      );
    }
    if (peakMemoryUsed > 50) {
      recommendations.push(
        "Investigate memory usage patterns and implement cleanup routines",
      );
    }
    if (!meetsPerformanceTargets) {
      recommendations.push(
        "Focus on meeting MCP (<200ms) and CLI (<500ms) response time targets",
      );
    }
