...
export const DEFAULT_TELEMETRY_CONFIG: TelemetryConfig = {
  ...
  endpoint:
    process.env.AST_HELPER_TELEMETRY_ENDPOINT ||
    "https://api.ast-copilot-helper.dev/telemetry",
 ...
 */
export function getDefaultAnonymizationConfig(
 ...
 */
export function getTelemetryFeatures(
  privacyLevel: PrivacyLevel,
): TelemetryFeatures {
...
export function validateTelemetryConfig(
  ...
  if (config.flushInterval !== undefined) {
    if (
      ...
      config.flushInterval > 24 * 60 * 60 * 1000
    ) {
  ...
  if (config.retention?.maxEvents !== undefined) {
    if (
      config.retention.maxEvents < 100 ||
      config.retention.maxEvents > 100000
    ) {
...
export function getEnvironmentConfig(): Partial<TelemetryConfig> {
  ...
  if (process.env.AST_HELPER_TELEMETRY_PRIVACY_LEVEL) {
    const privacyLevel = process.env
      .AST_HELPER_TELEMETRY_PRIVACY_LEVEL as PrivacyLevel;
  ...
  if (process.env.AST_HELPER_TELEMETRY_FLUSH_INTERVAL) {
    const flushInterval = parseInt(
      process.env.AST_HELPER_TELEMETRY_FLUSH_INTERVAL,
      10,
    );
