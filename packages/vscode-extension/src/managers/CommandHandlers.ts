...
export class CommandHandlers {
   ...
   */
  public registerCommands(_context: vscode.ExtensionContext): vscode.Disposable[] {
  ...
  private async handleServerStatus(): Promise<void> {
    ...
    if (!this.serverProcessManager) {
      vscode.window.showInformationMessage('Server process manager not initialized');
      return;
    }
  ...
  private async handleOpenSettings(): Promise<void> {
    ...
    await vscode.commands.executeCommand('workbench.action.openSettings', 'astCopilotHelper');
  }
  ...
  private async handleParseWorkspace(): Promise<void> {
    ...
    if (!this.mcpClientManager || this.mcpClientManager.getState() !== 'connected') {
      const message = 'MCP client not connected. Please start the server first.';
      this.outputChannel.appendLine(message);
      vscode.window.showWarningMessage(message);
      return;
    }
    ...
    try {
      ...
      await vscode.window.withProgress({
        ...
        progress.report({ increment: 25, message: 'Connecting to AST server...' });

        // Call AST server to parse workspace
        const result = await this.mcpClientManager!.callTool('parse_workspace', {
          workspace_path: vscode.workspace.rootPath || '',
          include_patterns: vscode.workspace.getConfiguration('astCopilotHelper').get('includePatterns', ['**/*.ts', '**/*.js', '**/*.py']),
          exclude_patterns: vscode.workspace.getConfiguration('astCopilotHelper').get('excludePatterns', ['**/node_modules/**', '**/dist/**'])
        });

        progress.report({ increment: 75, message: 'Processing results...' });

        this.outputChannel.appendLine(`Workspace parsing result: ${JSON.stringify(result, null, 2)}`);
        vscode.window.showInformationMessage('Workspace parsed successfully!');
      });
    } catch (error: any) {
  ...
  private async handleClearIndex(): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(`Clear index result: ${JSON.stringify(result, null, 2)}`);
  ...
  private async handleAnalyzeIssue(): Promise<void> {
    ...
    try {
      await vscode.window.withProgress({
        ...
        progress.report({ increment: 25, message: 'Fetching issue details...' });

        const result = await this.mcpClientManager!.callTool('analyze_github_issue', {
        ...
        this.outputChannel.appendLine(`Issue analysis completed: ${JSON.stringify(result, null, 2)}`);
        vscode.window.showInformationMessage('GitHub issue analyzed successfully!');
      });
    } catch (error: any) {
  ...
  private async handleGenerateCode(): Promise<void> {
    ...
    try {
      await vscode.window.withProgress({
        ...
        const result = await this.mcpClientManager!.callTool('generate_code', {
        ...
        this.outputChannel.appendLine(`Code generation completed: ${JSON.stringify(result, null, 2)}`);
  ...
  private async handleCreatePullRequest(): Promise<void> {
    ...
    try {
      ...
      const currentBranch = branches.find((branch: any) => branch.name === repo.state.HEAD?.name);
      ...
      this.outputChannel.appendLine(`Pull request preparation completed: ${title}`);
  ...
  private async handleReviewCode(): Promise<void> {
    ...
    try {
      await vscode.window.withProgress({
        ...
        progress.report({ increment: 25, message: 'Analyzing code structure...' });
        ...
        this.outputChannel.appendLine(`Code review completed: ${JSON.stringify(result, null, 2)}`);
  ...
  private async handleValidateConfiguration(): Promise<void> {
    ...
    try {
      ...
      if (!serverPath) {
        vscode.window.showWarningMessage(
          ...
          if (selection === 'Open Settings') {
            vscode.commands.executeCommand('workbench.action.openSettings', 'astCopilotHelper.serverPath');
          }
        });
        return;
      }
      ...
      const connectionTimeout = config.get<number>('client.connectionTimeout', 30000);
      const requestTimeout = config.get<number>('client.requestTimeout', 15000);
      const healthCheckInterval = config.get<number>('server.healthCheckInterval', 30000);
      ...
      this.outputChannel.appendLine('Configuration validation completed successfully');
