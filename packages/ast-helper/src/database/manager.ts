...
export class ASTDatabaseManager {
    ...
    for (const dir of directories) {
      ...
      if (!dryRun) {
        try {
          ...
          throw DatabaseErrors.directoryCreationFailed(
            dir.path,
            error as Error,
          );
        }
      ...
      try {
        ...
        if (actualMode !== expectedMode) {
          result.warnings.push(
            `Directory permissions may be incorrect. Expected: ${expectedMode.toString(8)}, Got: ${actualMode.toString(8)}`,
          );
        }
      } catch (error) {
  ...
  async getDatabaseSize(): Promise<DatabaseSize> {
    ...
    const directories = [
      ...
      {
        path: structure.grammars,
        key: "grammars" as keyof typeof size.breakdown,
      },
  ...
  private async validateDiskSpace(targetPath: string): Promise<void> {
    ...
    try {
      ...
      throw DatabaseErrors.insufficientSpace(
        targetPath,
