...
import type {
  ...
  CertificationLevel,
} from "../src/deployment-certification/types.js";
...
describe("Deployment Certification Framework", () => {
  ...
  describe("DeploymentCertificationManager", () => {
    ...
    it("should emit events during certification", async () => {
      ...
      manager.on("certification:start", () =>
        events.push("certification:start"),
      );
      ...
      manager.on("certification:complete", () =>
        events.push("certification:complete"),
      );
    ...
    it("should calculate correct summary metrics", async () => {
      ...
      expect(["low", "medium", "high", "critical"]).toContain(
        result.summary.deploymentRisk,
      );
      expect(["proceed", "fix-issues", "rollback", "hold"]).toContain(
    ...
    it("should track performance metrics", async () => {
      ...
      expect(result.performance.averageDeploymentTime).toBeGreaterThanOrEqual(
        0,
      );
    ...
    it("should track resource usage", async () => {
      ...
      expect(
        result.resources.efficiency.resourceUtilization,
    ...
    it("should provide environment status", async () => {
      ...
      expect([
        ...
        "rolled-back",
        "approved",
      ]).toContain(result.environmentStatus.status);
    ...
    it("should include deployment strategy", async () => {
      ...
      expect(["blue-green", "canary", "rolling", "recreate"]).toContain(
    ...
    it("should handle approval workflow", async () => {
      ...
      expect(["approved", "rejected", "pending"]).toContain(
        result.approval.finalDecision,
      );
  ...
  describe("DeploymentCertificationConfigManager", () => {
    ...
    it("should provide environment-specific configurations", () => {
      ...
      environments.forEach((env) => {
        const envConfig =
          DeploymentCertificationConfigManager.getEnvironmentConfig(env);
