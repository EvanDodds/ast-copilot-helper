...
import type {
  ...
  ScalingPoint,
} from "../../../packages/ast-helper/src/performance/types";
...
describe("ConcurrencyProfiler", () => {
  ...
  describe("runConcurrencyBenchmarks", () => {
    ...
    it("should test multiple worker counts", async () => {
      ...
      result.scalabilityMetrics.throughputScaling.forEach(
        (point: ScalingPoint, index: number) => {
    ...
    it("should determine optimal worker count", async () => {
      ...
      expect(result.scalabilityMetrics.optimalWorkerCount).toBeLessThanOrEqual(
        mockConfig.maxWorkers,
      );
    });
    ...
    it("should handle errors gracefully", async () => {
      ...
      await expect(
        profiler.runConcurrencyBenchmarks(errorConfig),
      ).rejects.toThrow();
    });
  });
  ...
  describe("Task Generation", () => {
    it("should handle different workload types", async () => {
      ...
      const parsingResult =
        await profiler.runConcurrencyBenchmarks(parsingConfig);
      const queryingResult =
        await profiler.runConcurrencyBenchmarks(queryingConfig);
  ...
  describe("Scalability Analysis", () => {
    it("should analyze scaling metrics", async () => {
      ...
      result.scalabilityMetrics.latencyScaling.forEach(
        (point: ScalingPoint) => {
          expect(point).toHaveProperty("workerCount");
  ...
  describe("Error Handling", () => {
    it("should handle invalid configurations", async () => {
      ...
      await expect(
        profiler.runConcurrencyBenchmarks(invalidConfig),
      ).rejects.toThrow();
    });
    ...
    it("should handle worker creation failures", async () => {
      ...
      try {
        ...
        const { ConcurrencyProfiler } = await import(
          "../../../packages/ast-helper/src/performance/concurrency-profiler"
        );
        const failingProfiler = new ConcurrencyProfiler();

        await expect(
          failingProfiler.runConcurrencyBenchmarks(mockConfig),
        ).rejects.toThrow();
      } finally {
