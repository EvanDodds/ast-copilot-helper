...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `AST MCP Helper activation failed: ${error}`,
    );
  }
}
    ...
    if (serverProcessManager) {
      try {
        ...
        outputChannel?.appendLine(
          "Server stopped successfully during deactivation",
        );
      } catch (error) {
        outputChannel?.appendLine(
          `Failed to stop server during deactivation: ${error}`,
        );
      }
    }
 ...
 */
function initializeConfigurationWatcher(
  context: vscode.ExtensionContext,
): void {
  const configWatcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration("astCopilotHelper")) {
      outputChannel.appendLine(
        "AST Helper configuration changed, applying updates...",
      );
        ...
        try {
          ...
          if (uiManager) {
            uiManager.showErrorNotification(
              `Failed to auto-start server: ${error}`,
              true,
            );
          }
        }
