...
export class DatabaseConfigurationManager {
    ...
    async validateConfig(config: any): Promise<void> {
        ...
        const requiredStringFields = [
            'modelName', 'modelHost', 'version', 'created', 'lastUpdated'
        ];

        const requiredArrayFields = [
            'parseGlob', 'excludeGlob', 'watchGlob'
        ];

        const requiredNumberFields = [
            'watchDebounce', 'topK', 'snippetLines', 'batchSize',
            'concurrency', 'maxMemory'
        ];

        const requiredBooleanFields = [
            'enableTelemetry', 'enableNative'
        ];
        ...
        for (const field of requiredArrayFields) {
            ...
            } else if (!config[field].every((item: any) => typeof item === 'string')) {
        ...
        if (typeof config.indexParams !== 'object' || config.indexParams === null) {
            ...
            if (typeof indexParams.efConstruction !== 'number' ||
                indexParams.efConstruction < 16 ||
                indexParams.efConstruction > 800) {
                errors.push("indexParams.efConstruction must be a number between 16 and 800");
            }

            if (typeof indexParams.M !== 'number' ||
                indexParams.M < 4 ||
                indexParams.M > 64) {
                errors.push("indexParams.M must be a number between 4 and 64");
            }

            if (typeof indexParams.ef !== 'number' ||
                indexParams.ef < 16 ||
                indexParams.ef > 512) {
        ...
        if (config.snippetLines && (config.snippetLines < 1 || config.snippetLines > 50)) {
        ...
        if (config.concurrency && (config.concurrency < 1 || config.concurrency > 32)) {
            errors.push("concurrency must be between 1 and 32");
        }

        if (config.maxMemory && (config.maxMemory < 512 || config.maxMemory > 32768)) {
            errors.push("maxMemory must be between 512 and 32768 MB");
        }

        if (config.watchDebounce && (config.watchDebounce < 50 || config.watchDebounce > 5000)) {
