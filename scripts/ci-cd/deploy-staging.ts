...
class StagingDeployment {
  ...
  private getPreviousDeployment(): string | null {
    ...
    try {
      if (existsSync(statePath)) {
        ...
        const successfulDeployments = deploymentHistory.filter(d => d.success && d.healthCheckPassed);
  ...
  async deploy(): Promise<DeploymentResult> {
    ...
    try {
      ...
      this.log(`✅ Staging deployment completed successfully in ${duration} seconds`);
      ...
      if (this.config.rollbackEnabled && result.rollbackInfo?.previousVersion && result.rollbackInfo.previousVersion !== 'none') {
        this.log(`🔄 Attempting rollback to version: ${result.rollbackInfo.previousVersion}`);
        try {
