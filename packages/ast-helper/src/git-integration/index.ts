...
export class GitRepositoryValidator {
  ...
  async validateGitReference(ref: string, workspaceDir: string): Promise<void> {
    try {
      const isValid = await this.gitManager.validateGitReference(
        ref,
        workspaceDir,
      );
...
export class GitFileSelector {
  ...
  async selectFiles(
    ...
    try {
      ...
      if (options.base && options.base !== "HEAD") {
        await this.gitValidator.validateGitReference(
          options.base,
          workspaceDir,
        );
      }
      ...
      const repositoryRoot = await this.gitValidator
        .getGitManager()
        .getRepositoryRoot(workspaceDir);
      const candidateFiles = gitFiles.map((file) =>
        resolve(repositoryRoot, file),
      );

      const filesWithMetadata = await this.getFileMetadata(
        candidateFiles,
        repositoryRoot,
      );

      // Step 4: Apply filtering
      const { files, skipped, errors } = await this.filterFiles(
        filesWithMetadata,
        config,
      );
      ...
      filePaths.map(async (filePath) => {
        try {
          ...
          if (stats.isFile()) {
            ...
            const gitStatus = await this.determineGitStatus(
              filePath,
              repositoryRoot,
            );
   ...
   */
  private async determineGitStatus(
    _filePath: string,
    repositoryRoot: string,
  ): Promise<GitFileMetadata["gitStatus"]> {
    try {
      const gitManager = this.gitValidator.getGitManager();
    ...
    for (const fileMetadata of filesMetadata) {
      try {
        ...
        if (fileMetadata.size > maxFileSize) {
          errors.push(
            `${fileMetadata.relativePath}: File too large (${this.formatBytes(fileMetadata.size)} > 10MB)`,
          );
          continue;
        }
        ...
        if (config.parseGlob && config.parseGlob.length > 0) {
          ...
          const matchesConfig = this.matchesConfigPatterns(
        ...
        errors.push(
          `${fileMetadata.relativePath}: ${(error as Error).message}`,
        );
      }
    }
