...
describe('DependencyLicenseAnalyzer', () => {
  ...
  describe('initialization', () => {
    ...
    it('should initialize compatibility matrix', async () => {
      const result = await analyzer.analyzeDependencies('/test/project', 'MIT', {
  ...
  describe('dependency scanning', () => {
    ...
    it('should handle missing package.json gracefully', async () => {
      ...
      const result = await analyzer.analyzeDependencies('/nonexistent/project', 'MIT', {
        skipNpmRegistry: true
      });
...
Permission is hereby granted...`;
  ...
  describe('license conflict detection', () => {
    ...
    it('should handle unknown licenses based on conflict resolution mode', async () => {
      ...
      const strictResult = await analyzer.analyzeDependencies('/test/project', 'MIT', {
        skipNpmRegistry: true,
        conflictResolutionMode: 'strict'
      });
      ...
      const permissiveResult = await analyzer.analyzeDependencies('/test/project', 'MIT', {
        skipNpmRegistry: true,
  ...
  describe('error handling', () => {
    it('should handle dependency analysis failures gracefully', async () => {
      ...
      expect(result.dependencies).toHaveLength(2);
      expect(result.dependencies.some(d => d.name === 'working-package')).toBe(true);
      expect(result.dependencies.some(d => d.name === 'failing-package')).toBe(true);
      
      const failedDep = result.dependencies.find(d => d.name === 'failing-package');
  ...
  describe('options handling', () => {
    it('should respect depth option', async () => {
      ...
      const shallowResult = await analyzer.analyzeDependencies('/test/project', 'MIT', {
        skipNpmRegistry: true,
        depth: 0
      });
      ...
      const deepResult = await analyzer.analyzeDependencies('/test/project', 'MIT', {
        skipNpmRegistry: true,
        depth: 1
      });

      expect(deepResult.dependencies).toHaveLength(2);
      expect(deepResult.dependencies.some(d => d.name === 'child-package')).toBe(true);
      expect(deepResult.dependencies.some(d => d.name === 'grandchild-package')).toBe(true);
    });
  });
});