...
const downloader = new ModelDownloader({
  ...
  progressCallback: (progress) => {
    console.log(
      `${progress.percentage}% - ${progress.speed} MB/s - ETA: ${progress.eta}s`,
    );
  },
});
...
const completedPath = await downloader.resumeDownload(
  "./downloads/partial-model.onnx",
);
```
...
try {
  ...
  if (result.shouldRetry) {
    console.log(
      `Retrying in ${result.retryDelay}ms (attempt ${result.retryCount})`,
    );
...
_This API documentation is automatically generated from TypeScript source code and kept up to date with each release._
