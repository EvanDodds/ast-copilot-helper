...
export class ModelStatusCommandHandler {
  ...
  private async showModelStatus(
    ...
    if (cacheResult.hit) {
      ...
      if (cacheResult.metadata) {
        ...
        this.logger.info(
          `  Downloaded: ${metadata.downloadedAt.toLocaleString()}`,
        );
        ...
        if (metadata.usageStats) {
          ...
          this.logger.info(
            `  Last used: ${metadata.usageStats.lastUsed.toLocaleString()}`,
          );
        }
      }
    } else {
  ...
  private async showSystemStatus(
    ...
    const formatCounts = models.reduce(
      (acc, model) => {
    ...
    const utilization = ((cacheStats.totalSize / maxCacheSize) * 100).toFixed(
      1,
    );
    this.logger.info(
      `  Utilization: ${utilization}% (${this.formatBytes(cacheStats.totalSize)} / 10GB)`,
    );

    // Configuration
    this.logger.info("Configuration:");
    this.logger.info(
      `  Models directory: ${config.model?.modelsDir || "Default"}`,
    );
    this.logger.info(
      `  Download timeout: ${config.model?.downloadTimeout || "Default"} ms`,
    );
    this.logger.info(
      `  Max concurrent downloads: ${config.model?.maxConcurrentDownloads || "Default"}`,
    );
    this.logger.info(
      `  Progress reporting: ${config.model?.showProgress !== false ? "Enabled" : "Disabled"}`,
    );
    ...
    if (options.performance) {
      ...
      this.logger.info(
        `  Download speed: ${this.formatBytes(metrics.downloadSpeed)}/s`,
      );
      this.logger.info(
