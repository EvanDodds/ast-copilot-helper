...
describe("DatabaseConfigurationManager", () => {
  ...
  describe("configuration file creation", () => {
    ...
    it("should prevent overwriting without force flag", async () => {
      ...
      await expect(manager.createConfigurationFile(tempDir)).rejects.toThrow(
  ...
  describe("configuration validation", () => {
    ...
    it("should reject invalid string fields", async () => {
      ...
      await expect(manager.validateConfig(config)).rejects.toThrow(
        "Field 'modelName' must be a non-empty string",
      );
    });
    ...
    it("should reject invalid array fields", async () => {
      ...
      await expect(manager.validateConfig(config)).rejects.toThrow(
        "Field 'parseGlob' must be an array",
      );
    });
    ...
    it("should reject invalid timestamps", async () => {
      ...
      await expect(manager.validateConfig(config)).rejects.toThrow(
        "created must be a valid ISO timestamp",
      );
    });
  ...
  describe("configuration persistence", () => {
    ...
    it("should handle missing configuration file", async () => {
      ...
      const configPath = join(tempDir, "config.json");
      await expect(manager.loadConfig(tempDir)).rejects.toThrow(
        "Configuration file not accessible",
      );
    });
    ...
    it("should handle invalid JSON", async () => {
      ...
      await expect(manager.loadConfig(tempDir)).rejects.toThrow("Invalid JSON");
    });
  });
});
