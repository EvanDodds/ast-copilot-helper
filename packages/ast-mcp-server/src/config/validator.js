...
exports.validateConfigTypes = validateConfigTypes;
exports.validateConfigConstraints = validateConfigConstraints;
/**
...
function validateTransportConfig(transport, errors, warnings) {
  ...
  if (!["stdio", "websocket", "http"].includes(transport.type)) {
    errors.push(
      `Invalid transport type: ${transport.type}. Must be stdio, websocket, or http`,
    );
  }
  ...
  if (transport.maxConnections !== undefined) {
    if (transport.maxConnections < 1) {
      ...
      warnings.push(
        "maxConnections is very high, consider performance implications",
      );
    }
  }
  // Timeout validation
  if (
    transport.connectionTimeout !== undefined &&
    transport.connectionTimeout < 1000
  ) {
    warnings.push("connectionTimeout is very low, may cause connection issues");
  }
  if (
    transport.requestTimeout !== undefined &&
    transport.requestTimeout < 1000
  ) {
  ...
  if (transport.type === "websocket") {
    if (
      transport.maxMessageSize !== undefined &&
...
function validatePerformanceConfig(performance, errors, warnings) {
  ...
  if (
    performance.requestQueueSize !== undefined &&
    performance.requestQueueSize < 1
  ) {
  ...
  // Memory validation
  if (
    performance.maxMemoryUsage !== undefined &&
    performance.maxMemoryUsage < 64 * 1024 * 1024
  ) {
...
function validateLoggingConfig(logging, errors, warnings) {
  ...
  if (logging.level && !validLevels.includes(logging.level)) {
    errors.push(
      `Invalid log level: ${logging.level}. Must be one of: ${validLevels.join(", ")}`,
    );
  }
  ...
  if (
    logging.slowQueryThreshold !== undefined &&
    logging.slowQueryThreshold < 0
  ) {
  ...
  if (logging.logRequestBody || logging.logResponseBody) {
    warnings.push(
      "Request/response body logging may impact performance and security",
    );
  }
  if (logging.level === "trace" || logging.level === "debug") {
    warnings.push(
      `Log level '${logging.level}' may impact performance in production`,
    );
  }
}
/**
...
function validateSecurityConfig(security, errors, warnings) {
  ...
  if (
    security.rateLimitRequests !== undefined &&
    security.rateLimitRequests < 1
  ) {
    errors.push("rateLimitRequests must be at least 1");
  }
  if (
    security.rateLimitWindow !== undefined &&
    security.rateLimitWindow < 1000
  ) {
  ...
  if (!security.enableAuthentication) {
    warnings.push(
      "Authentication is disabled, consider enabling for production use",
    );
  }
}
/**
 * Validate database configuration
 */
function validateDatabaseConfig(database, errors, warnings) {
  if (!database || !database.path || database.path.trim().length === 0) {
    errors.push("Database path cannot be empty");
  }
  if (
    database &&
    database.backupInterval !== undefined &&
    database.backupInterval < 60000
  ) {
 ...
 * Environment-specific validation
 */
function validateEnvironmentSpecific(config, _errors, warnings) {
 ...
 */
function validateConfigTypes(_config) {
  const errors = [];
 ...
 */
function validateConfigConstraints(config) {
  const errors = [];
  // Cross-field validation
  if (config.performance.requestTimeout && config.transport.requestTimeout) {
    if (config.performance.requestTimeout > config.transport.requestTimeout) {
      errors.push(
        "Performance requestTimeout cannot exceed transport requestTimeout",
      );
    }
  }
  if (
    ...
    if (
      config.performance.maxConcurrentRequests >
      config.transport.maxConnections * 10
    ) {
      errors.push(
        "maxConcurrentRequests seems too high relative to maxConnections",
      );
    }
  }
  return errors;
