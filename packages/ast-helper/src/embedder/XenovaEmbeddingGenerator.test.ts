...
describe("XenovaEmbeddingGenerator - Xenova Integration", () => {
  ...
  describe("Model Initialization", () => {
    ...
    it("should throw error when trying to generate embeddings before initialization", async () => {
      await expect(generator.generateEmbeddings(["test"])).rejects.toThrow(
        EmbeddingGenerationError,
      );
    });
  ...
  describe("Configuration Validation", () => {
    it("should validate batch size limits when initialized", async () => {
      ...
      await expect(
        generator.batchProcess(mockAnnotations, { batchSize: 0 }),
      ).rejects.toThrow("Batch size must be between 1 and 128");

      await expect(
        generator.batchProcess(mockAnnotations, { batchSize: 200 }),
      ).rejects.toThrow("Batch size must be between 1 and 128");
    });
    ...
    it("should validate concurrency limits when initialized", async () => {
      ...
      await expect(
        generator.batchProcess(mockAnnotations, { maxConcurrency: 0 }),
      ).rejects.toThrow("Max concurrency must be between 1 and 8");

      await expect(
        generator.batchProcess(mockAnnotations, { maxConcurrency: 10 }),
      ).rejects.toThrow("Max concurrency must be between 1 and 8");
    });
  });
  ...
  describe("Enhanced Embedding Generation", () => {
    it("should use text processor for annotation preparation", () => {
      ...
      const prepareMethod = (generator as any).prepareTextForEmbedding.bind(
        generator,
      );
    ...
    it("should validate embedding vectors properly", () => {
      ...
      const validateMethod = (generator as any).validateEmbeddings.bind(
        generator,
      );
      ...
      // Wrong count
      expect(() => validateMethod(validEmbeddings, 3)).toThrow(
        "Expected 3 embeddings, got 2",
      );
    ...
    it("should calculate embedding confidence scores", () => {
      const confidenceMethod = (
        generator as any
      ).calculateEmbeddingConfidence.bind(generator);

      // Normal embedding vector
      const normalEmbedding = Array.from(
        { length: 768 },
        () => Math.random() * 2 - 1,
      );
