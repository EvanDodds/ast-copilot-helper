...
describe('IntegrityValidator', () => {
  ...
  describe('validateIntegrity', () => {
    ...
    it('should detect database structure corruption', async () => {
      ...
      expect(report.recommendedActions).toContain('Verify file system permissions and disk space');
    });
    ...
    it('should provide comprehensive performance metrics', async () => {
      ...
      expect(report.performanceMetrics).toBeDefined();
      expect(typeof report.performanceMetrics.structureCheckTime).toBe('number');
      expect(typeof report.performanceMetrics.embeddingCheckTime).toBe('number');
      expect(typeof report.performanceMetrics.metadataCheckTime).toBe('number');
      expect(typeof report.performanceMetrics.consistencyCheckTime).toBe('number');
      
      expect(report.performanceMetrics.structureCheckTime).toBeGreaterThanOrEqual(0);
      expect(report.performanceMetrics.embeddingCheckTime).toBeGreaterThanOrEqual(0);
      expect(report.performanceMetrics.metadataCheckTime).toBeGreaterThanOrEqual(0);
      expect(report.performanceMetrics.consistencyCheckTime).toBeGreaterThanOrEqual(0);
    });

    it('should handle validation errors gracefully', async () => {
      // Mock database manager throwing error
      mockDbManager.validateDatabaseStructure = vi.fn().mockRejectedValue(new Error('Database access failed'));
  ...
  describe('recoverFromCorruption', () => {
    it('should perform corruption recovery with default options', async () => {
      const mockReport = {
        ...
        embeddingIntegrity: { isValid: false, totalFiles: 2, corruptedFiles: ['corrupted.json'] },
        metadataIntegrity: { isValid: false, totalMetadata: 1, corruptedMetadata: ['bad_metadata.json'], checksumMismatches: 1 },
        fileConsistency: { isValid: false, totalFiles: 3, orphanedFiles: 1, missingReferences: 0 },
    ...
    it('should track recovery metrics correctly', async () => {
      const mockReport = {
        ...
        duration: 100,
        databaseIntegrity: { isValid: true },
        embeddingIntegrity: { isValid: false, totalFiles: 2, corruptedFiles: ['file1.json', 'file2.json'] },
        metadataIntegrity: { isValid: true, totalMetadata: 0, corruptedMetadata: [], checksumMismatches: 0 },
        fileConsistency: { isValid: true, totalFiles: 0, orphanedFiles: 0, missingReferences: 0 },
        recommendedActions: [],
        performanceMetrics: {
  ...
  describe('Integration scenarios', () => {
    it('should provide valid recommendations for healthy database', async () => {
      ...
      if (report.isValid) {
        expect(report.recommendedActions).toContain('Database integrity is good - no action required');
      }
  ...
  describe('Error handling', () => {
    it('should handle database structure validation errors', async () => {
      mockDbManager.validateDatabaseStructure = vi.fn().mockRejectedValue(new Error('Connection failed'));
