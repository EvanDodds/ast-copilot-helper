...
class WorkflowOptimizer {
  ...
  private identifyBottlenecks(): void {
    ...
    const averageDuration =
      this.workflowMetrics.reduce((sum, metric) => sum + metric.duration, 0) /
      this.workflowMetrics.length;
    ...
    bottlenecks.forEach((job) => {
      console.log(
        `  • ${job.jobName}: ${job.duration}s (${Math.round((job.duration / averageDuration - 1) * 100)}% above average)`,
      );
    });
  }
  ...
  private generateOptimizationSuggestions(): void {
    ...
    this.optimizationSuggestions.push({
      ...
      implementation:
        "Add cache key based on package-lock.json hash with fallback keys",
    });

    // Parallelization improvements
    const parallelizableJobs = this.workflowMetrics.filter(
      (m) => m.parallelizable,
    );
    if (parallelizableJobs.length > 1) {
      this.optimizationSuggestions.push({
        ...
        implementation:
          "Configure matrix strategy for build-and-test and performance-benchmarks",
      });
    }

    // Resource allocation optimization
    const highResourceJobs = this.workflowMetrics.filter(
      (m) => m.resources.memory > 2048,
    );
    if (highResourceJobs.length > 0) {
      this.optimizationSuggestions.push({
        ...
        implementation:
          "Use larger runners for high-resource jobs, standard for others",
      });
    }
  ...
  private calculatePotentialImprovements(): void {
    const totalImprovement = this.optimizationSuggestions.reduce(
      (sum, suggestion) => sum + suggestion.expectedImprovement,
      0,
    );

    const currentTotalTime = this.workflowMetrics.reduce(
    ...
    console.log(
      `  Current total time: ${Math.round(currentTotalTime / 60)}m ${currentTotalTime % 60}s`,
    );
    console.log(
      `  Optimized time: ${Math.round(optimizedTime / 60)}m ${Math.round(optimizedTime % 60)}s`,
    );
    console.log(
      `  Time savings: ${Math.round((currentTotalTime - optimizedTime) / 60)}m ${Math.round((currentTotalTime - optimizedTime) % 60)}s (${Math.round((1 - optimizedTime / currentTotalTime) * 100)}%)`,
    );
  }
  ...
  public async generateOptimizationReport(): Promise<void> {
    ...
    const report = {
      ...
      currentMetrics: {
        ...
        totalDuration: this.workflowMetrics.reduce(
          (sum, m) => sum + m.duration,
          0,
        ),
        averageDuration:
          this.workflowMetrics.reduce((sum, m) => sum + m.duration, 0) /
          this.workflowMetrics.length,
        successRate:
          (this.workflowMetrics.filter((m) => m.status === "success").length /
            this.workflowMetrics.length) *
          100,
        parallelizationRatio:
          (this.workflowMetrics.filter((m) => m.parallelizable).length /
            this.workflowMetrics.length) *
          100,
      },
      optimizations: {
        suggestions: this.optimizationSuggestions,
        totalExpectedImprovement: this.optimizationSuggestions.reduce(
...
<body>
    <div class="container">
        ...
        ${report.optimizations.suggestions
          .map(
            (s: any) => `
