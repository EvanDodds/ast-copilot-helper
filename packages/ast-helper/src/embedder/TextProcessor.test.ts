...
describe('CodeTextProcessor - Text Preparation', () => {
  ...
  describe('Code-Specific Processing', () => {
    it('should clean function signatures properly', () => {
      const annotation: Annotation = {
        nodeId: 'test-sig',
        signature: 'public async function calculateTotal(items: Item[]): Promise<number>',
        summary: 'Test'
      };
  ...
  describe('Text Validation', () => {
    ...
    it('should detect excessively long text', () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.issues.some(issue => issue.includes('exceeds maximum length'))).toBe(true);
    });
    
    it('should detect excessive repetition', () => {
      const repetitiveText = 'test test test test test test test test test test test test';
      const validation = processor.validateInputText(repetitiveText);
      
      expect(validation.isValid).toBe(false);
      expect(validation.issues.some(issue => issue.includes('excessive repetition'))).toBe(true);
    });
  });
  ...
  describe('Configuration Management', () => {
    ...
    it('should preserve other config values when updating', () => {
      ...
      const updatedConfig = processor.getConfig();
      expect(updatedConfig.preserveCodeStructure).toBe(initialConfig.preserveCodeStructure);
      expect(updatedConfig.maxSnippetLength).toBe(initialConfig.maxSnippetLength);
    });
  });
