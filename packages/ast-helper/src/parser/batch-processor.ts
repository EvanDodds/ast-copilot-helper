...
export class BatchProcessor extends EventEmitter {
  ...
  private cache = new Map<
    string,
    { result: ParseResult; hash: string; timestamp: number }
  >();
  ...
  private performanceMetrics: {
    ...
    languageStats: Map<
      string,
      { files: number; nodes: number; parseTime: number; errors: number }
    >;
  } = {
    startTime: 0,
    parseTimes: [],
  ...
  async processBatch(
    ...
    try {
      ...
      const { supportedFiles, unsupportedFiles, oversizedFiles } =
        await this.validateFiles(files, opts);

      // Handle unsupported files
      this.handleUnsupportedFiles(
        unsupportedFiles,
        "unsupported",
        results,
        summary,
  ...
  private async processFileChunk(
    ...
    const promises = files.map(async (file, index) => {
      ...
      return this.processFile(
        ...
        options,
        completedCount,
        files.length + baseCompletedCount,
      );
    });
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (options.continueOnError) {
        const settledResult = fileResults[
          i
        ] as PromiseSettledResult<ParseResult>;
        if (settledResult && settledResult.status === "rejected") {
          ...
          const result = (settledResult as PromiseFulfilledResult<ParseResult>)
            .value;
          this.processParseResult(
            file,
            absolutePath,
            result,
            results,
   ...
   */
  private async shouldThrottleForMemory(
    maxMemoryMB?: number,
  ): Promise<boolean> {
    if (!maxMemoryMB) {
      return false;
    ...
    this.memoryMonitor = setInterval(() => {
      ...
      if (this.performanceMetrics.memoryHistory.length > 1000) {
        this.performanceMetrics.memoryHistory =
          this.performanceMetrics.memoryHistory.slice(-1000);
      }
    }, 1000);
  }
  ...
  private async withTimeout<T>(
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error(`Parse timeout: ${context} (${timeoutMs}ms)`)),
          timeoutMs,
        ),
      ),
    ]);
  ...
  private emitProgress(
    ...
    const progress: BatchProcessingProgress = {
      ...
      avgParseTimeMs:
        this.performanceMetrics.parseTimes.length > 0
          ? this.performanceMetrics.parseTimes.reduce((a, b) => a + b, 0) /
            this.performanceMetrics.parseTimes.length
          : 0,
    };
  ...
  private calculateMemoryStats(summary: BatchProcessingSummary): void {
    ...
    summary.memoryStats.avgUsageMB =
  ...
  private generateMetrics(): BatchProcessingMetrics {
    ...
    const parseTimeDistribution = {
      ...
      p50:
        parseTimes.length > 0
          ? (parseTimes[Math.floor(parseTimes.length * 0.5)] ?? 0)
          : 0,
      p95:
        parseTimes.length > 0
          ? (parseTimes[Math.floor(parseTimes.length * 0.95)] ?? 0)
          : 0,
      p99:
        parseTimes.length > 0
          ? (parseTimes[Math.floor(parseTimes.length * 0.99)] ?? 0)
          : 0,
    };
  ...
  private mergeOptions(
    ...
    return {
      concurrency:
        options.concurrency ?? Math.min(8, Math.max(1, os.cpus().length)),
