...
describe('ConfigManager', () => {
  ...
  describe('Default Configuration', () => {
    ...
    it('should use environment-specific defaults', async () => {
      ...
      expect(config.logging.level).toBe(DEVELOPMENT_CONFIG.logging?.level || 'info');
      expect(config.features.enableTestEndpoints).toBe(DEVELOPMENT_CONFIG.features?.enableTestEndpoints ?? false);
    });
  });
  ...
  describe('File Configuration', () => {
    ...
    it('should merge file config with defaults', async () => {
      ...
      await fs.promises.writeFile(configPath, JSON.stringify(partialConfig, null, 2));
    ...
    it('should handle file loading errors gracefully', async () => {
      ...
      await expect(configManager.loadConfig({ configFile: nonExistentPath })).rejects.toThrow('Configuration file not found');
    });
    ...
    it('should handle invalid JSON gracefully', async () => {
      ...
      await expect(configManager.loadConfig({ configFile: invalidJsonPath })).rejects.toThrow('Invalid JSON in configuration file');
    });
  });
  ...
  describe('Validation Integration', () => {
    it('should validate configuration after loading', async () => {
      ...
      await expect(configManager.loadConfig({ configFile: configPath, strictMode: true })).rejects.toThrow('Configuration validation failed');
    });
    ...
    it('should allow loading invalid config with validation disabled', async () => {
      ...
      const config = await configManager.loadConfig({ configFile: configPath, validateConfig: false });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle permission errors gracefully', async () => {
      ...
      await expect(configManager.loadConfig({ configFile: configPath })).rejects.toThrow('Permission denied reading configuration file');
