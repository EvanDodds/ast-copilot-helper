...
describe("Input Validation System", () => {
      ...
      it("should allow adding custom validation rules", async () => {
        const customRule: ValidationRule = {
          ...
          validate: async (input: unknown) => {
            ...
            const isValid =
              typeof input === "string" && emailPattern.test(input);
            return {
              isValid,
  ...
  describe("Input Validation Configuration", () => {
    it("should use default configuration correctly", () => {
      ...
      expect(DEFAULT_INPUT_VALIDATION_CONFIG.commandInjection.enabled).toBe(
        true,
      );
  ...
  describe("Input Validation Utilities", () => {
    describe("Quick Validation", () => {
      it("should validate email addresses", async () => {
        expect(
          await InputValidationUtils.quickValidate("user@example.com", "email"),
        ).toBe(true);
        expect(
          await InputValidationUtils.quickValidate("invalid-email", "email"),
        ).toBe(false);
      });

      it("should validate URLs", async () => {
        expect(
          await InputValidationUtils.quickValidate(
            "https://example.com",
            "url",
          ),
        ).toBe(true);
        expect(
          await InputValidationUtils.quickValidate("not-a-url", "url"),
        ).toBe(false);
      });

      it("should validate phone numbers", async () => {
        expect(
          await InputValidationUtils.quickValidate("+1 234 567 8900", "phone"),
        ).toBe(true);
        expect(
          await InputValidationUtils.quickValidate("invalid-phone", "phone"),
        ).toBe(false);
      });

      it("should validate IP addresses", async () => {
        expect(
          await InputValidationUtils.quickValidate("192.168.1.1", "ip"),
        ).toBe(true);
        expect(
          await InputValidationUtils.quickValidate("999.999.999.999", "ip"),
        ).toBe(false);
      });
      ...
      it("should validate UUIDs", async () => {
        ...
        expect(
          await InputValidationUtils.quickValidate("not-a-uuid", "uuid"),
        ).toBe(false);
      });
    });
    ...
    describe("Validation Middleware", () => {
      it("should create Express.js middleware", () => {
        const middleware =
          InputValidationUtils.createValidationMiddleware(validator);
