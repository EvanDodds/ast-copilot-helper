...
describe("Maintenance Scripts", () => {
  ...
  describe("health-check.mjs", () => {
    ...
    it("should calculate overall health score correctly", () => {
      mockHealthChecker.determineOverallStatus.mockImplementation(
        (percentage: number) => {
          if (percentage >= 90) {
            return "excellent";
          }
  ...
  describe("cleanup.mjs", () => {
    it("should identify build artifacts for cleanup", () => {
      ...
      mockCleanup.findFiles.mockImplementation((pattern: string) => {
        ...
        if (pattern === "*.tsbuildinfo") {
          return [
            "/fake/tsconfig.tsbuildinfo",
            "/fake/packages/ast-helper/tsconfig.tsbuildinfo",
          ];
        }
        return [];
      });
    ...
    it("should identify old log files", () => {
      ...
      mockCleanup.isFileOld.mockImplementation((path: string, days: number) => {
        if (path.includes("old.log") && days === 7) {
          return true;
        }
        if (path.includes("recent.log") && days === 30) {
          return false;
        }
        return false;
      });
    ...
    it("should respect dry-run mode", () => {
      mockCleanup.shouldDelete.mockImplementation(
        (item: string, type: string, dryRun?: boolean) => {
          if (dryRun) {
            return false;
          }
          return true;
      ...
      const shouldDelete = mockCleanup.shouldDelete(
        "/fake/file",
        "test file",
        true,
      );
      ...
      const shouldDeleteForce = mockCleanup.shouldDelete(
        "/fake/file",
        "test file",
        false,
      );
  ...
  describe("Error Handling", () => {
    it("should handle network errors gracefully", () => {
      ...
      mockDependencyUpdater.checkOutdated.mockResolvedValue({
        packages: [],
...
describe("Maintenance Configuration", () => {
  it("should load and validate configuration files", () => {
    ...
    const config = JSON.parse(
      vi.mocked(readFileSync)(".maintenance-config.json", "utf-8") as string,
    );
...
describe("Maintenance Configuration", () => {
  it("should load and validate configuration files", async () => {
    ...
    // Test configuration loading
    const config = JSON.parse(
      readFileSync(".maintenance-config.json", "utf-8"),
    );
  ...
  it("should handle missing configuration gracefully", async () => {
    ...
    // Should use default configuration when file doesn't exist
    const { DependencyUpdater } = await import(
      "../../../scripts/maintenance/update-dependencies.mjs"
    );
...
describe("Performance Tests", () => {
  it("should handle large repositories efficiently", async () => {
    ...
    const { RepositoryCleanup } = await import(
      "../../../scripts/maintenance/cleanup.mjs"
    );
