...
import { SecurityValidator, SecurityConfigPresets } from './packages/ast-helper/dist/security-compliance/index.js';

console.log('🔒 Starting Security and Compliance Validation Demo...\n');

async function runSecurityValidationDemo() {
  try {
    console.log('1. Creating security validator with production configuration...');
    ...
    console.log(`   ✓ High Severity Issues: ${result.summary.highSeverityIssues}`);
    ...
    result.categories.forEach((category, index) => {
      console.log(`   ${index + 1}. ${category.category}: ${category.passed ? '✅' : '❌'} (${category.score}%)`);
    });
    console.log('');
    ...
    console.log(`   ✓ Dependencies Scanned: ${result.environment.dependencies.length}`);
    console.log(`   ✓ Network Encryption: ${result.environment.networkConfig.encryption ? 'Enabled' : 'Disabled'}\n`);

    console.log('5. Compliance assessment:');
    console.log(`   ✓ Overall Compliance Score: ${result.compliance.overallScore}%`);
    console.log(`   ✓ Standards Assessed: ${result.compliance.standards.length}`);
    console.log(`   ✓ Certifications: ${result.compliance.certifications.length}`);
