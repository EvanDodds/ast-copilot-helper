...
describe('Node Classifier', () => {
  ...
  describe('TypeScript Classification', () => {
    const createTSNode = (type: string, name?: string, parent?: RawNodeData): RawNodeData => ({
      type,
      name,
      language: 'typescript',
      parent,
    ...
    it('should classify TypeScript control flow correctly', () => {
      ...
      expect(classifier.classifyNode(ifNode).nodeType).toBe(NodeType.IF_STATEMENT);
      expect(classifier.classifyNode(forNode).nodeType).toBe(NodeType.FOR_LOOP);
      expect(classifier.classifyNode(whileNode).nodeType).toBe(NodeType.WHILE_LOOP);
    });
    ...
    it('should classify TypeScript imports/exports correctly', () => {
      ...
      expect(classifier.classifyNode(importNode).nodeType).toBe(NodeType.IMPORT);
      expect(classifier.classifyNode(exportNode).nodeType).toBe(NodeType.EXPORT);
    });

    it('should handle context-aware classification', () => {
      const classParent = createTSNode('class_declaration', 'MyClass');
      const identifierNode = createTSNode('identifier', 'className', classParent);
      
      const result = classifier.classifyNode(identifierNode, { parent: classParent });
  ...
  describe('JavaScript Classification', () => {
    ...
    it('should classify JavaScript functions correctly', () => {
      ...
      expect(classifier.classifyNode(funcDecl).nodeType).toBe(NodeType.FUNCTION);
      expect(classifier.classifyNode(funcExpr).nodeType).toBe(NodeType.FUNCTION);
      expect(classifier.classifyNode(arrowFunc).nodeType).toBe(NodeType.ARROW_FUNCTION);
    });
  ...
  describe('Python Classification', () => {
    const createPyNode = (type: string, name?: string, parent?: RawNodeData): RawNodeData => ({
      type,
      name,
      language: 'python',
      parent,
    ...
    it('should classify Python functions correctly', () => {
      ...
      expect(classifier.classifyNode(funcNode).nodeType).toBe(NodeType.FUNCTION);
      expect(classifier.classifyNode(lambdaNode).nodeType).toBe(NodeType.ARROW_FUNCTION);
    });
    ...
    it('should classify Python control flow correctly', () => {
      ...
      expect(classifier.classifyNode(forNode).nodeType).toBe(NodeType.FOR_LOOP);
      expect(classifier.classifyNode(whileNode).nodeType).toBe(NodeType.WHILE_LOOP);
      expect(classifier.classifyNode(withNode).nodeType).toBe(NodeType.TRY_CATCH);
      expect(classifier.classifyNode(tryNode).nodeType).toBe(NodeType.TRY_CATCH);
    });

    it('should handle Python context rules', () => {
      const funcParent = createPyNode('function_definition', 'my_function');
      const identifierNode = createPyNode('identifier', 'func_name', funcParent);
      
      const result = classifier.classifyNode(identifierNode, { parent: funcParent });
    ...
    it('should classify Python class attributes', () => {
      ...
      const assignmentNode = createPyNode('assignment', 'attribute', classParent);
      
      const result = classifier.classifyNode(assignmentNode, { parent: classParent });
  ...
  describe('Statistics Tracking', () => {
    ...
    it('should reset statistics correctly', () => {
      const node: RawNodeData = { type: 'class_declaration', language: 'typescript' };
  ...
  describe('Accuracy Validation', () => {
    it('should validate classification accuracy', () => {
      const testData = [
        { node: { type: 'class_declaration', language: 'typescript' }, expected: NodeType.CLASS },
        { node: { type: 'function_declaration', language: 'typescript' }, expected: NodeType.FUNCTION },
        { node: { type: 'variable_declarator', language: 'typescript' }, expected: NodeType.VARIABLE },
        { node: { type: 'unknown_type', language: 'typescript' }, expected: NodeType.VARIABLE }, // This should be wrong
      ];
  ...
  describe('ClassificationUtils', () => {
    describe('isContainerType', () => {
      it('should identify container types correctly', () => {
        ...
        expect(ClassificationUtils.isContainerType(NodeType.FUNCTION)).toBe(true);
        expect(ClassificationUtils.isContainerType(NodeType.IF_STATEMENT)).toBe(true);
        
        expect(ClassificationUtils.isContainerType(NodeType.VARIABLE)).toBe(false);
        expect(ClassificationUtils.isContainerType(NodeType.PARAMETER)).toBe(false);
        expect(ClassificationUtils.isContainerType(NodeType.STRING_LITERAL)).toBe(false);
      });
    });

    describe('isDeclarationType', () => {
      it('should identify declaration types correctly', () => {
        expect(ClassificationUtils.isDeclarationType(NodeType.CLASS)).toBe(true);
        expect(ClassificationUtils.isDeclarationType(NodeType.FUNCTION)).toBe(true);
        expect(ClassificationUtils.isDeclarationType(NodeType.VARIABLE)).toBe(true);
        expect(ClassificationUtils.isDeclarationType(NodeType.INTERFACE)).toBe(true);
        
        expect(ClassificationUtils.isDeclarationType(NodeType.IF_STATEMENT)).toBe(false);
        expect(ClassificationUtils.isDeclarationType(NodeType.COMMENT)).toBe(false);
        expect(ClassificationUtils.isDeclarationType(NodeType.STRING_LITERAL)).toBe(false);
      });
    });
    ...
    describe('getHierarchyLevel', () => {
      it('should return correct hierarchy levels', () => {
        ...
        expect(ClassificationUtils.getHierarchyLevel(NodeType.FUNCTION)).toBe(4);
        expect(ClassificationUtils.getHierarchyLevel(NodeType.VARIABLE)).toBe(6);
        expect(ClassificationUtils.getHierarchyLevel(NodeType.PARAMETER)).toBe(7);
