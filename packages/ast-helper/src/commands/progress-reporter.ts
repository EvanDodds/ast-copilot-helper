...
export class ProgressReporter extends EventEmitter<ProgressReporterEvents> {
  ...
  constructor(logger: Logger, options: Partial<ProgressDisplayOptions> = {}) {
    super();
    this.logger = logger;
  ...
  update(update: ParseProgressUpdate): void {
    ...
    this.stats = {
      ...
      filesPerSecond:
        update.rate || this.calculateRate(update.completed, elapsedMs),
      estimatedTimeRemainingMs:
        update.estimatedTimeRemaining ||
        this.calculateETA(update.completed, update.total, elapsedMs),
    ...
    if (update.memoryUsageMB) {
      this.stats.peakMemoryUsageMB = Math.max(
        this.stats.peakMemoryUsageMB,
        update.memoryUsageMB,
      );
      this.stats.memoryPressure = this.calculateMemoryPressure(
        update.memoryUsageMB,
      );
    }
  ...
  private formatProgressLine(): string {
    ...
    if (this.displayOptions.showFileDetails && stats.currentFile) {
      ...
      currentFileStr =
        relativePath.length > this.displayOptions.maxFilePathLength
          ? "..." +
    ...
    if (this.displayOptions.showMemoryUsage && stats.memoryUsageMB > 0) {
      const memColor =
        colors && stats.memoryPressure === "high" ? "\x1b[31m" : "";
  ...
  private displayFinalSummary(): void {
    ...
    const duration = this.formatDuration(
      Math.round(stats.elapsedTimeMs / 1000),
    );
    ...
    console.log(
      `${successColor}✓ Processed: ${stats.completedFiles}${resetColor} files in ${duration}`,
    );
    if (stats.failedFiles > 0) {
      console.log(
        `${errorColor}✗ Failed: ${stats.failedFiles}${resetColor} files`,
      );
    }
    console.log(
      `${infoColor}Performance:${resetColor} ${avgRate} files/sec, ${avgNodes} avg nodes/file`,
    );
    if (stats.totalNodes > 0) {
      console.log(
        `${infoColor}AST Nodes:${resetColor} ${(stats.totalNodes / 1000).toFixed(1)}k total`,
      );
    }
    if (stats.memoryUsageMB > 0) {
      console.log(
        `${infoColor}Memory:${resetColor} ${stats.peakMemoryUsageMB.toFixed(0)}MB peak usage`,
      );
    }
  }
