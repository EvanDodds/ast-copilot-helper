...
describe("EmbedCommand", () => {
  ...
  describe("execute", () => {
    it("should validate configuration on dry run", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "✅ Dry run completed - configuration valid",
      );
    });
    ...
    it("should handle empty annotations gracefully", async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        "⚠️  No annotations found to embed",
      );
    });
    ...
    it("should process annotations and generate embeddings", async () => {
      ...
      vi.spyOn(embedCommand as any, "storeResults").mockResolvedValue(
        undefined,
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "📊 Processing 1 annotations",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "🔄 Embedding 1 new annotations",
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        "✅ Embedding generation completed: 1 embeddings generated",
      );
    });
    ...
    it("should skip already embedded annotations unless force is used", async () => {
      ...
      vi.spyOn(
        embedCommand as any,
        "filterUnembeddedAnnotations",
      ).mockResolvedValue([]); // No unembedded
      mockGenerator.initialize.mockResolvedValue(undefined);
    ...
    it("should handle errors gracefully", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        "❌ Embedding generation failed: Test error",
      );
    });
  ...
  describe("configuration validation", () => {
    it("should validate batch size limits", async () => {
      ...
      await expect(embedCommand.execute(options)).rejects.toThrow(
    ...
    it("should validate memory limits", async () => {
      ...
      await expect(embedCommand.execute(options)).rejects.toThrow(
        "Memory limit must be between 512 MB and 16 GB",
      );
    });
  });
