...
export class CompatibilityCheckerImpl implements CompatibilityChecker {
  ...
  async initialize(config: CompatibilityConfig): Promise<void> {
    ...
    if (config.breakingChangeThreshold && (config.breakingChangeThreshold <= 0 || config.breakingChangeThreshold > 1)) {
  ...
  async checkApiCompatibility(baseVersion: string, newVersion: string): Promise<CompatibilityCheck> {
    ...
    if (!this.validateVersion(baseVersion) || !this.validateVersion(newVersion)) {
    ...
    try {
      ...
      console.log('📊 API Changes detected:', JSON.stringify(apiChanges, null, 2));
      ...
      const rawBreakingChanges = mockBreakingChanges.length > 0 ? mockBreakingChanges : (apiChanges.breakingChanges || []);
      
      const breakingChanges: BreakingChange[] = rawBreakingChanges.map((change: any) => ({
        description: change.description,
        migration: change.migration || 'Update API calls to match new signature',
        affectedApi: change.methods || [],
        severity: change.severity || 'major',
        automatedMigration: false
      }));
      
      console.log('🔍 Mapped Breaking Changes:', JSON.stringify(breakingChanges, null, 2));
  ...
  async checkConfigCompatibility(baseVersion: string, newVersion: string): Promise<CompatibilityCheck> {
    ...
    try {
      const configChanges = await this.analyzeConfigChanges(baseVersion, newVersion);
      ...
      return {
        ...
        migrationRequired: false,
        confidence: 0.5,
        details: [`Configuration check failed: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
  ...
  async checkCliCompatibility(baseVersion: string, newVersion: string): Promise<CompatibilityCheck> {
    ...
    try {
      ...
      return {
        ...
        migrationRequired: false,
        confidence: 0.3,
        details: [`CLI check failed: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
  ...
  async checkDataFormatCompatibility(baseVersion: string, newVersion: string): Promise<CompatibilityCheck> {
    ...
    try {
      const dataFormatChanges = await this.analyzeDataFormatChanges(baseVersion, newVersion);
      
      let breakingChanges: BreakingChange[] = [];
      
      // Handle real implementation structure
      if (dataFormatChanges.breakingChanges && Array.isArray(dataFormatChanges.breakingChanges)) {
        breakingChanges = dataFormatChanges.breakingChanges.map((change: any) => ({
          description: change.description,
      ...
      const migrationRequired = breakingChanges.length > 0 || (dataFormatChanges as any)?.migrationRequired === true;
      ...
      return {
        ...
        migrationRequired: false,
        confidence: 0.3,
        details: [`Data format check failed: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
  ...
  async generateMigrationGuide(baseVersion: string, newVersion: string, providedBreakingChanges?: BreakingChange[]): Promise<MigrationGuide> {
    this.ensureInitialized();
    ...
    if (breakingChanges && breakingChanges.length > 0) {
      ...
      for (const change of breakingChanges) {
        const step = {
          ...
          type: change.automatedMigration ? 'automated' as const : 'manual' as const,
          actions: [{
            type: change.automatedMigration ? SuggestionActionType.CONFIG_UPDATE : SuggestionActionType.CODE_CHANGE,
            description: change.migration || 'Update required',
            automated: change.automatedMigration || false,
            riskLevel: change.severity === 'major' ? RiskLevel.HIGH : RiskLevel.MEDIUM
          }],
    ...
    return {
      ...
      description: steps.length > 1 ? 'Breaking changes require migration steps.' : 'This release is backward compatible with no migration required.',
      steps,
  ...
  private async analyzeCliChanges(baseVersion: string, newVersion: string): Promise<any> {
    // In a real implementation, this would:
  ...
  private calculateConfidence(_checkType: string, analysisResult: any): number {
    ...
    if (analysisResult) {
      ...
      if (analysisResult.breakingChanges && Array.isArray(analysisResult.breakingChanges)) {
