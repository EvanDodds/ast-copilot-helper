...
export class UIManager {
    ...
    if (this.serverProcessManager) {
      ...
      this.serverProcessManager.on("started", (info) => {
        this.showSuccessNotification(
          `AST MCP Server started (PID: ${info.pid})`,
        );
      });
      ...
      this.serverProcessManager.on("crashed", (exitCode, signal) => {
        this.showWarningNotification(
          `Server crashed (exit code: ${exitCode}, signal: ${signal})`,
        );
      });

      this.serverProcessManager.on("restarting", (attempt, maxAttempts) => {
        this.showInfoNotification(
          `Restarting server (attempt ${attempt}/${maxAttempts})`,
        );
      });
    }
  ...
  public updateStatusBar(state?: string): void {
    ...
    try {
      const serverState =
        state || this.serverProcessManager?.getState() || "unknown";
  ...
  public showSuccessNotification(message: string, showActions?: boolean): void {
    ...
    if (showActions) {
      vscode.window
        .showInformationMessage(message, "Show Logs", "View Status")
        .then((selection) => {
          switch (selection) {
            case "Show Logs":
  ...
  public showWarningNotification(message: string, showActions?: boolean): void {
    ...
    if (showActions) {
      vscode.window
        .showWarningMessage(message, "Show Logs", "Retry")
        .then((selection) => {
          switch (selection) {
            case "Show Logs":
              this.outputChannel.show();
  ...
  public showErrorNotification(message: string, showActions?: boolean): void {
    ...
    if (showActions) {
      vscode.window
        .showErrorMessage(message, "Show Logs", "Report Issue", "Open Settings")
        .then((selection) => {
          switch (selection) {
            case "Show Logs":
              this.outputChannel.show();
            ...
            case "Report Issue":
              vscode.env.openExternal(
                vscode.Uri.parse(
                  "https://github.com/EvanDodds/ast-copilot-helper/issues",
                ),
              );
              break;
            case "Open Settings":
  ...
  public showProgressNotification(
    ...
    let progressResolver:
      | ((
          value: vscode.Progress<{ message?: string; increment?: number }>,
        ) => void)
      | null = null;
    ...
    switch (state) {
      ...
      case "running":
        if (
          ...
          this.mcpClientManager.getState() === "disconnected"
        ) {
      ...
      default:
        this.outputChannel.appendLine(
          `No retry action available for state: ${state}`,
        );
    }
  }
