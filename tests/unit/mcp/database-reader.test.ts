...
import type { ASTNode, ASTNodeMatch, QueryOptions } from '../../../packages/ast-mcp-server/src/types.js';
...
describe('ASTDatabaseReader', () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(reader['dbManager'], 'validateDatabaseStructure').mockResolvedValue({
      isValid: true,
      errors: [],
      warnings: [],
      missingDirectories: [],
  ...
  describe('node retrieval', () => {
    ...
    it('should get child nodes', async () => {
      ...
      expect(children.every((child: ASTNode) => child.parentId === parentId)).toBe(true);
    });
  });
  ...
  describe('hot reload', () => {
    it('should setup hot reload detection when enabled', async () => {
      ...
      vi.spyOn(hotReloadReader['dbManager'], 'isInitialized').mockResolvedValue(true);
      vi.spyOn(hotReloadReader['dbManager'], 'validateDatabaseStructure').mockResolvedValue({
        isValid: true,
        errors: [],
        warnings: [],
        missingDirectories: [],
      ...
      vi.spyOn(hotReloadReader['dbManager'], 'getDatabaseStructure').mockReturnValue({
        root: mockDbPath,
        asts: join(mockDbPath, 'asts'),
        annots: join(mockDbPath, 'annots'),
        grammars: join(mockDbPath, 'grammars'),
