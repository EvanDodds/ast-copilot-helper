...
if (!nativeBinding) {
  ...
  console.warn(
    "⚠️  No native binding found - creating runtime stub for TypeScript compilation",
  );
  console.warn(
    "   This is normal in CI environments. The module will fail at runtime until Rust binaries are built.",
  );
  ...
  nativeBinding = {
    AstCoreEngineApi: class AstCoreEngineApi {
      constructor() {
        throw new Error(
          "Native binding not available - run cargo build --release",
        );
      }
      initialize() {
        throw new Error(
...
const { AstCoreEngineApi, createDefaultEngine } = nativeBinding;
