...
class ProductionDeployment {
  ...
  constructor() {
    this.deploymentLogPath = path.join(process.cwd(), 'production-deployment.log');
    
    // Get deployment configuration from environment or defaults
    this.config = {
      environment: 'production',
      version: process.env.CI_COMMIT_SHA?.substring(0, 8) || this.getLatestCommit(),
  ...
  private async executeCommand(command: string, description: string): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Deployment step failed: ${description} - ${error.message}`);
    }
  }
  ...
  private async comprehensiveHealthCheck(): Promise<{ [url: string]: boolean }> {
    ...
    for (const url of this.config.healthCheckUrls) {
      ...
      try {
        ...
        if (success) {
          ...
          this.log(`❌ Health check failed for ${url} after ${maxAttempts} attempts`);
        }
      } catch (error: any) {
    ...
    this.log(`Health check summary: ${passedCount}/${totalCount} endpoints healthy`);
  ...
  private saveDeploymentState(result: DeploymentResult): void {
    const statePath = path.join(process.cwd(), 'production-deployment-state.json');
  ...
  private getPreviousDeployment(): string | null {
    ...
    try {
      if (existsSync(statePath)) {
        const deploymentHistory: DeploymentResult[] = JSON.parse(readFileSync(statePath, 'utf8'));
        const successfulDeployments = deploymentHistory.filter(d => d.success && d.allHealthChecksPassed);
  ...
  async deploy(): Promise<DeploymentResult> {
    ...
    this.log(`Strategy: ${this.config.blueGreenDeployment ? 'Blue-Green' : 'Rolling'}`);
    ...
    const result: DeploymentResult = {
      ...
      deploymentStrategy: this.config.blueGreenDeployment ? 'blue-green' : 'rolling'
    };
    ...
    try {
      ...
      await this.executeCommand('yarn run build', 'Building project for production');
      await this.executeCommand('yarn run test:all', 'Running comprehensive tests');
      await this.executeCommand('yarn run security:audit', 'Running security audit');
      await this.executeCommand('yarn run quality:check', 'Running quality checks');
      ...
      if (this.config.rollbackEnabled && result.rollbackInfo?.previousVersion && result.rollbackInfo.previousVersion !== 'none') {
        this.log(`🔄 Attempting production rollback to version: ${result.rollbackInfo.previousVersion}`);
        try {
  ...
  async rollback(targetVersion: string): Promise<void> {
    ...
    try {
      ...
      this.log(`✅ Production rollback to version ${targetVersion} completed successfully`);
    } catch (error: any) {
