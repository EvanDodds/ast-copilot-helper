...
export class AttributionGenerator {
   ...
   */
  async generateThirdPartyLicenseFile(
    dependencies: DependencyLicense[],
  ): Promise<AttributionDocument> {
  ...
  async generateCreditsFile(
    ...
    const sortedPackages = Array.from(packageContributors.entries()).sort(
  ...
  async generateLicenseMetadata(
    ...
    const metadata = {
      ...
      complianceInfo: {
        attributionRequiredCount: dependencies.filter(
          (d) => d.attributionRequired,
        ).length,
        uniqueLicenses: Array.from(
          new Set(dependencies.map((d) => d.license.spdxId || d.license.name)),
        ),
        copyleftDependencies: dependencies.filter(
          (d) => d.license.compatibility.isCopeyleft,
        ).length,
      },
    };
  ...
  async applyCustomTemplate(
    ...
    try {
      const templateContent = await fs.readFile(
        this.config.customAttributionTemplate,
        "utf8",
      );
      ...
      console.warn(
        `Failed to apply custom template ${this.config.customAttributionTemplate}:`,
        error,
      );
      return null;
    }
