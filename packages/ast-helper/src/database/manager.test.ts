...
describe("ASTDatabaseManager", () => {
  ...
  describe("database structure validation", () => {
    ...
    it("should detect missing directories", async () => {
      ...
      expect(
        result.errors.some((e: string) =>
          e.includes("Missing required directory"),
        ),
      ).toBe(true);
    });
    ...
    it("should detect missing configuration files", async () => {
      ...
      expect(result.errors.some((e: string) => e.includes("config.json"))).toBe(
        true,
      );
      expect(
        result.errors.some((e: string) => e.includes("version.json")),
      ).toBe(true);
    });
  });
  ...
  describe("database size calculation", () => {
    ...
    it("should calculate size correctly for initialized database", async () => {
      ...
      expect(size.breakdown.other).toBe(
        configContent.length + versionContent.length,
      );
    });
  });
  ...
  describe("utility methods", () => {
    ...
    it("should get complete database structure", () => {
      ...
      expect(structure.config).toBe(
        join(tempWorkspace, ".astdb", "config.json"),
      );
      expect(structure.version).toBe(
        join(tempWorkspace, ".astdb", "version.json"),
      );
    });
    ...
    it("should ensure subdirectory exists", async () => {
      ...
      const customPath =
        await manager.ensureDirectoryExists("custom/nested/path");
      const fs = (manager as any).fs;
  ...
  describe("error handling", () => {
    it("should handle permission errors gracefully", async () => {
      ...
      await expect(
        readOnlyManager.createDirectoryStructure(),
      ).rejects.toThrow();
    });
    ...
    it("should validate disk space before operations", async () => {
      ...
      await expect(
        manager.createDirectoryStructure(options),
