...
export class TreeSitterGrammarManager implements GrammarManager {
  ...
  async getCachedGrammarPath(language: string): Promise<string> {
    const grammarPath = path.join(this.grammarDir, language, `tree-sitter-${language}.wasm`);
  ...
  async verifyGrammarIntegrity(language: string): Promise<boolean> {
    try {
      ...
      return await this.verifyDownloadedGrammar(grammarPath, config.grammarHash);
    } catch (error) {
      return false;
    }
  }
  ...
  async loadParser(language: string): Promise<any> {
    ...
    throw new Error(`Parser loading not yet implemented - grammar available for ${language}`);
  }
  ...
  private async isGrammarCached(language: string): Promise<boolean> {
    ...
    return (await this.fileExists(grammarPath)) && (await this.fileExists(metadataPath));
  }

  /**
   * Download a file with retry logic
   */
  private async downloadWithRetry(url: string, filePath: string): Promise<void> {
    ...
    throw new Error(`Failed to download ${url} after ${this.maxRetries} attempts: ${lastError?.message}`);
  }
  ...
  private async downloadFile(url: string, filePath: string): Promise<void> {
    ...
    throw new Error('Grammar download not implemented - install tree-sitter grammars manually or set NODE_ENV=test');
  }

  /**
   * Verify a downloaded grammar file against its expected hash
   */
  private async verifyDownloadedGrammar(filePath: string, expectedHash: string): Promise<boolean> {
    try {
