...
describe("Configuration Validator", () => {
  ...
  describe("Basic Validation", () => {
    ...
    it("should require server version", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Server version is required and cannot be empty",
      );
    });
  ...
  describe("Transport Validation", () => {
    ...
    it("should reject invalid transport types", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid transport type: invalid. Must be stdio, websocket, or http",
      );
    });
    ...
    it("should require port for non-stdio transports", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Port is required for websocket transport",
      );
    });
    ...
    it("should warn about unused settings for stdio", () => {
      ...
      expect(result.warnings).toContain(
        "Port setting ignored for stdio transport",
      );
      expect(result.warnings).toContain(
        "Host setting ignored for stdio transport",
      );
    });
  });
  ...
  describe("Performance Validation", () => {
    ...
    it("should warn about high query result limits", () => {
      ...
      expect(result.warnings).toContain(
        "maxQueryResults is very high, may impact performance",
      );
    });
  ...
  describe("Logging Validation", () => {
    ...
    it("should reject invalid log levels", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Invalid log level: invalid. Must be one of: error, warn, info, debug, trace",
      );
    });
    ...
    it("should warn about performance impact of verbose logging", () => {
      ...
      expect(result.warnings).toContain(
        "Log level 'debug' may impact performance in production",
      );
    });
  ...
  describe("Security Validation", () => {
    ...
    it("should validate TLS configuration", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "TLS keyFile is required when certFile is specified",
      );
    });
    ...
    it("should warn about CORS wildcards", () => {
      ...
      expect(result.warnings).toContain(
        "CORS origin wildcard (*) should be avoided in production",
      );
    });
  ...
  describe("Environment-Specific Validation", () => {
    ...
    it("should warn about test endpoints in production", () => {
      ...
      expect(result.warnings).toContain(
        "Test endpoints enabled in production environment",
      );
    });
    ...
    it("should warn about verbose logging in test environment", () => {
      ...
      expect(result.warnings).toContain(
        "Verbose logging in test environment may slow down tests",
      );
    });
  });
