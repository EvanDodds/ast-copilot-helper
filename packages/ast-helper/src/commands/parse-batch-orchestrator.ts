...
export class ParseBatchOrchestrator extends EventEmitter<ParseBatchOrchestratorEvents> {
  ...
  async processFiles(
    ...
    try {
      ...
      const batchOptions = this.createEnhancedBatchOptions(
        parseOptions,
        config,
      );
  ...
  private prioritizeFiles(
    files: string[],
    priority?: ParseBatchOptions["processingPriority"],
  ): string[] {
  ...
  private setupMemoryMonitoring(options: ParseBatchOptions): void {
    ...
    this.memoryMonitor = setInterval(() => {
      ...
      if (
        ...
        memoryUsageMB > options.memoryThresholdMB
      ) {
  ...
  private createCurrentProgress(): ParseBatchProgress {
    ...
    const filesPerSecond =
      this.performanceMetrics.totalFilesProcessed / Math.max(1, elapsed / 1000);
    const estimatedCompletion = new Date(
      now +
        (elapsed / Math.max(1, this.processedChunks)) *
          (this.totalChunks - this.processedChunks),
    );
    ...
    return {
      ...
      skipped: 0,
      avgParseTimeMs:
        this.performanceMetrics.totalFilesProcessed > 0
          ? this.performanceMetrics.totalParseTime /
  ...
  private updatePerformanceMetrics(result: BatchProcessingResult): void {
    ...
    this.performanceMetrics.totalMemoryUsage =
      result.summary.memoryStats.peakUsageMB;
