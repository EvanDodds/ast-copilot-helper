...
export class PerformanceBenchmarkRunner extends EventEmitter {
  ...
  public async runBenchmark(
    ...
    try {
      ...
      const result: PerformanceTestResult = {
        ...
        suggestions: passed
          ? []
          : await this.generateSuggestions(benchmark, measurement),
      };
      ...
      const failedResult: PerformanceTestResult = {
        ...
        suggestions: [
          "Fix the underlying issue preventing benchmark execution",
        ],
      };
  ...
  public async runBenchmarks(
    benchmarks: PerformanceBenchmark[] = DEFAULT_PERFORMANCE_BENCHMARKS,
  ): Promise<PerformanceTestResult[]> {
  ...
  public async runScalabilityTest(
    ...
    for (
      let scale = test.workload.startValue;
      scale <= test.workload.endValue;
      scale += test.workload.stepSize
    ) {
  ...
  public async runCpuProfile(): Promise<CpuProfileResult> {
    ...
    const averageUsage =
      ((endCpu.user + endCpu.system) / (duration * 1000)) * 100;
  ...
  public async validatePerformance(): Promise<PerformanceValidationResult> {
    ...
    const criticalFailures = benchmarkResults.filter(
      (r) => !r.passed && this.isCriticalBenchmark(r.benchmark),
    ).length;
    const score = (passedBenchmarks / totalBenchmarks) * 100;
    const overallPassed =
      score >= this.config.thresholds.minSuccessRate && criticalFailures === 0;
  ...
  private async measureCliPerformance(
    benchmark: PerformanceBenchmark,
  ): Promise<number> {
  ...
  private async measureMcpServerPerformance(
    benchmark: PerformanceBenchmark,
  ): Promise<number> {
    switch (benchmark.name) {
  ...
  private async measureVSCodePerformance(
    benchmark: PerformanceBenchmark,
  ): Promise<number> {
  ...
  private calculateDeviation(
    measured: number,
    target: { value: number; constraint: string },
  ): number {
  ...
  private createErrorMeasurement(
    benchmark: PerformanceBenchmark,
    error: unknown,
  ): PerformanceMeasurement {
    return {
  ...
  private async runScalabilityStep(
    test: ScalabilityTest,
    scale: number,
    ...
    for (const metric of test.metrics) {
      switch (metric) {
        ...
        case "command-response-time":
          measurements[metric] =
            baseTime * scaleFactor * (0.8 + Math.random() * 0.4);
          break;
        case "memory-usage":
          measurements[metric] = 50 * scaleFactor * (0.9 + Math.random() * 0.2);
          break;
        case "error-rate":
          measurements[metric] = Math.max(
            0,
            (scaleFactor - 1) * 0.01 * Math.random(),
          );
          break;
        default:
  ...
  private evaluateScalabilityStep(
    _test: ScalabilityTest,
  ...
  private analyzeScalingBehavior(
    workloadPoints: WorkloadPoint[],
    test: ScalabilityTest,
  ): "linear" | "logarithmic" | "constant" | "degraded-graceful" | "failure" {
  ...
  private async simulateCliParsing(projectPath: string): Promise<number> {
    const baseTime = projectPath.includes("large")
      ? 8000
      : projectPath.includes("medium")
        ? 1500
        : 300;
  ...
  private async generateSuggestions(
    ...
    if (
      benchmark.category === "cli" &&
      measurement.value > benchmark.target.value
    ) {
  ...
  private generateScalabilityRecommendations(
    ...
    if (maxSuccessful < maxPossible * 0.8) {
      recommendations.push(
        `Scalability limited to ${maxSuccessful}/${maxPossible} test points - consider architectural improvements`,
      );
    }
  ...
  private generateMemoryRecommendations(
    ...
    if (peak > 1024 * 1024 * 1024) {
      // 1GB
      recommendations.push(
        "Peak memory usage exceeds 1GB - consider memory optimization",
      );
    }

    if (peak / average > 3) {
      recommendations.push(
        "High memory spikes detected - review memory allocation patterns",
      );
    }
  ...
  private async generatePerformanceRecommendations(
    ...
    const failedScalability = scalabilityResults.filter(
      (r) => r.behavior === "failure",
    );
