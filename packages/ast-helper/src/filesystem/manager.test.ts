...
describe("FileSystemManager", () => {
  ...
  describe("path operations", () => {
    it("should normalize paths", () => {
      ...
      expect(fsManager.normalizePath("path/to/../file.txt")).toBe(
        `path${sep}file.txt`,
      );
      expect(fsManager.normalizePath("path//double//slash")).toBe(
        `path${sep}double${sep}slash`,
      );
    });
  ...
  describe("file operations", () => {
    ...
    it("should create parent directories for atomic write", async () => {
      ...
      const parentExists = await fsManager.exists(
        join(tempDir, "nested", "deep"),
      );
    ...
    it("should not overwrite existing file by default", async () => {
      ...
      await expect(fsManager.copyFile(sourcePath, destPath)).rejects.toThrow(
  ...
  describe("file listing", () => {
    beforeEach(async () => {
      ...
      await writeFile(
        join(tempDir, "subdir1", "nested", "file4.py"),
        "content",
      );
    });
    ...
    it("should apply custom filter", async () => {
      ...
      const expectedPath = files.find(
        (f) => f.includes("subdir1") && f.endsWith("file3.txt"),
      );
  ...
  describe("utility methods", () => {
    ...
    it("should set file permissions (where supported)", async () => {
      ...
      await expect(
        fsManager.setFilePermissions(filePath, 0o755),
