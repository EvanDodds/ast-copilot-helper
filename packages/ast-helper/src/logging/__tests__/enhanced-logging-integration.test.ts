...
describe("Enhanced Logging Integration", () => {
  ...
  it("should demonstrate embedding operation tracking", async () => {
    ...
    const operationLogs = logCalls.filter(
      (call) => call[0]?.includes?.("operation") || call[1]?.operation,
    );
    ...
    const completionLog = logCalls.find(
      (call) =>
        ...
        call[1]?.event === "operation_complete",
    );
  ...
  it("should demonstrate error handling with context", async () => {
    ...
    const errorLog = mockConsoleLog.mock.calls.find(
      (call) =>
  ...
  it("should demonstrate hierarchical operation tracking", async () => {
    ...
    logger.addCheckpoint(childContext, "ast_parsing_complete", {
      nodesProcessed: 1500,
    });
  ...
  it("should demonstrate performance metrics collection", async () => {
    ...
    logger.addCheckpoint(context, "batch_processing_complete", {
      processingTime: 15,
    });
