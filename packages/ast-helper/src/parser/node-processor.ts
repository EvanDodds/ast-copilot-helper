...
export class NodeProcessor {
  ...
  async processFile(
    ...
    try {
      ...
      const rootNode: Partial<ASTNode> = {
        ...
        children: [],
        sourceText: this.config.includeSourceText
      ...
      if (this.config.enableSerialization && nodes.length > 0) {
        serializedPath = await this.serializeNodes(
          nodes,
          filePath,
          language,
          fileHash,
   ...
   * Validate processed nodes
   */
  validateProcessedNodes(nodes: ASTNode[]): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (const node of nodes) {
      ...
      if (node.start && node.end) {
        if (node.start.line > node.end.line) {
          errors.push(
            `Invalid position range for node ${node.id}: start line > end line`,
          );
        }
        if (
          ...
          errors.push(
            `Invalid position range for node ${node.id}: start column > end column`,
          );
        }
      }
      ...
      if (
        node.significance !== undefined &&
        !Object.values(SignificanceLevel).includes(node.significance)
      ) {
        errors.push(
          `Invalid significance level for node ${node.id}: ${node.significance}`,
        );
      }
    }
  ...
  getProcessingStatsSummary(results: ProcessingResult[]): {
    ...
    const totalNodes = successful.reduce(
      (sum, r) => sum + r.stats.totalNodes,
      0,
    );
    const averageProcessingTime =
      successful.length > 0
        ? successful.reduce((sum, r) => sum + r.stats.processingTimeMs, 0) /
          successful.length
        : 0;

    const nodeDistribution: Record<NodeType, number> = {} as any;
    const significanceDistribution: Record<SignificanceLevel, number> =
      {} as any;

    for (const result of successful) {
      for (const [type, count] of Object.entries(result.stats.nodesByType)) {
        nodeDistribution[type as NodeType] =
          (nodeDistribution[type as NodeType] || 0) + count;
      }

      for (const [significance, count] of Object.entries(
        result.stats.nodesBySignificance,
      )) {
        const sig = significance as unknown as SignificanceLevel;
        significanceDistribution[sig] =
          (significanceDistribution[sig] || 0) + count;
      }
    }
   ...
   */
  private async applyProcessingStages(
    nodes: ASTNode[],
    ...
    try {
      ...
      if (this.config.calculateSignificance) {
        processedNodes = processedNodes.map((node) => {
          if (node.significance === undefined) {
            ...
            node.significance =
              this.significanceCalculator.calculateSignificance(
                node,
                significanceContext,
              );
          }
      ...
      if (this.config.generateSignatures || this.config.calculateComplexity) {
        processedNodes = processedNodes.map((node) => {
          if (
            this.config.generateSignatures &&
            !node.signature &&
            this.shouldGenerateSignature(node)
          ) {
            node.signature = this.generateSignature(node);
          }

          if (
            this.config.calculateComplexity &&
  ...
  private async serializeNodes(
    ...
    await this.serializer.saveToFile(
      nodes,
      filePath,
      outputPath,
      language,
  ...
  private calculateProcessingStats(
    ...
    for (const node of nodes) {
      ...
      nodesBySignificance[node.significance] =
        (nodesBySignificance[node.significance] || 0) + 1;
    }

    const nodesPerSecond =
      processingTimeMs > 0
        ? (nodes.length * 1000) / processingTimeMs
        : nodes.length > 0
          ? nodes.length * 1000
          : 0; // If 0ms but nodes exist, assume 1ms
    const avgTimePerNodeUs =
      nodes.length > 0 ? (processingTimeMs * 1000) / nodes.length : 0;
  ...
  private calculateOverallStats(
    results: ProcessingResult[],
    totalTimeMs: number,
  ): BatchProcessingResult["overallStats"] {
    const successful = results.filter((r) => r.success);
    const failed = results.filter((r) => !r.success);
    ...
    for (const result of successful) {
      for (const [type, count] of Object.entries(result.stats.nodesByType)) {
        nodesByType[type as NodeType] =
          (nodesByType[type as NodeType] || 0) + count;
      }
    ...
    const avgPeakMemory =
      successful.length > 0 ? totalPeakMemory / successful.length : 0;
    const avgAvgMemory =
      successful.length > 0 ? totalAvgMemory / successful.length : 0;

    const nodesPerSecond =
      totalTimeMs > 0
        ? (totalNodes * 1000) / totalTimeMs
        : totalNodes > 0
          ? totalNodes * 1000
          : 0; // If 0ms but nodes exist, assume 1ms
    const avgTimePerNodeUs =
      totalNodes > 0 ? (totalTimeMs * 1000) / totalNodes : 0;
      ...
      performance: {
        nodesPerSecond:
          totalProcessingTime > 0
            ? (totalNodes * 1000) / totalProcessingTime
            : totalNodes > 0
              ? totalNodes * 1000
              : 0, // If 0ms but nodes exist, assume 1ms
        avgTimePerNodeUs:
          totalNodes > 0 ? (totalProcessingTime * 1000) / totalNodes : 0,
      },
    };
