...
import { FileSystemErrors, GitErrors, ValidationErrors } from '../errors/factories.js';
import { FileSystemManager } from '../filesystem/manager.js';
import { GitManager } from '../git/manager.js';
import { createLogger } from '../logging/index.js';
...
export class WorkspaceDetector {
    ...
    async detectWorkspace(
        ...
        try {
            workspaceInfo = await this.detectByGitRepository(startDirResolved, indicators);
            this.logger.debug('Detected workspace by Git repository', { root: workspaceInfo.root });
        } catch (error) {
            this.logger.debug('Git repository detection failed', { error: (error as Error).message });
        }

        // Method 2: Package.json or other project files
        if (!workspaceInfo) {
            try {
                workspaceInfo = await this.detectByProjectFiles(startDirResolved, indicators, maxTraversal);
                this.logger.debug('Detected workspace by project files', { root: workspaceInfo.root });
            } catch (error) {
                this.logger.debug('Project file detection failed', { error: (error as Error).message });
            }
        }
        ...
        if (!workspaceInfo) {
            if (allowExisting) {
                workspaceInfo = await this.createCurrentDirectoryWorkspace(startDirResolved, indicators);
                this.logger.debug('Using current directory as workspace', { root: workspaceInfo.root });
            } else {
        ...
        workspaceInfo = await this.enhanceWorkspaceInfo(workspaceInfo, startDirResolved);
    ...
    async validateWorkspaceForDatabase(
        ...
        try {
            ...
            await this.fs.atomicWriteFile(testFile, testContent, { encoding: 'utf8' });
    ...
    private async detectByProjectFiles(
        ...
        while (currentDir !== rootPath && traversalCount < maxTraversal) {
            ...
            if (foundIndicators.length > 0) {
                ...
                const gitRoot = await this.git.getRepositoryRoot(currentDir).catch(() => null);
    ...
    private async createCurrentDirectoryWorkspace(
        ...
        const gitRoot = await this.git.getRepositoryRoot(startDir).catch(() => null);
     ...
     */
    private async findIndicators(dir: string, indicators: string[]): Promise<string[]> {
        const found: string[] = [];
     ...
     */
    private async detectNestedWorkspace(workspaceInfo: WorkspaceInfo): Promise<boolean> {
     ...
     */
    async suggestWorkspaceRoots(startDir: string = process.cwd()): Promise<string[]> {
        ...
        while (currentDir !== rootPath && traversalCount < maxTraversal) {
            const indicators = await this.findIndicators(currentDir, DEFAULT_WORKSPACE_INDICATORS);
