...
export class MetadataManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error(
        `Failed to initialize metadata manager: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async storeMetadata(
    ...
    try {
      ...
      logger.debug(
        `Stored metadata for ${modelConfig.name} v${modelConfig.version}`,
      );
    } catch (error) {
      logger.error(
        `Failed to store metadata for ${modelConfig.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async getMetadata(modelConfig: ModelConfig): Promise<ModelMetadata | null> {
    try {
      ...
      logger.debug(
        `Metadata not found for ${modelConfig.name} v${modelConfig.version}`,
      );
      return null;
    }
  }
    ...
    try {
      ...
      for (const modelInfo of index.models) {
        ...
        if (
          query.minUsage &&
          (!metadata.usageStats ||
            metadata.usageStats.loadCount < query.minUsage)
        ) {
          continue;
        }
        if (query.maxAge) {
      ...
      logger.error(
        `Failed to query metadata: ${error instanceof Error ? error.message : String(error)}`,
      );
      return [];
    }
  }
  ...
  async updateMetadata(
    ...
    try {
      ...
      if (!existing) {
        logger.warn(
          `Cannot update metadata for non-existent model: ${modelConfig.name} v${modelConfig.version}`,
        );
        return false;
      }
      ...
      logger.debug(
        `Updated metadata for ${modelConfig.name} v${modelConfig.version}`,
      );
      return true;
    } catch (error) {
      logger.error(
        `Failed to update metadata for ${modelConfig.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
  ...
  async removeMetadata(modelConfig: ModelConfig): Promise<boolean> {
    try {
      ...
      logger.debug(
        `Removed metadata for ${modelConfig.name} v${modelConfig.version}`,
      );
      return true;
    } catch (error) {
      logger.error(
        `Failed to remove metadata for ${modelConfig.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
  ...
  async synchronize(): Promise<{
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `Could not scan models directory: ${error instanceof Error ? error.message : String(error)}`,
        );
      }

      logger.info(
        `Metadata synchronization complete: ${stats.added} added, ${stats.removed} removed, ${stats.updated} updated`,
      );
      return stats;
    } catch (error) {
      logger.error(
        `Failed to synchronize metadata: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      logger.error(
        `Failed to get metadata stats: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async recordModelUsage(
    ...
    try {
      ...
      if (!metadata) {
        logger.warn(
          `No metadata found for ${modelConfig.name}, creating basic entry`,
        );
        metadata = {
          config: modelConfig,
      ...
      logger.debug(
        `Recorded usage for ${modelConfig.name}: ${usageData.itemsProcessed} items, ${usageData.processingTime}ms`,
      );
    } catch (error) {
      logger.error(
        `Failed to record usage for ${modelConfig.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async getUsageAnalytics(
    modelConfig: ModelConfig,
  ): Promise<ModelUsageAnalytics | null> {
    try {
      ...
      const firstUsedDate =
        stats.firstUsed instanceof Date
          ? stats.firstUsed
      ...
      logger.error(
        `Failed to get usage analytics for ${modelConfig.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  async getSystemUsageAnalytics(): Promise<SystemUsageAnalytics> {
    try {
      ...
      analytics.averageSuccessRate =
        allMetadata.length > 0 ? totalSuccessRate / allMetadata.length : 0;
      ...
      if (analytics.averageSuccessRate < 90) {
        analytics.recommendedActions.push(
          "Consider model optimization - success rate below 90%",
        );
      }
      ...
      if (avgProcessingTime > 1000) {
        analytics.recommendedActions.push(
          "Performance optimization needed - average processing time > 1s",
        );
      }
      ...
      logger.error(
        `Failed to get system usage analytics: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  private generateOptimizationRecommendations(
    ...
    if (stats.successRate < 95) {
      recommendations.push(
        `Success rate is ${stats.successRate.toFixed(1)}% - investigate error patterns`,
      );
    }

    if (stats.averageProcessingTime > 500) {
      recommendations.push(
        `Average processing time ${stats.averageProcessingTime.toFixed(0)}ms is high - consider optimization`,
      );
    }

    if (stats.errorCount > stats.loadCount * 0.1) {
      recommendations.push(
        `Error rate ${((stats.errorCount / stats.loadCount) * 100).toFixed(1)}% is high - review error handling`,
      );
    }
    ...
    if (recentPerformance.length >= 5) {
      const recentAverage =
        recentPerformance.reduce(
          (sum: number, entry) => sum + entry.processingTime,
          0,
        ) / recentPerformance.length;
      if (recentAverage > stats.averageProcessingTime * 1.5) {
        recommendations.push(
          "Recent performance degradation detected - consider model refresh",
        );
      }
    }
    ...
    if (stats.cacheHitRate !== undefined && stats.cacheHitRate < 50) {
      recommendations.push(
        `Cache hit rate ${stats.cacheHitRate.toFixed(1)}% is low - review caching strategy`,
      );
    }
 ...
 */
export async function queryModelMetadata(
  query?: MetadataQuery,
