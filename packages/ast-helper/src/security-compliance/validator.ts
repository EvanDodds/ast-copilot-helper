...
export class SecurityValidator extends EventEmitter {
   ...
   */
  private async runCategoryTest(
    category: SecurityCategory,
  ): Promise<SecurityCategoryResult> {
    ...
    const criticalIssues = tests.filter(
  ...
  private async runVulnerabilityScanning(): Promise<SecurityTestResult[]> {
    const config = this.config.getCategoryConfig(
  ...
  private calculateSummary(
    ...
    const highSeverityIssues = categories.reduce(
      (sum, cat) =>
        sum +
        cat.tests.filter((t) => !t.passed && t.severity === "high").length,
      0,
    );
    ...
    const complianceScore =
      ...
      Math.max(
        1,
        categories.filter((cat) => cat.category === "compliance").length,
      );
  ...
  private async assessCompliance(
    _categories: SecurityCategoryResult[],
