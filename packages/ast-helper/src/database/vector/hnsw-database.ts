...
export class HNSWVectorDatabase implements VectorDatabase {
  ...
  async initialize(_config?: VectorDBConfig): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to initialize HNSW vector database: ${(error as Error).message}`);
    }
  }
  ...
  private async initializeHNSWIndex(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to initialize HNSW index: ${(error as Error).message}`);
    }
  }
  ...
  private async rebuildIndexFromStorage(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to rebuild index from storage: ${(error as Error).message}`);
    }
  }
   ...
   */
  async insertVector(nodeId: string, vector: number[], metadata: VectorMetadata): Promise<void> {
  ...
  async insertVectors(vectors: VectorInsert[]): Promise<void> {
    ...
    for (const vector of vectors) {
      try {
        ...
        const wasStorageSuccess = !storageResult.errors.some(e => e.nodeId === vector.nodeId);
        ...
        if (wasStorageSuccess) {
          ...
          if (label) {
            ...
            console.warn(`Label mapping not found for nodeId: ${vector.nodeId}`);
          }
        }
      } catch (error) {
        console.warn(`HNSW insertion failed for ${vector.nodeId}: ${(error as Error).message}`);
      }
    }
  ...
  async searchSimilar(vector: number[], k = 10, ef?: number): Promise<SearchResult[]> {
    ...
    try {
      ...
      if (searchTime > maxQueryTime) {
        console.warn(`Slow vector search: ${searchTime.toFixed(2)}ms (target: ${maxQueryTime}ms)`);
      }
  ...
  async rebuild(): Promise<void> {
    ...
    try {
      ...
      await this.index.initIndex(this.config.maxElements, this.config.M, this.config.efConstruction);
