...
describe('Node ID Generator', () => {
  ...
  describe('Statistics and Performance', () => {
    it('should track generation statistics', () => {
      const generator = new NodeIDGenerator({ enablePerformanceTracking: true });
      const node: NodeIdentityData = {
        filePath: '/file.ts',
        type: NodeType.FUNCTION,
        name: 'func1',
  ...
  describe('Configuration Options', () => {
    it('should respect enableCollisionDetection option', () => {
      const generatorWithCollisions = new NodeIDGenerator({ enableCollisionDetection: true });
      const generatorWithoutCollisions = new NodeIDGenerator({ enableCollisionDetection: false });
