...
import { createVectorDBConfig, VectorMetadata, VectorInsert } from '../vector/types.js';
...
describe('SQLiteVectorStorage', () => {
  ...
  describe('single vector operations', () => {
    const sampleVector = Array.from({ length: 768 }, (_, i) => Math.random() - 0.5);
    ...
    it('should insert a vector successfully', async () => {
      const label = await storage.insertVector('test-node-1', sampleVector, sampleMetadata);
    ...
    it('should reject vector with wrong dimensions', async () => {
      ...
      await expect(storage.insertVector('test-node-1', wrongSizeVector, sampleMetadata))
    ...
    it('should reject duplicate node IDs', async () => {
      ...
      await expect(storage.insertVector('test-node-1', sampleVector, sampleMetadata))
  ...
  describe('batch operations', () => {
    ...
    it('should handle partial batch failures', async () => {
      ...
      const existingVector = Array.from({ length: 768 }, () => Math.random() - 0.5);
  ...
  describe('multiple vector retrieval', () => {
    it('should retrieve multiple vectors efficiently', async () => {
      ...
      const vectors = nodeIds.map(() => Array.from({ length: 768 }, () => Math.random() - 0.5));
    ...
    it('should skip non-existent node IDs', async () => {
      await storage.insertVector('existing-node', Array.from({ length: 768 }, () => Math.random() - 0.5), {
      ...
      const results = await storage.getVectors(['existing-node', 'non-existent-node']);
  ...
  describe('search metadata retrieval', () => {
    it('should retrieve metadata for search results', async () => {
      ...
      for (let i = 0; i < nodeIds.length; i++) {
        await storage.insertVector(nodeIds[i], Array.from({ length: 768 }, () => Math.random() - 0.5), {
          signature: `function test${i}()`,
          summary: `Test function ${i}`,
          fileId: `file-${i}`,
          filePath: `/path/to/file${i}.ts`,
  ...
  describe('error handling', () => {
    it('should throw error when accessing uninitialized storage', async () => {
      const uninitializedStorage = new SQLiteVectorStorage(createVectorDBConfig({
        dimensions: 768,
        storageFile: join(tempDir, 'uninitialized.sqlite'),
        indexFile: join(tempDir, 'uninitialized.hnsw'),
      }));

      await expect(uninitializedStorage.insertVector('test', [1, 2, 3], {} as VectorMetadata))
        .rejects.toThrow('not initialized');
    });

    it('should handle update of non-existent vector', async () => {
      await expect(storage.updateVector('non-existent', Array.from({ length: 768 }, () => 0.5)))
