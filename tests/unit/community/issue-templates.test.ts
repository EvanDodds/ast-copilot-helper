...
describe('GitHub Issue Templates', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.warn('Issue template directory not found:', (error as Error).message);
    }
  });
  ...
  it('should have valid YAML front matter in each template', async () => {
    for (const template of templates) {
      ...
      expect(yamlMatch, `Template ${template} should have YAML front matter`).toBeTruthy();
      
      if (yamlMatch) {
        const yamlContent = yamlMatch[1];
        expect(() => yaml.load(yamlContent), `Template ${template} should have valid YAML`).not.toThrow();
        
        const parsed = yaml.load(yamlContent) as any;
        expect(parsed.name, `Template ${template} should have a name`).toBeTruthy();
        expect(parsed.about, `Template ${template} should have an about field`).toBeTruthy();
        expect(parsed.title, `Template ${template} should have a title`).toBeTruthy();
        expect(parsed.labels, `Template ${template} should have labels`).toBeTruthy();
      }
    }
  });
  ...
  it('should have structured content sections in each template', async () => {
    const requiredSections = {
      'bug_report.md': ['Bug Description', 'To Reproduce', 'Expected Behavior', 'Environment'],
      'feature_request.md': ['Describe the solution', 'Use Case', 'Priority'],
      'performance_issue.md': ['Performance Problem Description', 'Current Behavior', 'System Information'],
      'documentation.md': ['Documentation Issue Type', 'Location', 'Suggested Improvement'],
    ...
    for (const [template, sections] of Object.entries(requiredSections)) {
      if (templates.includes(template)) {
        ...
        for (const section of sections) {
          expect(content.includes(section), `Template ${template} should contain section: ${section}`).toBeTruthy();
        }
      }
    }
  });
