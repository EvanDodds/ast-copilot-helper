...
describe('NPMPublisher', () => {
  ...
  describe('initialization', () => {
    ...
    it('should use default registry if none provided', async () => {
      ...
      expect((publisher as any).npmRegistry.url).toBe('https://registry.npmjs.org');
    });
  });
  ...
  describe('validation', () => {
    ...
    it('should report errors for missing npm', async () => {
      ...
      expect(result.errors.some((e: any) => e.code === 'NPM_NOT_AVAILABLE')).toBe(true);
    });
    ...
    it('should report warnings for missing recommended fields', async () => {
      ...
      expect(result.warnings.some((w: any) => w.code === 'MISSING_RECOMMENDED_FIELD')).toBe(true);
    });
    ...
    it('should report errors for invalid package names', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.code === 'INVALID_PACKAGE_NAME')).toBe(true);
    });
    ...
    it('should report errors for missing main file', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.code === 'MAIN_FILE_MISSING')).toBe(true);
    });
  });
  ...
  describe('publishing', () => {
    ...
    it('should publish successfully with valid packages', async () => {
      ...
      mockExecSync
        ...
        .mockReturnValueOnce('{"name":"@test/npm-package","versions":["2.0.0"],"dist":{"tarball":"https://registry.npmjs.org/@test/npm-package/-/npm-package-2.0.0.tgz"}}'); // npm view (verification)
    ...
    it('should handle publish failures gracefully', async () => {
      ...
      mockExecSync
        ...
        .mockReturnValueOnce('build completed') // npm run build
        .mockReturnValueOnce('tests passed') // npm test
        .mockImplementationOnce(() => { // npm publish
  ...
  describe('verification', () => {
    ...
    it('should verify successful publications', async () => {
      ...
      mockExecSync.mockReturnValue(JSON.stringify({
        name: '@test/npm-package',
        versions: ['2.0.0'],
        dist: { tarball: 'https://registry.npmjs.org/@test/npm-package/-/npm-package-2.0.0.tgz' },
      }));
