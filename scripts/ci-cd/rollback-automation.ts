...
class RollbackAutomation {
  ...
  constructor(
    ...
    this.rollbackLogPath = path.join(
      process.cwd(),
      `${environment}-rollback.log`,
    );
  }
  ...
  private async executeCommand(
    ...
    try {
      ...
      throw new Error(
        `Rollback step failed: ${description} - ${error.message}`,
      );
    }
  }
  ...
  private getDeploymentHistory(): DeploymentState[] {
    const statePath = path.join(
      process.cwd(),
      `${this.config.environment}-deployment-state.json`,
    );
  ...
  private findRollbackTarget(): string | null {
    ...
    const mostRecentDeployment =
      deploymentHistory[deploymentHistory.length - 1];
    const mostRecentWasSuccessful =
      successfulDeployments.includes(mostRecentDeployment);
  ...
  private async validateRollback(): Promise<boolean> {
    ...
    try {
      ...
      await this.executeCommand(
        "yarn run build",
        "Validating build after rollback",
      );

      // Step 2: Essential tests
      await this.executeCommand(
        "yarn run test:unit",
        "Running unit tests after rollback",
      );
  ...
  private saveRollbackResult(result: RollbackResult): void {
    const resultPath = path.join(
      process.cwd(),
      `${this.config.environment}-rollback-history.json`,
    );
...
async function main(): Promise<void> {
  ...
  if (args.length < 2) {
    console.error(
      "Usage: tsx rollback-automation.ts <environment> <reason> [target-version]",
    );
    console.error("Environment: staging | production");
    console.error(
      'Example: tsx rollback-automation.ts production "Health check failure" v1.2.3',
    );
    process.exit(1);
  }
