...
import { ConfigurationManager, ASTConfiguration } from '../managers/ConfigurationManager';
...
describe('ConfigurationManager', () => {
  ...
  describe('Configuration Validation', () => {
    ...
    it('should detect invalid server path', async () => {
      ...
      expect(validation.errors).toContain('Server path is not set or does not exist');
    });
    ...
    it('should detect invalid timeout values', () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors).toContain('Connection timeout should be at least 5000ms');
      expect(validation.errors).toContain('Request timeout should be at least 1000ms');
    });
    ...
    it('should detect invalid log level', () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors).toContain('Log level must be one of: error, warn, info, debug');
    });
  });
  ...
  describe('Configuration Schema', () => {
    it('should provide configuration schema', () => {
      ...
      expect(schema['astCopilotHelper.server.autoRestart'].type).toBe('boolean');
    });
    ...
    it('should have proper schema structure', () => {
      ...
      expect(schema['astCopilotHelper.client.connectionTimeout'].minimum).toBe(5000);
      
      // Check UI settings
      expect(schema['astCopilotHelper.ui.logLevel'].enum).toEqual(['error', 'warn', 'info', 'debug']);
  ...
  describe('Change Handlers', () => {
    ...
    it('should call change handlers', () => {
      ...
      const onConfigChange = (vscode.workspace.onDidChangeConfiguration as any).mock.calls[0][0];
