...
export class ModelVerifyCommandHandler {
   ...
   */
  private async verifySingleModel(modelName: string, cache: ModelCache): Promise<void> {
  ...
  private async verifyAllModels(cache: ModelCache): Promise<void> {
    ...
    for (const modelName of ModelRegistry.listModels()) {
      ...
      try {
        ...
        this.logger.error(`❌ ${modelName} - Error: ${(error as Error).message}`);
        failed++;
      }
    }

    this.logger.info(`📊 Verification complete: ${verified} passed, ${failed} failed`);
  }
}