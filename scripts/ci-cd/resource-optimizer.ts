...
class ResourceOptimizer {
  ...
  private calculateEfficiencyMetrics(): void {
    ...
    this.resourceUsage.forEach((usage) => {
      console.log(`\n📋 ${usage.jobName}:`);
      console.log(
        `  CPU: ${usage.cpu.used}/${usage.cpu.allocated} cores (${usage.cpu.efficiency}% efficient)`,
      );
      console.log(
        `  Memory: ${this.formatMemory(usage.memory.used)}/${this.formatMemory(usage.memory.allocated)} (${usage.memory.efficiency}% efficient)`,
      );
      console.log(
        `  Storage: ${this.formatMemory(usage.storage.used)}/${this.formatMemory(usage.storage.allocated)} (${usage.storage.efficiency}% efficient)`,
      );
      console.log(
  ...
  private identifyOptimizationOpportunities(): void {
    ...
    const lowEfficiencyJobs = this.resourceUsage.filter(
      (usage) => usage.cpu.efficiency < 50 || usage.memory.efficiency < 50,
    );

    const overProvisionedJobs = this.resourceUsage.filter(
      (usage) => usage.cpu.efficiency < 30 && usage.memory.efficiency < 40,
    );

    const underProvisionedJobs = this.resourceUsage.filter(
      (usage) => usage.cpu.efficiency > 90 && usage.memory.efficiency > 85,
    );
  ...
  private generateRunnerRecommendations(): void {
    ...
    this.resourceUsage.forEach((usage) => {
      ...
      if (
        currentRunner &&
        recommendedRunner &&
        currentRunner.name !== recommendedRunner.name
      ) {
    ...
    console.log(
      `✅ Generated ${this.optimizationRecommendations.length} optimization recommendations`,
    );
  }
  ...
  private findOptimalRunner(usage: ResourceUsage): RunnerConfiguration | null {
    ...
    const suitableRunners = this.runnerConfigurations.filter(
      (runner) => runner.cpu >= requiredCpu && runner.memory >= requiredMemory,
    );

    // Sort by cost efficiency (performance per dollar)
    return (
      suitableRunners.sort((a, b) => {
  ...
  private calculateSavings(
    ...
    const recommendedCost =
      (usage.duration * recommendedRunner.costPerMinute) / 60;

    // Estimate time savings based on resource availability
    const performanceRatio =
      (recommendedRunner.cpu + recommendedRunner.memory / 1024) /
  ...
  private generateRecommendationReason(
    ...
    if (
      recommendedRunner.cpu > currentRunner.cpu ||
      recommendedRunner.memory > currentRunner.memory
    ) {
  ...
  private async generateOptimizedWorkflowConfig(): Promise<void> {
    const optimizedJobs = this.optimizationRecommendations.reduce(
      (acc, rec) => {
        acc[rec.jobName] = {
    ...
    const config = {
      ...
      optimizations: {
        "resource-allocation":
          "Optimized runner selection based on actual usage patterns",
    ...
    const configFile = path.join(
      this.outputDir,
      "optimized-workflow-config.json",
    );
  ...
  private calculateOptimizedCost(): number {
    ...
    this.resourceUsage.forEach((usage) => {
      const recommendation = this.optimizationRecommendations.find(
        (r) => r.jobName === usage.jobName,
      );
      if (recommendation) {
        const recommendedRunner = this.runnerConfigurations.find(
          (r) => r.name === recommendation.recommendedRunner,
        );
  ...
  private calculateTotalCostSavings(): number {
    return (
      this.optimizationRecommendations.reduce(
        (sum, rec) => sum + rec.expectedSavings.cost,
        0,
  ...
  private calculateTotalTimeSavings(): number {
    return (
      this.optimizationRecommendations.reduce(
        (sum, rec) => sum + rec.expectedSavings.time,
        0,
      ) / this.optimizationRecommendations.length
    );
  }
  ...
  private calculateTotalEfficiencyGain(): number {
    const currentAvgEfficiency =
      this.resourceUsage.reduce(
        (sum, usage) =>
          sum + (usage.cpu.efficiency + usage.memory.efficiency) / 2,
        0,
      ) / this.resourceUsage.length;

    const optimizedAvgEfficiency =
      this.optimizationRecommendations.reduce(
        (sum, rec) => sum + rec.expectedSavings.efficiency,
        0,
      ) / this.optimizationRecommendations.length;

    return (
      ((optimizedAvgEfficiency - currentAvgEfficiency) / currentAvgEfficiency) *
      100
    );
  }
...
<body>
    ...
    const htmlFile = path.join(
      this.outputDir,
      "resource-optimization-report.html",
    );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      "📄 Check ci-artifacts/resource-optimization/ for detailed reports",
    );
    console.log("");
  } catch (error) {
