...
describe("Production Readiness Configuration", () => {
  describe("DEFAULT_PRODUCTION_READINESS_CONFIG", () => {
    ...
    it("should have required test suites", () => {
      expect(
        DEFAULT_PRODUCTION_READINESS_CONFIG.testing.testSuites,
      ).toHaveLength(5);

      const suiteNames =
        DEFAULT_PRODUCTION_READINESS_CONFIG.testing.testSuites.map(
          (s) => s.name,
        );
  ...
  describe("validateConfig", () => {
    ...
    it("should reject configuration with no test suites", () => {
      ...
      expect(result.errors).toContain(
        "Testing configuration must include at least one test suite",
      );
    });
    ...
    it("should reject configuration with empty platforms", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        "Deployment configuration must include platforms",
      );
    });
    ...
    it("should reject configuration with no certification levels", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        "Certification configuration must include levels",
      );
    });
    ...
    it("should validate vulnerability scanning configuration", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        "Vulnerability scanning requires severity levels",
      );
    });
  });
  ...
  describe("mergeConfig", () => {
    ...
    it("should merge nested configuration objects", () => {
      ...
      expect(merged.security.inputValidation.testCases).toEqual([
        "custom-test",
      ]);
