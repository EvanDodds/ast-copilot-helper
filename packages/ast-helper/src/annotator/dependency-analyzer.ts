...
export class DependencyAnalyzer {
  ...
  private async extractImports(
    ...
    await this.traverseNode(node, async (currentNode) => {
      ...
      if (type === "importdeclaration") {
        const importInfo = await this.parseImportDeclaration(
          currentNode,
          filePath,
        );
        if (importInfo) {
      ...
      // Dynamic imports
      if (
        this.config.includeDynamicImports &&
        type === "import" &&
        nodeAny.arguments
      ) {
        const dynamicImport = await this.parseDynamicImport(
          currentNode,
          filePath,
        );
        if (dynamicImport) {
      ...
      if (this.config.includeTypeImports && type === "importtype") {
        const typeImport = await this.parseBasicImport(
          currentNode,
          filePath,
          true,
        );
  ...
  private async extractExports(
    ...
    await this.traverseNode(node, async (currentNode) => {
      ...
      if (
        ...
        const exportInfo = await this.parseExportDeclaration(
          currentNode,
          filePath,
        );
        if (exportInfo) {
   ...
   */
  private async parseImportDeclaration(
    node: ASTNode,
    filePath?: string,
   ...
   */
  private async parseDynamicImport(
    node: ASTNode,
    filePath?: string,
  ): Promise<DependencyInfo | null> {
    try {
   ...
   */
  private async parseBasicImport(
    node: ASTNode,
    filePath?: string,
    isTypeOnly = false,
  ): Promise<DependencyInfo | null> {
   ...
   */
  private async parseExportDeclaration(
    node: ASTNode,
    filePath?: string,
  ): Promise<DependencyInfo | null> {
    try {
      ...
      const source = nodeAny.source
        ? this.extractStringLiteral(nodeAny.source)
        : null;
      ...
      return {
        ...
        resolvedPath: source
          ? await this.resolveModulePath(source, filePath)
          : filePath || null,
      };
    } catch (error) {
   ...
   */
  private async parseCallExpression(
    node: ASTNode,
  ): Promise<DependencyInfo | null> {
    try {
      const nodeAny = node as any;
   ...
   */
  private async parseMemberExpression(
    node: ASTNode,
  ): Promise<DependencyInfo | null> {
    try {
      const nodeAny = node as any;
   ...
   */
  private async parseRequireCall(
    node: ASTNode,
    filePath?: string,
  ): Promise<DependencyInfo | null> {
    try {
  ...
  private async detectCycles(
    ...
    for (const dep of dependencies) {
      if (dep.resolvedPath && !visited.has(dep.resolvedPath)) {
        const cycle = await this.dfsDetectCycle(
          dep.resolvedPath,
          visited,
          recursionStack,
          filePath,
  ...
  private async traverseNode(
    node: ASTNode,
    callback: (node: ASTNode) => Promise<void>,
  ): Promise<void> {
    await callback(node);
