...
describe("Privacy and Consent Management", () => {
  ...
  describe("PrivacyRespectingDataAnonymizer", () => {
    ...
    it("should prevent double initialization", async () => {
      await expect(anonymizer.initialize()).rejects.toThrow(
        "already initialized",
      );
    });
    ...
    it("should apply different anonymization strategies", async () => {
      ...
      const testData = {
        sensitiveField:
          "this is sensitive information that should be handled differently",
      };
      ...
      const permissiveResult =
        await permissiveAnonymizer.anonymizeData(testData);
    ...
    it("should throw error when not initialized", async () => {
      ...
      await expect(uninitializedAnonymizer.anonymizeData({})).rejects.toThrow(
        "not initialized",
      );

      await expect(uninitializedAnonymizer.generateMachineId()).rejects.toThrow(
        "not initialized",
      );
    });
  });
