...
describe("MetadataExtractor", () => {
  ...
  const createMockNode = (
    ...
    if (!base.children?.length && !base.namedChildren?.length) {
      ...
      if (nameTypes.some((t) => type.includes(t))) {
        const identifierName = text.includes(" ")
          ? text.split(" ").pop() || text
          : text;
  ...
  describe("Basic Metadata Extraction", () => {
    ...
    it("should handle different languages", () => {
      ...
      const tsMetadata = extractor.extractMetadata(
        node,
        createContext("typescript"),
      );
      const pyMetadata = extractor.extractMetadata(
        node,
        createContext("python"),
      );
      const javaMetadata = extractor.extractMetadata(
        node,
        createContext("java"),
      );
