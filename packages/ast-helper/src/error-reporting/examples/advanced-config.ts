...
async function advancedConfigurationExample() {
  ...
  try {
    ...
    const customError = new Error(
      "Database connection timeout after retry attempts",
    );
    ...
    const performanceError = new Error(
      "API endpoint response time exceeded SLA threshold",
    );
    performanceError.name = "PerformanceViolation";

    const perfReport = await errorManager.generateErrorReport(
      performanceError,
      {
    ...
    const systemError = new Error(
      "Distributed transaction failed across microservices",
    );
    ...
    console.log(
      `🔄 Distributed system error reported: ${systemResult.errorId}\n`,
    );
    ...
    console.log(
      `   - Node.js Version: ${diagnosticsData.environment?.nodeVersion}`,
    );
    console.log(
      `   - Memory Usage: ${Math.round((diagnosticsData.environment?.memoryUsage?.heapUsed || 0) / 1024 / 1024)}MB`,
    );
    console.log(
      `   - Uptime: ${Math.round((diagnosticsData.environment?.uptime || 0) / 3600)}h`,
    );

    if (diagnosticsData.diagnostics) {
      console.log("🔍 Diagnostic Information:");
      console.log(
        `   - System Info Available: ${!!diagnosticsData.diagnostics.system}`,
      );
      console.log(
        `   - Runtime Info Available: ${!!diagnosticsData.diagnostics.runtime}`,
      );
      console.log(
        `   - Performance Info Available: ${!!diagnosticsData.diagnostics.performance}`,
      );
    }
    console.log();
    ...
    if (errorHistory.length > 0) {
      ...
      if (highSeverityErrors.length > 0) {
        ...
        highSeverityErrors.forEach((entry, index) => {
          console.log(
            `   ${index + 1}. ${entry.error.message} (${entry.error.severity})`,
          );
...
async function customErrorCategoriesExample() {
  ...
  for (const { name, error, context } of customErrorTypes) {
    ...
    console.log(
      `   💡 Suggestions: ${result.suggestions?.length || 0} generated`,
    );
    console.log();
  }
