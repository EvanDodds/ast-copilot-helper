...
export class AdvancedAttributionGenerator {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`Initialized attribution generator output directory: ${this.config.outputDirectory}`);
    } catch (error) {
   ...
   */
  async generateAllAttributions(dependencies: DependencyLicense[]): Promise<AttributionDocument[]> {
    console.log(`Generating advanced attributions for ${dependencies.length} dependencies...`);
    
    const filteredDependencies = this.filterDependencies(dependencies);
    const documents: AttributionDocument[] = [];

    console.log(`Processing ${filteredDependencies.length} dependencies after filtering...`);
    ...
    for (const format of formats) {
      try {
        const formatDocs = await this.generateForFormat(format, filteredDependencies);
        documents.push(...formatDocs);
      } catch (error) {
        console.error(`Failed to generate attribution in format ${format}:`, error);
      }
    }
    ...
    if (this.config.templates) {
      for (const template of this.config.templates) {
        try {
          const templateDoc = await this.generateFromTemplate(template, filteredDependencies);
          if (templateDoc) {
            documents.push(templateDoc);
          }
        } catch (error) {
          console.error(`Failed to generate from template ${template.name}:`, error);
        }
      }
    }
   ...
   */
  async generateEnhancedNoticeFile(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
    ...
    for (const [groupKey, groupDeps] of groupedDeps) {
      ...
      if (this.config.grouping?.subGrouping) {
        const subGrouped = this.groupDependencies(groupDeps, this.config.grouping.subGrouping);
   ...
   */
  async generateHtmlReport(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
...
`;
    ...
    const uniqueLicenses = [...new Set(filteredDeps.map(d => d.license.spdxId || d.license.name))];
...
`;
    ...
    for (const [groupKey, groupDeps] of groupedDeps) {
      ...
      for (const dep of groupDeps) {
        ...
        if (dep.license.text) {
          const truncatedText = dep.license.text.length > 500 
   ...
   */
  async generateMarkdownReport(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
   ...
   */
  async generateSpdxDocument(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
    ...
    const spdxDocument = {
      ...
      packages: filteredDeps.map((dep, index) => ({
        ...
        copyrightText: dep.copyrightHolders.length > 0 
   ...
   */
  async saveAttributionDocuments(documents: AttributionDocument[]): Promise<void> {
  ...
  private filterDependencies(dependencies: DependencyLicense[]): DependencyLicense[] {
    ...
    return dependencies.filter(dep => {
      ...
      if (filter.licenseTypes && filter.licenseTypes.length > 0) {
        if (!filter.licenseTypes.includes(dep.license.spdxId) && 
      ...
      if (filter.excludeLicenseTypes && filter.excludeLicenseTypes.length > 0) {
        if (filter.excludeLicenseTypes.includes(dep.license.spdxId) || 
  ...
  private groupDependencies(
    ...
    const actualGrouping = grouping || this.config.grouping || { type: 'license', direction: 'asc' };
    ...
    for (const dep of dependencies) {
      ...
      switch (actualGrouping.type) {
        ...
        case 'category':
          ...
          break;
        case 'author':
          groupKey = dep.copyrightHolders.length > 0 ? (dep.copyrightHolders[0] || 'Unknown') : 'Unknown';
          break;
        case 'domain':
    ...
    const sortedEntries = Array.from(groups.entries()).sort(([a], [b]) => {
      return actualGrouping.direction === 'asc' ? a.localeCompare(b) : b.localeCompare(a);
    });
  ...
  private buildTemplateVariables(dependencies: DependencyLicense[]): Record<string, string> {
    ...
    const variables: Record<string, string> = {
      ...
      attributionCount: dependencies.filter(d => d.attributionRequired).length.toString(),
  ...
  private async generateStatisticsSection(dependencies: DependencyLicense[]): Promise<string> {
  ...
  private async generateDependencySection(dependencies: DependencyLicense[]): Promise<string> {
    let content = '';
  ...
  private async generateLicenseTextSection(dependencies: DependencyLicense[]): Promise<string> {
  ...
  private async applyHeaderFooter(content: string, isFooter = false): Promise<string> {
  ...
  private async generateHtmlStatistics(dependencies: DependencyLicense[]): Promise<string> {
    const stats = this.calculateStatistics(dependencies);
  ...
  private async generateMarkdownStatistics(dependencies: DependencyLicense[]): Promise<string> {
    const stats = this.calculateStatistics(dependencies);
...
`;
  ...
  private async generateJsonMetadata(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
    const stats = this.calculateStatistics(dependencies);
  ...
  private async generateXmlReport(dependencies: DependencyLicense[]): Promise<AttributionDocument> {
    const stats = this.calculateStatistics(dependencies);
