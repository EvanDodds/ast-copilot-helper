...
export function validateModelConfig(config: ModelConfig): ValidationResult {
  ...
  if (!config.version || typeof config.version !== "string") {
    ...
    warnings.push(
      "Model version should follow semantic versioning (e.g., 1.0.0)",
    );
  }
  ...
  if (!config.checksum || typeof config.checksum !== "string") {
    ...
    errors.push(
      "Model checksum must be a valid SHA256 hash (64 hex characters)",
    );
  }
  ...
  if (config.tokenizer) {
    ...
    if (!tokenizerValidation.valid) {
      ...
      warnings.push(
        ...tokenizerValidation.warnings.map((w) => `Tokenizer: ${w}`),
      );
    }
  }
  ...
  // Additional warnings
  if (config.size > 1000000000) {
    // > 1GB
    warnings.push(
      "Large model size (>1GB) may cause memory issues on some systems",
    );
  }
...
export function validateRequirements(
  ...
  if (requirements.nodeVersion) {
    ...
    if (!isVersionCompatible(currentVersion, requirements.nodeVersion)) {
      warnings.push(
        `Node.js version ${requirements.nodeVersion} required, current: ${currentVersion}`,
      );
    }
  }
  ...
  if (requirements.memoryMB) {
    ...
    if (availableMemory < requirements.memoryMB) {
      warnings.push(
        `Model requires ${requirements.memoryMB}MB memory, available: ${availableMemory}MB`,
      );
    }
  }
  ...
  if (requirements.architecture && requirements.architecture.length > 0) {
    ...
    if (!requirements.architecture.includes(currentArch)) {
      errors.push(
        `Model requires architecture ${requirements.architecture.join("|")}, current: ${currentArch}`,
      );
    }
  }
  ...
  if (requirements.platforms && requirements.platforms.length > 0) {
    ...
    if (!requirements.platforms.includes(currentPlatform)) {
      errors.push(
        `Model requires platform ${requirements.platforms.join("|")}, current: ${currentPlatform}`,
      );
    }
  }
