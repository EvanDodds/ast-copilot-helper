  ...
  async releaseLock(lock: Lock): Promise<void> {
    try {
      ...
      if (lock.pid !== process.pid) {
        throw new LockError(
          `Cannot release lock owned by different process (PID: ${lock.pid})`,
          "INVALID_OWNER",
        );
      }
      ...
      if (!this.activeLocks.has(lock.id)) {
        throw new LockError(
          `Lock ${lock.id} is not active in this manager`,
          "LOCK_NOT_ACTIVE",
        );
      }
  ...
  async cleanup(): Promise<void> {
    ...
    for (const lock of locks) {
      try {
        ...
        console.error(
          `Failed to release lock ${lock.id}: ${(error as Error).message}`,
        );
      }
    }
  }
