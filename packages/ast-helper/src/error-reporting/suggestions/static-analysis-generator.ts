...
export class StaticAnalysisGenerator implements SuggestionGenerator {
  ...
  async generateSuggestions(
    ...
    try {
      ...
      for (const result of analysisResults) {
        const suggestion = await this.createSuggestionFromAnalysis(
          result,
          context,
        );
        if (suggestion) {
   ...
   */
  private async analyzeImportErrors(
    context: SuggestionContext,
  ): Promise<AnalysisResult[]> {
    const results: AnalysisResult[] = [];
  ...
  private async analyzeTypeErrors(
    ...
    const assignMatch = message.match(
    ...
    if (message.includes("Cannot use import statement outside a module")) {
      results.push({
        ...
        severity: "error",
        suggestion:
          "Configure TypeScript/Node.js for ES modules in package.json or tsconfig.json",
        fixable: true,
        confidence: 0.8,
      });
    }
   ...
   */
  private async analyzeSyntaxErrors(
    context: SuggestionContext,
  ): Promise<AnalysisResult[]> {
    const results: AnalysisResult[] = [];
    const message = context.error.message || "";
   ...
   */
  private async analyzeVariableErrors(
    context: SuggestionContext,
  ): Promise<AnalysisResult[]> {
    const results: AnalysisResult[] = [];
    const message = context.error.message || "";
   ...
   */
  private async analyzeFunctionErrors(
    context: SuggestionContext,
  ): Promise<AnalysisResult[]> {
    const results: AnalysisResult[] = [];
    const message = context.error.message || "";
   ...
   */
  private getConfidenceForResult(
    result: AnalysisResult,
  ...
  private extractContextClues(
    ...
    if (context.error.stack) {
      clues.push(
        `Error stack available: ${context.error.stack.split("\n")[0]}`,
      );
    }
  ...
  private isPossibleTypo(variable: string): boolean {
    ...
    for (const common of commonVariables) {
      if (
        this.calculateSimilarity(variable.toLowerCase(), common.toLowerCase()) >
        0.7
      ) {
        return true;
      }
