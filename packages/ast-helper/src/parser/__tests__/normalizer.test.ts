...
describe("AST Normalization System", () => {
  ...
  describe("ASTNormalizer", () => {
    describe("node normalization", () => {
      ...
      it("should handle nested scopes correctly", async () => {
        ...
        expect(result.children[0].metadata.scopeId).not.toBe(
          result.metadata.scopeId,
        );
      });
    });
    ...
    describe("normalization options", () => {
      ...
      it("should respect maximum depth limit", async () => {
        ...
        await expect(
          normalizer.normalize(deepNode, "typescript", "/test/file.ts", {
            maxDepth: 3,
          }),
    ...
    describe("multiple node normalization", () => {
      it("should normalize an array of nodes", async () => {
        ...
        const results = await normalizer.normalizeNodes(
          nodes,
          "typescript",
          "/test/file.ts",
        );
    ...
    describe("ID generation", () => {
      it("should generate consistent IDs for identical nodes", () => {
        ...
        const id1 = normalizer.generateNodeId(
          mockNode,
          "typescript",
          "/test/file.ts",
          "scope1",
        );
        const id2 = normalizer.generateNodeId(
          mockNode,
          "typescript",
          "/test/file.ts",
          "scope1",
    ...
    describe("custom configurations", () => {
      it("should use custom normalization configurations", () => {
        ...
        expect(
          customNormalizer["configs"].typescript.nodeTypeMappings[
            "custom_node"
          ],
        ).toBe("special_node");
  ...
  describe("NORMALIZATION_CONFIGS", () => {
    ...
    it("should map common node types correctly", () => {
      ...
      expect(tsConfig.nodeTypeMappings["function_declaration"]).toBe(
        "function_definition",
      );
      expect(tsConfig.nodeTypeMappings["class_declaration"]).toBe(
        "class_definition",
      );
      expect(tsConfig.nodeTypeMappings["call_expression"]).toBe(
        "function_call",
      );
