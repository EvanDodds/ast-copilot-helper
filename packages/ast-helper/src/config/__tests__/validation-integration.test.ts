...
describe("Configuration Validation Integration", () => {
  describe("End-to-End Validation", () => {
    ...
    it("should handle edge cases and type coercion", () => {
      ...
      expect(cleaned.fileWatching?.includePatterns).toEqual([
        "*.ts",
        "valid.js",
      ]);
  ...
  describe("Validation Utils", () => {
    it("should provide reusable validation utilities", () => {
      ...
      expect(numericRule.validate("not-a-number")).toContain(
        "must be a valid number",
      );

      const stringArrayRule = ValidationUtils.stringArray("testArray", 1);
      expect(stringArrayRule.validate(["valid"])).toBe(true);
      expect(stringArrayRule.validate([])).toContain(
        "must contain at least 1 valid",
      );
      expect(stringArrayRule.validate("not-array")).toContain(
        "must be an array",
      );
  ...
  describe("Schema Extension", () => {
    it("should support custom validation schemas", () => {
      ...
      expect(
        invalidResult.errors.find((e) => e.path === "customField"),
      ).toBeDefined();
    });
  });
});
