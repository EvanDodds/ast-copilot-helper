...
import { ASTNode, NodeType, SignificanceLevel, Position, NodeMetadata } from '../ast-schema';
...
describe('NodeSerializer', () => {
  ...
  const sampleNode: ASTNode = {
    ...
    sourceText: 'async function testFunction(param1: string, param2: number): Promise<void> { ... }',
  ...
  describe('constructor', () => {
    ...
    it('should merge provided config with defaults', () => {
      ...
      expect(config.includeMetadata).toBe(DEFAULT_SERIALIZATION_CONFIG.includeMetadata);
    });
  });
  ...
  describe('serializeNode', () => {
    ...
    it('should exclude sourceText when includeSourceText is false', () => {
      const configuredSerializer = new NodeSerializer({ includeSourceText: false });
    ...
    it('should skip validation when validateOnSerialize is false', () => {
      const nonValidatingSerializer = new NodeSerializer({ validateOnSerialize: false });
      const invalidNode = { ...sampleNode, id: '' } as ASTNode;
  ...
  describe('serializeFile', () => {
    it('should serialize multiple nodes with file metadata', () => {
      ...
      const serialized = serializer.serializeFile(nodes, '/test/file.ts', 'typescript', 'hash123');
    ...
    it('should calculate correct statistics', () => {
      const nodes = [
        { ...sampleNode, significance: SignificanceLevel.HIGH, type: NodeType.FUNCTION },
        { ...sampleNode, id: 'node2', significance: SignificanceLevel.MEDIUM, type: NodeType.CLASS },
      ];
      
      const serialized = serializer.serializeFile(nodes, '/test/file.ts', 'typescript');
      
      expect(serialized.metadata.stats.totalNodes).toBe(2);
      expect(serialized.metadata.stats.significanceLevels[SignificanceLevel.HIGH.toString()]).toBe(1);
      expect(serialized.metadata.stats.significanceLevels[SignificanceLevel.MEDIUM.toString()]).toBe(1);
  ...
  describe('file operations', () => {
    it('should save and load serialized data to/from file', async () => {
      ...
      // Save to file
      await serializer.saveToFile(nodes, '/test/file.ts', outputPath, 'typescript', 'hash123');
    ...
    it('should create output directory if it does not exist', async () => {
      ...
      await serializer.saveToFile([sampleNode], '/test/file.ts', outputPath, 'typescript');
    ...
    it('should handle pretty print configuration', async () => {
      ...
      await prettySerializer.saveToFile([sampleNode], '/test/file.ts', outputPath, 'typescript');
  ...
  describe('round-trip validation', () => {
    it('should validate successful round-trip serialization', () => {
      ...
      const isValid = serializer.validateRoundTrip(sampleNode, serialized, deserialized);
    ...
    it('should validate positions correctly', () => {
      ...
      const isValid = serializer.validateRoundTrip(nodeWithDifferentEnd, serialized, deserialized);
      expect(isValid).toBe(false);
    });
  ...
  describe('configuration management', () => {
    ...
    it('should return immutable configuration copy', () => {
      ...
      expect(freshConfig.prettyPrint).toBe(DEFAULT_SERIALIZATION_CONFIG.prettyPrint);
    });
  });
  ...
  describe('statistics and analysis', () => {
    it('should calculate serialization statistics', () => {
      const nodes = [
        { ...sampleNode, type: NodeType.FUNCTION, significance: SignificanceLevel.HIGH },
        { ...sampleNode, id: 'node2', type: NodeType.CLASS, significance: SignificanceLevel.MEDIUM },
        { ...sampleNode, id: 'node3', type: NodeType.FUNCTION, significance: SignificanceLevel.LOW },
      ];
      ...
      expect(stats.significanceDistribution[SignificanceLevel.HIGH.toString()]).toBe(1);
      expect(stats.significanceDistribution[SignificanceLevel.MEDIUM.toString()]).toBe(1);
      expect(stats.significanceDistribution[SignificanceLevel.LOW.toString()]).toBe(1);
    });
...
describe('SerializationUtils', () => {
  describe('preset serializers', () => {
    ...
    it('should create validating serializer with correct configuration', () => {
      const validatingSerializer = SerializationUtils.createValidatingSerializer();
  ...
  describe('utility functions', () => {
    ...
    it('should calculate compression ratio', () => {
      ...
      const ratio = SerializationUtils.calculateCompressionRatio(original, compressed);
...
describe('defaultSerializer', () => {
  ...
  it('should be reusable across calls', () => {
    ...
    const serialized2 = defaultSerializer.serializeNode({ ...sampleNode, id: 'test2' });
