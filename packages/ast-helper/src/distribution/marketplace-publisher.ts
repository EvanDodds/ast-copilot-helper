...
export class MarketplacePublisher implements Publisher {
  ...
  private loadCredentials(): void {
    ...
    const vsceToken =
      process.env.VSCE_PAT || this.findMarketplaceToken("vscode-marketplace");
    ...
    const ovsxToken =
      process.env.OVSX_PAT || this.findMarketplaceToken("openvsx");
  ...
  private findMarketplaceToken(
    marketplace: "vscode-marketplace" | "openvsx",
  ): string | undefined {
    if (!this.config) {
      return undefined;
    }

    const marketplaceConfig = this.config.marketplaces.find(
      (m) => m.type === marketplace,
    );
  ...
  private findPublisher(): string | undefined {
    ...
    const vsCodeMarketplace = this.config.marketplaces.find(
      (m) => m.type === "vscode-marketplace",
    );
    if (vsCodeMarketplace?.publisherId) {
      return vsCodeMarketplace.publisherId;
    }

    const vsCodePackage = this.config.packages.find(
      (p) => p.type === "vscode-extension",
    );
  ...
  private async verifyTools(): Promise<void> {
    try {
      ...
      throw new Error(
        "vsce (Visual Studio Code Extension Manager) is not installed. Install with: npm install -g vsce",
      );
    }
    ...
    try {
      ...
      console.warn(
        "ovsx is not installed. Open VSX publishing will be skipped. Install with: npm install -g ovsx",
      );
    }
  }
  ...
  async validate(_warnings: string[] = []): Promise<ValidationResult> {
    ...
    if (!this.config) {
      addError(
        ...
        "Marketplace publisher not initialized with config",
      );
      return { success: false, errors, warnings: validationWarnings };
    }

    // Check for VS Code extension packages
    const vsCodePackages = this.config.packages.filter(
      (p) => p.type === "vscode-extension",
    );
    if (vsCodePackages.length === 0) {
      addError(
        "NO_VSCODE_PACKAGES",
        "No VS Code extension packages found in configuration",
      );
      return { success: false, errors, warnings: validationWarnings };
    }
    ...
    try {
      ...
      if (!manifest.displayName) {
        addError(
          ...
          "Extension displayName is required",
          packageJsonPath,
        );
      }
      ...
      if (!manifest.engines?.vscode) {
        addError(
          "MISSING_VSCODE_ENGINE",
          "VS Code engine version is required",
          packageJsonPath,
        );
      }
      ...
      if (!manifest.categories || manifest.categories.length === 0) {
        addWarning(
          "MISSING_CATEGORIES",
          "Extension categories are recommended for better discoverability",
          packageJsonPath,
        );
      }
      ...
      if (!manifest.keywords || manifest.keywords.length === 0) {
        addWarning(
          "MISSING_KEYWORDS",
          "Extension keywords are recommended for better searchability",
          packageJsonPath,
        );
      }
      ...
      if (!manifest.icon) {
        addWarning(
          "MISSING_ICON",
          "Extension icon is recommended for marketplace presentation",
          packageJsonPath,
        );
      } else {
        ...
        try {
          ...
          addError(
            "ICON_FILE_NOT_FOUND",
            `Icon file not found: ${manifest.icon}`,
            packageJsonPath,
          );
        }
      }
      ...
      if (!manifest.repository) {
        addWarning(
          "MISSING_REPOSITORY",
          "Repository information is recommended",
          packageJsonPath,
        );
      }
      ...
      if (manifest.main) {
        ...
        try {
          ...
          addError(
            "MAIN_FILE_NOT_FOUND",
            `Main entry file not found: ${manifest.main}`,
            packageJsonPath,
          );
        }
      }
      ...
      try {
        ...
        addWarning(
          "MISSING_README",
          "README.md file is recommended for extension documentation",
          pkg.path,
        );
      }
      ...
      try {
        ...
        addWarning(
          "MISSING_CHANGELOG",
          "CHANGELOG.md file is recommended for version history",
          pkg.path,
        );
      }
    } catch (error) {
      addError(
        ...
        `Could not load or parse package.json: ${error}`,
        packageJsonPath,
      );
    }
  }
  ...
  private validateCredentials(
    ...
    if (!this.credentials.vscode?.token) {
      addError(
        "MISSING_VSCODE_TOKEN",
        "VS Code Marketplace personal access token is required (VSCE_PAT environment variable)",
      );
    }

    if (!this.credentials.vscode?.publisher) {
      addError(
        "MISSING_VSCODE_PUBLISHER",
        "VS Code Marketplace publisher is required",
      );
    }

    if (!this.credentials.openvsx?.token) {
      addWarning(
        "MISSING_OPENVSX_TOKEN",
        "Open VSX personal access token not provided (OVSX_PAT). Open VSX publishing will be skipped.",
      );
    }
  }
  ...
  private validateTools(
    ...
    if (!this.vsceVersion) {
      addError(
        "VSCE_NOT_AVAILABLE",
        "vsce tool not available. Install with: npm install -g vsce",
      );
    }
    ...
    if (!this.ovsxVersion) {
      addWarning(
        "OVSX_NOT_AVAILABLE",
        "ovsx tool not available. Install with: npm install -g ovsx for Open VSX publishing",
      );
    }
  }
    ...
    try {
      for (const pkg of vsCodePackages) {
        ...
        if (this.credentials.vscode) {
          const vsCodeResult = await this.publishToVSCodeMarketplace(
            pkg,
        ...
        if (this.credentials.openvsx && this.ovsxVersion) {
          const ovsxResult = await this.publishToOpenVSX(
            pkg,
            packageResult.packagePath!,
          );
      ...
      if (failedPublications.length > 0) {
        console.error(
          `❌ Some publications failed: ${failedPublications.map((r) => `${r.marketplace}: ${r.error}`).join(", ")}`,
        );
      }

      if (successfulPublications.length > 0) {
        console.log(
          `✅ Successfully published to ${successfulPublications.length} marketplace(s) in ${duration}ms`,
        );
        console.log(`📧 Publishing notification:`);
        console.log(
          `  Marketplaces: ${successfulPublications.map((r) => r.marketplace).join(", ")}`,
        );
        console.log(`  Version: ${this.config.version}`);
        console.log(
          `  Extensions: ${successfulPublications.map((r) => r.extensionId).join(", ")}`,
        );
  ...
  private async packageExtension(pkg: PackageConfig): Promise<PackageResult> {
    try {
      ...
      const packagePath = path.join(
        outputDir,
        `${pkg.name}-${this.config!.version}.vsix`,
      );
      ...
      console.log(
        `✅ Successfully packaged ${pkg.name} (${Math.round(stats.size / 1024)}KB)`,
      );
  ...
  private async publishToVSCodeMarketplace(
    pkg: PackageConfig,
    packagePath: string,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const publishCommand = [
          ...
          `"${packagePath}"`,
          "--pat",
          this.credentials.vscode!.token,
          "--no-git-tag-version",
          "--no-update-package-json",
        ].join(" ");

        console.log(
          `Publishing to VS Code Marketplace (attempt ${attempt}/${maxRetries})...`,
        );
        ...
        console.log(
          `✅ Successfully published ${extensionId} to VS Code Marketplace (attempt ${attempt})`,
        );
        ...
        if (attempt === maxRetries || !isRetryable) {
          ...
          console.error(
            `❌ Failed to publish to VS Code Marketplace after ${attempt} attempts: ${error}`,
          );
        ...
        console.warn(
          `⚠️ VS Code Marketplace publish attempt ${attempt} failed: ${error}`,
        );
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const publishCommand = [
          ...
          `"${packagePath}"`,
          "--pat",
          this.credentials.openvsx!.token,
        ].join(" ");

        console.log(
          `Publishing to Open VSX Registry (attempt ${attempt}/${maxRetries})...`,
        );
        ...
        console.log(
          `✅ Successfully published ${extensionId} to Open VSX Registry (attempt ${attempt})`,
        );
        ...
        if (attempt === maxRetries || !isRetryable) {
          ...
          console.error(
            `❌ Failed to publish to Open VSX Registry after ${attempt} attempts: ${error}`,
          );
  ...
  async verify(result: any): Promise<VerificationResult> {
    ...
    for (const pkg of packages) {
      ...
      if (pkg.marketplace === "vscode" || pkg.registry === "vscode") {
        const check = await this.verifyVSCodeMarketplace(
          pkg.extensionId || "",
          pkg.version || "",
        );
        checks.push(check);
      } else if (pkg.marketplace === "openvsx" || pkg.registry === "openvsx") {
        const check = await this.verifyOpenVSX(
          pkg.extensionId || "",
          pkg.version || "",
        );
        checks.push(check);
  ...
  private async verifyVSCodeMarketplace(
    extensionId: string,
    version: string,
    ...
    try {
      ...
      console.error(
        `Could not verify ${extensionId} in VS Code Marketplace: ${error}`,
      );
      return {
        name: `VS Code Marketplace - ${extensionId}`,
        success: false,
  ...
  private async loadExtensionManifest(
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.config) {
      for (const pkg of this.config.packages.filter(
        (p) => p.type === "vscode-extension",
      )) {
