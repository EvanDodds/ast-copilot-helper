...
export class ASTMCPServer extends EventEmitter implements MCPServer {
    ...
    try {
      ...
      if (!MCPProtocolHandler.isValidMethod(request.method)) {
        return MCPProtocolHandler.createErrorResponse(
          request.id ?? null,
          MCPProtocolHandler.createInvalidRequest(`Invalid method name: ${request.method}`)
        );
      }
      ...
      switch (request.method) {
        ...
        case 'resources/read':
          result = await this.handleReadResource(request.params as ResourceReadParams);
          break;
      ...
      return MCPProtocolHandler.createSuccessResponse(request.id ?? null, result);
  ...
  private async handleInitialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    if (!MCPProtocolHandler.isCompatibleProtocolVersion(params.protocolVersion)) {
