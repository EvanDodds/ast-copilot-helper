...
async function basicUsageExample() {
  ...
  try {
    ...
    const parseErrorReport = await errorManager.generateErrorReport(parseError, {
    ...
    console.log(`💡 Generated ${parseErrorResult.suggestions?.length || 0} suggestions for parse error\n`);
    ...
    console.log(`⚠️ Memory warning reported with ID: ${memoryResult.errorId}\n`);
    ...
    console.log(`🔧 Configuration error reported with ID: ${configResult.errorId}\n`);
    ...
    console.log(`📊 Environment: ${diagnosticsData.environment?.platform} ${diagnosticsData.environment?.arch}`);
    console.log(`⚡ Node version: ${diagnosticsData.environment?.nodeVersion}\n`);
...
async function errorTypeExamples() {
  ...
  const errorExamples = [
    ...
    },
    {
      ...
      context: { endpoint: '/api/v1/data', timeout: 5000, operation: 'api-request' }
    },
    {
      name: 'Performance Warning',
      error: new Error('Function execution time exceeds recommended threshold'),
      context: { functionName: 'processLargeDataset', executionTime: 2500, operation: 'function-execution' }
    }
  ];
