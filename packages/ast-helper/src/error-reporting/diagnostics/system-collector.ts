...
export class SystemDiagnosticCollector implements DiagnosticCollector {
  ...
  async collect(): Promise<Partial<DiagnosticData>> {
    try {
      const [osInfo, cpuInfo, memoryInfo, diskInfo, networkInfo] =
        await Promise.all([
          this.collectOSInfo(),
          this.collectCPUInfo(),
      ...
      throw new Error(
        `System diagnostic collection failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async getCPUUsage(): Promise<number[]> {
    ...
    if (
      this._cpuUsageCache &&
      now - this._cpuUsageTimestamp < this._cpuUsageCacheDuration
    ) {
  ...
  private calculateCPUUsage(
    start: Array<{ idle: number; total: number }>,
    end: Array<{ idle: number; total: number }>,
  ): number[] {
  ...
  private async getSwapInfo(): Promise<
    { total: number; free: number; used: number } | undefined
  > {
    try {
