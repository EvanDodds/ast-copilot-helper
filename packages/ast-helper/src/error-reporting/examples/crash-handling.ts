...
async function customCrashHandlingExample() {
  ...
  try {
    ...
    const memoryError = new Error(
      "Memory usage exceeded critical threshold: 95%",
    );
    ...
    const unhandledException = new Error(
      "Unhandled promise rejection in critical component",
    );
    unhandledException.name = "UnhandledPromiseRejection";

    const exceptionReport = await errorManager.generateErrorReport(
      unhandledException,
      {
    ...
    console.log(
      "✅ Unhandled exception captured and recovery plan activated\n",
    );
    ...
    const resourceReport = await errorManager.generateErrorReport(
      resourceError,
      {
    ...
    const cascadeError = new Error(
      "Primary service failure triggering cascade protection",
    );
    ...
    console.log(
      "✅ Cascade failure prevented with circuit breaker activation\n",
    );
    ...
    const recoveryStrategies = [
      ...
      },
      {
        name: "Load Shedding",
        description:
          "Reject lower priority requests to preserve system stability",
      ...
      },
      {
        name: "Data Recovery",
        description:
          "Restore from backups or rebuild corrupted data structures",
        triggers: [
          "data-corruption",
          "index-corruption",
          "state-inconsistency",
    ...
    const crashLikeErrors = errorHistory.filter(
      (entry) =>
    ...
    if (crashLikeErrors.length > 0) {
      console.log(
        `🔍 Crash Pattern Analysis (${crashLikeErrors.length} critical incidents):`,
      );

      // Analyze crash patterns
      const crashPatterns = crashLikeErrors.reduce(
        (acc, entry) => {
      ...
      // Time-based analysis
      const recentCrashes = crashLikeErrors.filter(
        (entry) =>
          entry.error.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000),
      );

      console.log(
        `   Recent critical incidents (24h): ${recentCrashes.length}`,
      );
    ...
    console.log(
      "🛡️ Advanced crash detection and recovery systems demonstrated",
    );
  } catch (error) {
...
async function emergencyResponseExample() {
  ...
  for (const scenario of emergencyScenarios) {
    ...
    const emergencyReport = await errorManager.generateErrorReport(
      emergencyError,
      {
