...
describe("CompatibilityCheckerImpl", () => {
  ...
  describe("API compatibility checking", () => {
    test("should check API compatibility between versions", async () => {
      const result = await compatibilityChecker.checkApiCompatibility(
        "1.0.0",
        "1.1.0",
      );
    ...
    test("should handle API compatibility check errors", async () => {
      ...
      await expect(
        compatibilityChecker.checkApiCompatibility("1.0.0", "1.1.0"),
  ...
  describe("configuration compatibility checking", () => {
    test("should check config compatibility", async () => {
      const result = await compatibilityChecker.checkConfigCompatibility(
        "1.0.0",
        "1.1.0",
      );
  ...
  describe("CLI compatibility checking", () => {
    test("should check CLI compatibility", async () => {
      const result = await compatibilityChecker.checkCliCompatibility(
        "1.0.0",
        "1.1.0",
      );
  ...
  describe("data format compatibility checking", () => {
    test("should check data format compatibility", async () => {
      const result = await compatibilityChecker.checkDataFormatCompatibility(
        "1.0.0",
        "1.1.0",
      );
    ...
    test("should detect data format breaking changes", async () => {
      ...
      vi.spyOn(
        dataEnabledChecker as any,
        "analyzeDataFormatChanges",
      ).mockResolvedValue({
      ...
      const result = await dataEnabledChecker.checkDataFormatCompatibility(
        "1.0.0",
        "2.0.0",
      );
    ...
    test("should skip data format check when disabled", async () => {
      ...
      const result = await disabledChecker.checkDataFormatCompatibility(
        "1.0.0",
        "1.1.0",
      );
  ...
  describe("comprehensive compatibility checking", () => {
    test("should check all compatibility aspects", async () => {
      const apiCheck = await compatibilityChecker.checkApiCompatibility(
        "1.0.0",
        "2.0.0",
      );
      const configCheck = await compatibilityChecker.checkConfigCompatibility(
        "1.0.0",
        "2.0.0",
      );
      const cliCheck = await compatibilityChecker.checkCliCompatibility(
        "1.0.0",
        "2.0.0",
      );
      const dataCheck = await compatibilityChecker.checkDataFormatCompatibility(
        "1.0.0",
        "2.0.0",
      );
    ...
    test("should find breaking changes between versions", async () => {
      const breakingChanges = await compatibilityChecker.findBreakingChanges(
        "1.0.0",
        "2.0.0",
      );
    ...
    test("should generate migration guide", async () => {
      const migrationGuide = await compatibilityChecker.generateMigrationGuide(
        "1.0.0",
        "2.0.0",
      );
    ...
    test("should aggregate breaking changes from all checks", async () => {
      ...
      vi.spyOn(
        ...
        breakingChanges: [
          {
            description: "Config breaking change",
            migration: "",
            affectedApi: [],
            severity: "major",
            automatedMigration: false,
  ...
  describe("breaking change detection", () => {
    test("should identify major breaking changes", async () => {
      ...
      const result = await compatibilityChecker.findBreakingChanges(
        "1.0.0",
        "2.0.0",
      );
  ...
  describe("migration guide generation", () => {
    test("should generate migration guide for breaking changes", async () => {
      ...
      const migrationGuide = await (
        compatibilityChecker as any
      ).generateMigrationGuide("1.0.0", "2.0.0", breakingChanges);
  ...
  describe("configuration validation", () => {
    test("should validate threshold settings", async () => {
      ...
      await expect(
        compatibilityChecker.initialize(invalidConfig),
    ...
    test("should handle missing base versions", async () => {
      ...
      await expect(
        emptyChecker.initialize(emptyConfig),
  ...
  describe("error handling", () => {
    test("should handle version parsing errors", async () => {
      await expect(
        compatibilityChecker.checkApiCompatibility("invalid", "1.1.0"),
