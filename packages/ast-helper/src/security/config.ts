...
export class SecurityConfigValidator {
  ...
  static validate(config: Partial<SecurityConfig>): SecurityConfig {
    ...
    if (
      !["basic", "comprehensive", "enterprise"].includes(validated.auditLevel)
    ) {
    ...
    if (invalidFrameworks.length > 0) {
      throw new Error(
        `Invalid compliance frameworks: ${invalidFrameworks.join(", ")}`,
      );
    }
  ...
  private static validateRule(rule: SecurityRule): void {
    ...
    if (!["low", "medium", "high", "critical"].includes(rule.severity)) {
      throw new Error(
        `Security rule ${rule.id} must have a valid severity level`,
      );
    }
    ...
    if (!rule.pattern && !rule.validator) {
      throw new Error(
        `Security rule ${rule.id} must have either a pattern or validator`,
      );
    }
  }
