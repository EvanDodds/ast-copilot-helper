...
describe("AdvancedAttributionGenerator", () => {
  ...
  describe("initialization", () => {
    it("should initialize successfully", async () => {
      ...
      expect(fs.mkdir).toHaveBeenCalledWith("/tmp/test-attribution", {
        recursive: true,
      });
    });
  ...
  describe("dependency filtering", () => {
    it("should filter dependencies by license type", async () => {
      ...
      const documents =
        await filteredGenerator.generateAllAttributions(testDependencies);

      expect(documents.length).toBeGreaterThan(0);
      // Should contain documents with only MIT licensed dependencies
      const jsonDoc = documents.find(
        (doc) => doc.filename === "attribution-metadata.json",
      );
      expect(jsonDoc).toBeDefined();

      const metadata = JSON.parse(jsonDoc!.content);
      expect(
        metadata.dependencies.every((dep: any) => dep.license.spdxId === "MIT"),
      ).toBe(true);
    });
    ...
    it("should exclude dependencies by license type", async () => {
      ...
      const metadata = JSON.parse(jsonDoc!.content);
      expect(
        metadata.dependencies.every(
          (dep: any) => dep.license.spdxId !== "GPL-3.0",
        ),
      ).toBe(true);
    });
    ...
    it("should filter by copyright notice requirement", async () => {
      ...
      expect(
        metadata.dependencies.every(
          (dep: any) => dep.copyrightHolders.length > 0,
        ),
      ).toBe(true);
    });
    ...
    it("should apply custom filter function", async () => {
      const filterConfig: AdvancedAttributionConfig = {
        ...config,
        filter: {
          customFilter: (dep: DependencyLicense) =>
            dep.packageName.startsWith("@types/"),
        },
      };
      ...
      const metadata = JSON.parse(jsonDoc!.content);
      expect(
        metadata.dependencies.every((dep: any) =>
  ...
  describe("dependency grouping", () => {
    it("should group dependencies by license", async () => {
      ...
      const documents =
        await groupedGenerator.generateAllAttributions(testDependencies);
  ...
  describe("format generation", () => {
    it("should generate enhanced NOTICE file", async () => {
      const documents =
        await generator.generateAllAttributions(testDependencies);
      const noticeDoc = documents.find((doc) => doc.filename === "NOTICE");
    ...
    it("should generate interactive HTML report", async () => {
      const documents =
        await generator.generateAllAttributions(testDependencies);
      const htmlDoc = documents.find(
        (doc) => doc.filename === "attribution-report.html",
      );
    ...
    it("should generate comprehensive markdown report", async () => {
      const documents =
        await generator.generateAllAttributions(testDependencies);
      const markdownDoc = documents.find(
        (doc) => doc.filename === "ATTRIBUTION.md",
      );
    ...
    it("should generate JSON metadata", async () => {
      ...
      expect(metadata.generator).toBe(
        "ast-copilot-helper-advanced-attribution-generator",
      );
    ...
    it("should generate XML report", async () => {
      ...
      const documents =
        await xmlGenerator.generateAllAttributions(testDependencies);
      const xmlDoc = documents.find(
        (doc) => doc.filename === "attributions.xml",
      );

      expect(xmlDoc).toBeDefined();
      expect(xmlDoc!.type).toBe("metadata");
      expect(xmlDoc!.content).toContain(
    ...
    it("should generate SPDX-compliant document", async () => {
      ...
      const testPackage = spdxData.packages.find(
        (pkg: any) => pkg.name === "test-package-1",
      );
...
`,
      ...
      const templateGenerator = new AdvancedAttributionGenerator(
        templateConfig,
      );
      const documents =
        await templateGenerator.generateAllAttributions(testDependencies);

      const customDoc = documents.find(
        (doc) => doc.filename === "CUSTOM-NOTICE.txt",
      );
      ...
      expect(customDoc!.content).toContain(
        "test-package-1 v1.0.0 (MIT License)",
      );
    });
    ...
    it("should handle template generation errors gracefully", async () => {
      ...
      expect(documents.length).toBeGreaterThan(0);
      const badDoc = documents.find(
        (doc) => doc.filename === "BAD-TEMPLATE.txt",
      );
  ...
  describe("statistics and analysis", () => {
    ...
    it("should generate license analysis", async () => {
      const noticeDoc =
        await generator.generateEnhancedNoticeFile(testDependencies);
  ...
  describe("file operations", () => {
    ...
    it("should handle file save errors gracefully", async () => {
      ...
      await expect(
        generator.saveAttributionDocuments(documents),
  ...
  describe("error handling", () => {
    ...
    it("should handle format generation errors", async () => {
      ...
      const documents =
        await errorGenerator.generateAllAttributions(testDependencies);
