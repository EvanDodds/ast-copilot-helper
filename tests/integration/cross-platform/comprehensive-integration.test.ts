...
describe('Cross-Platform Integration Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    console.log(`🔧 Setting up cross-platform integration test environment at ${testOutputDir}`);
  });
  ...
  describe('Individual Component Integration', () => {
    test('should integrate FileSystemTester successfully', async () => {
      ...
      console.log(`   ✅ FileSystemTester: ${passedTests.length}/${result.testResults.length} tests passed`);
      
      // Should have reasonable pass rate (at least 80%)
      expect(passedTests.length / result.testResults.length).toBeGreaterThan(0.8);
    }, 30000);
    ...
    test('should integrate BinaryCompatibilityTester successfully', async () => {
      ...
      console.log(`   ✅ BinaryCompatibilityTester: ${passedTests.length}/${result.testResults.length} tests passed`);
      
      // Should have some passing tests (at least 50% since some binaries may not be available)
      expect(passedTests.length / result.testResults.length).toBeGreaterThan(0.5);
    }, 30000);
    ...
    test('should integrate NodeVersionCompatibilityTester successfully', async () => {
      ...
      console.log(`   ✅ NodeVersionCompatibilityTester: ${passedTests.length}/${result.testResults.length} tests passed`);
      
      // Should have high pass rate for current Node.js version
      expect(passedTests.length / result.testResults.length).toBeGreaterThan(0.9);
    }, 30000);
    ...
    test('should integrate PerformanceBenchmarker successfully', async () => {
      ...
      console.log(`   ✅ PerformanceBenchmarker: ${passedTests.length}/${result.testResults.length} tests passed (Grade: ${result.summary.performanceGrade})`);
      
      // Should have reasonable performance (at least 70% pass rate)
      expect(passedTests.length / result.testResults.length).toBeGreaterThan(0.7);
      expect(['A', 'B', 'C', 'D', 'F']).toContain(result.summary.performanceGrade);
    }, 60000);
  });

  describe('CrossPlatformTestRunner Integration', () => {
    test('should integrate all components successfully', async () => {
      console.log('🚀 Running full CrossPlatformTestRunner integration test...');
      ...
      if (platformResult && 'testResults' in platformResult) {
        ...
        const filesystemTests = platformResult.testResults.filter((test: TestResult) => test.category === 'filesystem');
        const binaryTests = platformResult.testResults.filter((test: TestResult) => test.category === 'binary');
        const nodejsTests = platformResult.testResults.filter((test: TestResult) => test.category === 'nodejs');
        const performanceTests = platformResult.testResults.filter((test: TestResult) => test.category === 'performance');
        ...
        console.log(`      FileSystem: ${filesystemTests.filter(t => t.passed).length}/${filesystemTests.length}`);
        console.log(`      Binary: ${binaryTests.filter(t => t.passed).length}/${binaryTests.length}`);
        console.log(`      Node.js: ${nodejsTests.filter(t => t.passed).length}/${nodejsTests.length}`);
        console.log(`      Performance: ${performanceTests.filter(t => t.passed).length}/${performanceTests.length}`);
        
        const totalPassed = platformResult.testResults.filter(test => test.passed).length;
        const totalTests = platformResult.testResults.length;
        const passRate = totalPassed / totalTests;
        
        console.log(`   ✅ Overall: ${totalPassed}/${totalTests} tests passed (${Math.round(passRate * 100)}%)`);
    ...
    test('should handle selective component testing', async () => {
      ...
      if (platformResult) {
        ...
        const binaryTests = platformResult.testResults.filter(test => test.category === 'binary');
        const nodejsTests = platformResult.testResults.filter(test => test.category === 'nodejs');
        ...
        console.log(`   ✅ Selective testing: FileSystem(${filesystemTests.length}) + Performance(${performanceTests.length})`);
      }
    }, 90000);
  });
  ...
  describe('Error Handling and Edge Cases', () => {
    test('should handle component failures gracefully', async () => {
      ...
      if (platformResult) {
        ...
        const failedTests = platformResult.testResults.filter(test => !test.passed);
        ...
        for (const failedTest of failedTests) {
          ...
          const isExpectedFailure = failedTest.details && 'expectedFailure' in failedTest.details;
          const isOptional = failedTest.details && 'optional' in failedTest.details;
          
          if (!hasErrorInfo && !isExpectedFailure && !isOptional) {
            console.warn(`Test ${failedTest.name} failed without error information`);
          }
        }
        
        console.log(`   ✅ Error handling: ${failedTests.length} failed tests handled gracefully`);
      }
    }, 90000);
    ...
    test('should provide consistent results across multiple runs', async () => {
      ...
      if (platformResult1 && platformResult2) {
        ...
        expect(Math.abs(platformResult1.testResults.length - platformResult2.testResults.length)).toBeLessThan(3);
        
        // Pass rates should be reasonably consistent (within 10%)
        const passRate1 = platformResult1.testResults.filter(t => t.passed).length / platformResult1.testResults.length;
        const passRate2 = platformResult2.testResults.filter(t => t.passed).length / platformResult2.testResults.length;
        
        expect(Math.abs(passRate1 - passRate2)).toBeLessThan(0.1);
        
        console.log(`   ✅ Consistency: Pass rates ${Math.round(passRate1 * 100)}% vs ${Math.round(passRate2 * 100)}%`);
      }
    }, 90000);
  });
  ...
  describe('Performance and Scalability', () => {
    test('should complete comprehensive testing within reasonable time', async () => {
      ...
      console.log(`   ⏱️ Integration completed in ${Math.round(duration / 1000)}s`);
      ...
      if (platformResult) {
        ...
        console.log(`   📊 Performance: ${totalTests} tests, avg ${Math.round(averageTestTime)}ms per test`);
      }
    }, 180000);
    ...
    test('should provide comprehensive test coverage', async () => {
      ...
      if (platformResult) {
        const testsByCategory = platformResult.testResults.reduce((acc, test) => {
        ...
        const totalTests = Object.values(testsByCategory).reduce((sum, count) => sum + count, 0);
