...
*Full production support with comprehensive feature set*
...
*Strong community support with comprehensive syntax coverage*
...
for (const [filePath, language] of Object.entries(languages)) {
  ...
  console.log(`Parsed ${filePath} (${language}):`, result.nodes.length, 'nodes');
}
```
...
```typescript
// OLD
import { TreeSitterParser, TreeSitterManager } from '@ast-copilot-helper/ast-helper';
...
for (const lang of testLanguages) {
  ...
  console.log(`${lang} parsing:`, result.errors.length === 0 ? 'SUCCESS' : 'ERRORS');
}
```
