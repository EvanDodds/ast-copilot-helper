...
describe('BinaryCompatibilityTester', () => {
  ...
  describe('binary compatibility tests', () => {
    ...
    it('should test platform detection accurately', async () => {
      const result = await tester.runTests();
      const platformTest = result.testResults.find(test => 
    ...
    it('should test Node.js addon loading', async () => {
      const result = await tester.runTests();
      const addonTest = result.testResults.find(test => 
        test.name === 'node_addon_loading'
      );
    ...
    it('should test WebAssembly support', async () => {
      const result = await tester.runTests();
      const wasmTest = result.testResults.find(test => 
        test.name === 'webassembly_support'
      );
    ...
    it('should test binary architecture validation', async () => {
      const result = await tester.runTests();
      const archTest = result.testResults.find(test => 
        test.name === 'binary_architecture'
      );
    ...
    it('should test dynamic library loading', async () => {
      const result = await tester.runTests();
      const libTest = result.testResults.find(test => 
        test.name === 'dynamic_library_loading'
      );
    ...
    it('should test memory usage patterns', async () => {
      const result = await tester.runTests();
      const memoryTest = result.testResults.find(test => 
        test.name === 'memory_usage'
      );
    ...
    it('should test performance benchmarks', async () => {
      ...
      const perfTest = result.testResults.find(test => 
        test.name === 'performance_benchmarks'
      );
  ...
  describe('platform-specific behavior', () => {
    ...
    it('should handle platform-specific modules appropriately', async () => {
      const result = await tester.runTests();
      const moduleTests = result.testResults.filter(test => 
        test.name.startsWith('native_module_') || test.name.startsWith('tree_sitter_')
      );
  ...
  describe('error handling and edge cases', () => {
    ...
    it('should cleanup test files properly', async () => {
      ...
      if (testDirExists) {
        ...
        await expect(fs.rm(testDir, { recursive: true, force: true })).resolves.not.toThrow();
      }
    });
  });
