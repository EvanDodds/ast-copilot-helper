...
export class PerformanceValidateCommandHandler
  ...
  async execute(
    options: PerformanceValidateOptions,
    _config: Config,
  ): Promise<void> {
    ...
    try {
      ...
      if (format === "json") {
        ...
        console.log(
          `Tests: ${validation.summary.passedTests}/${validation.summary.totalTests} passed (${validation.summary.passRate.toFixed(1)}%)`,
        );
        ...
        if (!validation.passed) {
          ...
          for (const test of failedTests) {
            console.log(
              `  ❌ ${test.criterion}: ${test.actual} (target: ${test.target})`,
            );
