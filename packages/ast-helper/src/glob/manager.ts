...
export class GlobManager implements GlobMatcher {
  ...
  async expandPatterns(
    ...
    try {
      const resolvedBasePath = resolve(
        basePath || this.options.baseDirectory || process.cwd(),
      );
      ...
      for (const pattern of patterns) {
        if (!this.isValidPattern(pattern)) {
          throw GlobErrors.invalidPattern(
            pattern,
      ...
      for (const filePath of allFiles) {
        const relativePath = this.normalizePath(
          relative(resolvedBasePath, filePath),
        );
        ...
        if (this.match(patterns, relativePath)) {
          ...
          for (const pattern of patterns) {
            if (
              this.match([pattern], relativePath) &&
              !result.matchedPatterns.includes(pattern)
            ) {
      ...
      result.unmatchedPatterns = patterns.filter(
        (p) => !result.matchedPatterns.includes(p),
      );
      ...
      throw GlobErrors.expansionFailed(
        patterns,
  ...
  private getPatternMatcher(pattern: string): PatternMatcher {
    if (!this.patternCache.has(pattern)) {
      this.patternCache.set(
        pattern,
        new PatternMatcher(pattern, this.options.caseSensitive),
      );
    }
  ...
  private updateStats(result: GlobResult): void {
    ...
    this.stats.avgTimePerPattern =
      this.stats.totalDuration / this.stats.patternsProcessed;
    ...
    this.stats.cacheHitRatio =
      totalCacheAccess > 0 ? cacheHits / totalCacheAccess : 0;
  }
}
