...
describe('Community Analytics System', () => {
  ...
  describe('AnalyticsScheduler', () => {
    ...
    it('should generate overview from results', () => {
      const mockResults = {
        '7d': {
          summary: {
            communityGrowth: { totalContributors: 5, newContributors: 2, growthRate: 40 },
            engagement: { issueCloseRate: 80, prMergeRate: 90, stars: 100, forks: 25 }
          },
          insights: [
            { type: 'positive', impact: 'high', message: 'Great activity', category: 'activity' }
          ]
        },
        '30d': {
          error: 'API Error'
  ...
  describe('Analytics System Integration', () => {
    ...
    it('should provide meaningful duration formatting', () => {
      ...
      expect(analyticsInstance.formatDuration(oneDay + oneHour + oneMinute)).toBe('1d 1h');
    });
