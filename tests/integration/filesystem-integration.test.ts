...
class FileSystemTestUtils {
   ...
   */
  private async createUniqueTempPath(
    prefix: string,
    suffix: string = "",
  ): Promise<string> {
...
class ComprehensiveFileSystemIntegrationTestSuite {
  ...
  async testFileListingOperations(): Promise<void> {
    const tempDir = await this.testUtils.createTempDir();
    const structure =
      await this.testUtils.createTestDirectoryStructure(tempDir);
    ...
    const absoluteRecursiveFiles = recursiveFiles.map((file) =>
      join(tempDir, file),
    );
  ...
  async testFileWatchingCapabilities(): Promise<void> {
    ...
    expect(mockWatcher.getWatchedPaths()).not.toContain(
  ...
  async testCrossPlatformCompatibility(): Promise<void> {
    ...
    const testFile = this.fsManager.resolvePath(
      "test/nested/file.txt",
      tempDir,
    );
    await this.fsManager.atomicWriteFile(testFile, "test content", {
      ensureDir: true,
    });
  ...
  async testErrorHandling(): Promise<void> {
    ...
    await expect(
      this.fsManager.getFileStats(nonExistentPath),
    ).rejects.toThrow();
  ...
  async testFileSystemPerformance(): Promise<void> {
    ...
    const statsPromises = createdFiles.map((file) =>
      this.fsManager.getFileStats(file),
    );
...
describe("File System Integration Tests", () => {
  ...
  describe("Real File Watcher Integration", () => {
    it("should integrate with ChokidarFileWatcher for real file monitoring", async () => {
      ...
      expect(fileWatcher.getWatchStats().watchedFiles).toBeGreaterThanOrEqual(
        0,
      );
