...
export class FileSystemTester {
    ...
    private async testSpecialCharacters(): Promise<TestResult[]> {
        ...
        const platformCharSets = specialCharSets[this.platform as keyof typeof specialCharSets] || specialCharSets.linux;
        ...
        for (const charSet of platformCharSets) {
            ...
            try {
                ...
                const isExpectedFailure = this.isExpectedSpecialCharFailure(charSet.chars, error);
    ...
    private async testFilePermissions(): Promise<TestResult[]> {
        ...
        for (const permTest of permissionTests) {
            ...
            try {
                const testFile = join(this.testDir, `perm-test-${permTest.mode.toString(8)}.txt`);
    ...
    private async testLongPaths(): Promise<TestResult[]> {
        ...
        try {
            ...
            for (let i = 0; i < maxDepth; i++) {
                currentPath = join(currentPath, `very-long-directory-name-level-${i.toString().padStart(3, '0')}`);
            }
    ...
    private async testUnicodeSupport(): Promise<TestResult[]> {
        ...
        const unicodeTests = [
            { name: 'Basic Latin Extended', filename: 'tëst-fîlé.txt', content: 'Hëllö Wörld!' },
            { name: 'Cyrillic', filename: 'тест.txt', content: 'Привет мир!' },
            { name: 'CJK', filename: '测试文件.txt', content: '你好世界！' },
            { name: 'Emoji', filename: 'test🔥file.txt', content: 'Hello 🌍 World! 🚀' },
            { name: 'Mixed Scripts', filename: 'test-тест-测试.txt', content: 'Mixed: English, Русский, 中文' }
        ];
    ...
    private async testFileAttributes(): Promise<TestResult[]> {
        ...
        try {
            ...
            const hasValidDates = stats.mtime instanceof Date && stats.ctime instanceof Date;
    ...
    private async testConcurrentAccess(): Promise<TestResult[]> {
        ...
        try {
            ...
            const allSuccessful = results_data.every((content, i) => 
                content === `Content from operation ${i}`
            );
    ...
    private isExpectedSpecialCharFailure(filename: string, error: unknown): boolean {
        const errorMsg = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase();
        
        // Windows-specific invalid character errors
        if (this.platform === 'win32') {
            const windowsInvalidChars = ['<', '>', ':', '"', '|', '?', '*'];
            return windowsInvalidChars.some(char => filename.includes(char)) &&
    ...
    private isExpectedLongPathFailure(error: unknown): boolean {
        const errorMsg = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase();
        return errorMsg.includes('path') && (errorMsg.includes('long') || errorMsg.includes('limit') || errorMsg.includes('name too long'));
    }

    private isExpectedSymlinkFailure(error: unknown): boolean {
        const errorMsg = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase();
        return errorMsg.includes('operation not permitted') || 
    ...
    private isExpectedHardlinkFailure(error: unknown): boolean {
        const errorMsg = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase();
        return errorMsg.includes('cross-device') || 
