...
export class MCPConnectionManager extends EventEmitter {
  ...
  getStats() {
    ...
    return {
      ...
      connectionsByTransport: connections.reduce((acc, c) => {
   ...
   */
  async sendMessage(connectionId: string, message: MCPResponse | MCPNotification): Promise<void> {
   ...
   */
  private async handleNewConnection(transport: Transport, connectionInfo: ConnectionInfo): Promise<void> {
  ...
  private async handleMessage(connectionId: string, message: any): Promise<void> {
    ...
    try {
      ...
      if (this.isRequest(parsedMessage)) {
        ...
        logger.warn('Received invalid message format', { connectionId, message });
      }
    } catch (error) {
   ...
   */
  private convertToTransportMessage(message: MCPResponse | MCPNotification): TransportMessage {
