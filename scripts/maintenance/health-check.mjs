...
class RepositoryHealthChecker {
  ...
  async runHealthChecks() {
    ...
    try {
      ...
      if (
        this.failOnIssues &&
        (results.overall === "critical" || results.overall === "poor")
      ) {
        process.exit(1);
      }
    } catch (error) {
  ...
  async checkGitHealth() {
    ...
    try {
      ...
      if (status) {
        result.issues.push(
          `Uncommitted changes detected (${status.split("\n").length} files)`,
        );
        result.score -= 10;
      } else {
        result.score += 20;
      ...
      try {
        const largeFiles = execSync(
          'find . -name ".git" -prune -o -type f -size +10M -print',
          {
            cwd: ROOT_DIR,
            encoding: "utf-8",
          },
        ...
        if (largeFiles) {
          result.issues.push(
            `Large files detected: ${largeFiles.split("\n").length}`,
          );
          result.score -= 15;
        } else {
          result.score += 15;
    ...
    try {
      ...
      for (const packageFile of packageFiles) {
        try {
          ...
          result.issues.push(
            `Failed to process ${packageFile}: ${error.message}`,
          );
        }
      }
      ...
      if (totalDeps > 0) {
        ...
        if (outdatedPercentage === 0) result.score += 40;
          ...
          result.issues.push(
            `${outdatedPercentage.toFixed(1)}% dependencies are outdated`,
          );
      ...
      if (lockFilesFound === packageFiles.length) {
        ...
        result.issues.push(
          `Missing lock files for ${packageFiles.length - lockFilesFound} packages`,
        );
      }
  ...
  async checkSecurity() {
    ...
    try {
      ...
      for (const pattern of sensitiveFiles) {
        try {
          const found = execSync(
            `find . -name "${pattern}" -not -path "./.git/*" | wc -l`,
            {
              cwd: ROOT_DIR,
              encoding: "utf-8",
            },
      ...
      if (foundSensitive === 0) {
        ...
        result.issues.push(
          `${foundSensitive} potentially sensitive files found`,
        );
      ...
      if (existsSync(rootPackage)) {
        ...
        if (dangerousScripts === 0) {
          ...
          result.issues.push(
            `${dangerousScripts} potentially dangerous script patterns found`,
          );
  ...
  async checkCodeQuality() {
    ...
    try {
      ...
      const hasLintConfig = lintConfigs.some((config) =>
        existsSync(join(ROOT_DIR, config)),
      );
      ...
      if (codeStats.totalFiles > 0) {
        ...
        if (avgLinesPerFile < 200) {
          ...
          result.issues.push(
            `Large files detected (avg ${Math.round(avgLinesPerFile)} lines/file)`,
          );
        }
    ...
    try {
      ...
      if (existsSync(rootPackage)) {
        ...
        const testScripts = Object.keys(scripts).filter(
          (name) => name.includes("test") || name.includes("spec"),
        );
      ...
      if (codeStats.testFiles > 0) {
        ...
        const testCoverage =
          (codeStats.testFiles / codeStats.sourceFiles) * 100;
        result.metrics.testCoverageByFiles = Math.round(testCoverage);

        if (testCoverage < 25) {
          result.issues.push(
            `Low test file coverage (${Math.round(testCoverage)}%)`,
          );
        }
      } else {
    ...
    try {
      ...
      try {
        ...
        if (buildTime < 30000) {
          ...
          result.issues.push(
            `Build time is slow (${Math.round(buildTime / 1000)}s)`,
          );
        }
      } catch {
      ...
      try {
        ...
        if (testTime < 10000) {
          // < 10 seconds
          result.score += 25;
        } else if (testTime < 30000) {
          // < 30 seconds
          result.score += 15;
        } else {
          result.issues.push(
            `Test suite is slow (${Math.round(testTime / 1000)}s)`,
          );
          result.score += 5;
        }
      } catch {
      ...
      if (existsSync(nodeModulesDir)) {
        try {
          ...
          if (sizeInMB < 200) {
            ...
            result.issues.push(
              `node_modules is very large (${Math.round(sizeInMB)}MB)`,
            );
          }
        } catch {
    ...
    try {
      ...
      try {
        ...
        const daysSinceLastCommit =
          (Date.now() / 1000 - parseInt(lastCommit)) / (24 * 60 * 60);
        ...
        if (daysSinceLastCommit < 7) {
          ...
          result.issues.push(
            `Last commit was ${Math.round(daysSinceLastCommit)} days ago`,
          );
        }
      } catch {
      ...
      const hasTemplates = templateDirs.some((template) =>
        existsSync(join(ROOT_DIR, template)),
      );
      ...
      const hasReleaseConfig = releaseConfigs.some((config) =>
        existsSync(join(ROOT_DIR, config)),
      );
      if (hasReleaseConfig) {
        result.score += 15;
  ...
  findPackageJsonFiles() {
    ...
    try {
      const entries = execSync(
        'find packages -name "package.json" -type f 2>/dev/null || true',
        {
          cwd: ROOT_DIR,
          encoding: "utf-8",
        },
      ...
      for (const ext of extensions) {
        try {
          const files = execSync(
            `find . -name "*.${ext}" -not -path "./node_modules/*" -not -path "./.git/*" -type f`,
            {
              cwd: ROOT_DIR,
              encoding: "utf-8",
            },
  ...
  outputResults(results) {
    ...
    console.log(
      `Health Score: ${results.score}/${results.maxScore} (${results.percentage}%)`,
    );
    console.log(`Timestamp: ${results.timestamp}\n`);

    for (const [checkName, result] of Object.entries(results.checks)) {
      console.log(
        `${this.getStatusEmoji(result.status)} ${checkName}: ${result.status.toUpperCase()} (${result.score || 0}/${result.maxScore || 100})`,
      );
