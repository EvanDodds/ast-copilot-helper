...
describe("ParseGrammarError", () => {
  ...
  it("should create grammar error without language", () => {
    ...
    expect(error.suggestions).toContain(
      "Verify internet connection for grammar downloads",
    );
...
describe("ParseRuntimeError", () => {
  it("should create runtime error with context-aware suggestions", () => {
    ...
    expect(error.suggestions).toContain(
      "Reduce batch size or enable memory throttling",
    );
    expect(error.suggestions).toContain(
  ...
  it("should generate timeout-specific suggestions", () => {
    ...
    expect(error.suggestions).toContain(
      "Check if file is unusually large or complex",
    );
  });
...
describe("ParseFileSystemError", () => {
  ...
  it("should create too many files error with appropriate suggestions", () => {
    const error = new ParseFileSystemError("EMFILE: too many open files");
...
describe("ParseTimeoutError", () => {
  it("should create timeout error with timing information", () => {
    ...
    expect(error.suggestions).toContain(
      "Consider increasing timeout from 5000ms",
    );
    expect(error.suggestions).toContain(
      "Check if file is extremely large or complex",
    );
  });

  it("should create timeout error without timing information", () => {
    const error = new ParseTimeoutError("Operation timed out", "/file.ts");

    expect(error.suggestions).not.toContain("Consider increasing timeout from");
    expect(error.suggestions).toContain(
      "Verify system performance and available resources",
    );
  });
});
...
describe("ParseMemoryError", () => {
  it("should create memory error with usage information", () => {
    ...
    expect(error.suggestions).toContain(
      "Memory usage was 512MB when error occurred",
    );
    expect(error.suggestions).toContain(
      "Enable memory throttling in batch processor",
    );
  ...
  it("should create memory error without usage information", () => {
    const error = new ParseMemoryError("Memory allocation failed");
...
describe("ParseErrorHandler", () => {
  ...
  describe("error creation methods", () => {
    ...
    it("should create various error types", () => {
      const grammarError = errorHandler.createGrammarError(
        "Grammar error",
        "python",
      );
      ...
      const timeoutError = errorHandler.createTimeoutError(
        "Timeout",
        "/file.ts",
        1000,
      );
      ...
      const memoryError = errorHandler.createMemoryError(
        "Memory error",
        "/file.ts",
        256,
      );
  ...
  describe("error recoverability and retry strategies", () => {
    it("should identify recoverable errors", () => {
      ...
      nonRecoverableErrors.forEach((err) => {
        ...
        expect(errorHandler.isRecoverable(errorDetails)).toBe(
          err.message.includes("ENOENT")
            ? false
            : err.type !== "syntax" && err.type !== "configuration",
        );
      });
    });
