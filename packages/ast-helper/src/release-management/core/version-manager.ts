...
import type { VersioningConfig, ChangelogEntry } from "../types.js";
import { ReleaseType, ReleaseChannel } from "../types.js";
...
export class VersionManagerImpl implements VersionManager {
  ...
  async initialize(config: VersioningConfig): Promise<void> {
    ...
    if (config.initialVersion) {
      try {
        ...
        throw new Error(
          `Invalid initial version format: ${config.initialVersion}`,
        );
      }
    }
    ...
    switch (type) {
      ...
      case ReleaseType.PRERELEASE:
        if (parsed.prerelease) {
          const prereleaseNumber = this.extractPrereleaseNumber(
            parsed.prerelease,
          );
          ...
          return this.buildVersion(
            ...
            parsed.minor,
            parsed.patch,
            `${prereleaseType}.${prereleaseNumber + 1}`,
            parsed.build,
          );
        } else {
    ...
    try {
      ...
      if (this.config.scheme === "semver") {
        if (!this.isValidSemver(version)) {
          console.log(
            `❌ Version validation failed: ${version} - invalid semver format`,
          );
          return false;
        }
      }
      ...
      switch (type) {
        case ReleaseType.PRERELEASE:
          if (!parsed.prerelease) {
            console.log(
              `❌ Version validation failed: ${version} - prerelease type requires prerelease identifier`,
            );
            return false;
          }
          break;
        ...
        case ReleaseType.MAJOR:
          ...
          if (parsed.prerelease) {
            console.log(
              `❌ Version validation failed: ${version} - stable release type should not have prerelease identifier`,
            );
            return false;
          }
          break;
   ...
   */
  async validateVersionProgression(
    version: string,
    type: ReleaseType,
  ): Promise<void> {
    this.ensureInitialized();
    ...
    try {
      ...
      switch (type) {
        case ReleaseType.MAJOR:
          if (parsed.major <= currentParsed.major) {
            throw new Error(
              `Major version must be greater than current: ${parsed.major} <= ${currentParsed.major}`,
            );
          }
          break;
        ...
        case ReleaseType.MINOR:
          if (
            ...
            throw new Error(
              `Minor version must be greater than current minor: ${version} vs ${currentVersion}`,
            );
          }
          break;
        ...
        case ReleaseType.PATCH:
          if (
            ...
            throw new Error(
              `Patch version must be greater than current patch: ${version} vs ${currentVersion}`,
            );
          }
          break;
        ...
        case ReleaseType.PRERELEASE:
          if (!parsed.prerelease) {
            throw new Error(
              `Prerelease version must include prerelease identifier: ${version}`,
            );
          }
          break;
      }
      ...
      if (this.config.strictMode) {
        if (type === ReleaseType.MAJOR && parsed.minor !== 0) {
          throw new Error(
            `Major release must reset minor to 0 in strict mode: ${version}`,
          );
        }
        if (
          (type === ReleaseType.MAJOR || type === ReleaseType.MINOR) &&
          parsed.patch !== 0
        ) {
          throw new Error(
            `Major/Minor release must reset patch to 0 in strict mode: ${version}`,
          );
        }
      }
      ...
      console.error(
        `❌ Version progression validation failed: ${version} - ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  async updateVersion(version: string, packages: string[]): Promise<void> {
    ...
    console.log(
      `📝 Updating version to ${version} in ${packages.length} packages`,
    );
    ...
    for (const packagePath of packages) {
      try {
        ...
        throw new Error(
          `Failed to update version in ${packagePath}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  ...
  getVersionChannel(version: string): ReleaseChannel {
    ...
    const prereleaseType = this.extractPrereleaseType(
      parsed.prerelease,
    ).toLowerCase();
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error(
        "VersionManager not initialized. Call initialize() first.",
      );
    }
  }
