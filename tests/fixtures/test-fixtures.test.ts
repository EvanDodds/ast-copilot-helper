...
describe('Test Fixtures and Synthetic Data', () => {
  ...
  it('should generate synthetic repository with specified node count', async () => {
    const nodeCount = 500;
    const repoPath = await ASTTestHelpers.generateSyntheticRepository(nodeCount);
    tempDirs.push(repoPath);

    // Verify repository structure
    const srcExists = await fs.access(repoPath + '/src').then(() => true).catch(() => false);
    expect(srcExists).toBe(true);

    // Verify package.json exists
    const packageExists = await fs.access(repoPath + '/package.json').then(() => true).catch(() => false);
    expect(packageExists).toBe(true);

    // Verify tsconfig.json exists
    const tsconfigExists = await fs.access(repoPath + '/tsconfig.json').then(() => true).catch(() => false);
