...
describe("MetadataManager", () => {
  ...
  describe("Initialization", () => {
    it("should initialize metadata directory structure", async () => {
      ...
      expect(
        existsSync(join(TEST_METADATA_DIR, "metadata", "index.json")),
      ).toBe(true);
    });
  ...
  describe("Metadata Updates", () => {
    ...
    it("should fail to update non-existent metadata", async () => {
      ...
      const success = await metadataManager.updateMetadata(nonExistentModel, {
        verified: false,
      });
    ...
    it("should handle update errors gracefully", async () => {
      ...
      const success = await invalidManager.updateMetadata(mockModel, {
        verified: false,
      });
  ...
  describe("Metadata Synchronization", () => {
    beforeEach(async () => {
      ...
      // Create model files
      await fs.writeFile(
        join(modelsDir, "test-model-1.0.0.onnx"),
        "test content",
      );
      await fs.writeFile(
        join(modelsDir, "embedding-model-2.1.0.onnx"),
        "embedding content",
      );
      await fs.writeFile(
        join(modelsDir, "orphan-model-1.5.0.onnx"),
        "orphan content",
      );
  ...
  describe("Concurrent Operations", () => {
    ...
    it("should handle concurrent queries", async () => {
      ...
      const promises = Array(5)
        .fill(0)
        .map(() => metadataManager.getMetadata(mockModel));
...
describe("Metadata Convenience Functions", () => {
  ...
  it("should provide convenient metadata storage", async () => {
    ...
    await expect(
      storeModelMetadata(mockModel, metadata),
    ).resolves.not.toThrow();
  });

  it("should provide convenient metadata retrieval", async () => {
    const { storeModelMetadata, getModelMetadata } = await import(
      "./metadata.js"
    );
  ...
  it("should provide convenient metadata querying", async () => {
    const { storeModelMetadata, queryModelMetadata } = await import(
      "./metadata.js"
    );
