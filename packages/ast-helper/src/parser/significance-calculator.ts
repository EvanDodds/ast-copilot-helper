...
export class SignificanceCalculator {
  ...
  private getBaseSignificance(nodeType: NodeType): SignificanceLevel {
    ...
    if (
      [NodeType.FILE, NodeType.MODULE, NodeType.NAMESPACE].includes(nodeType)
    ) {
    ...
    if ([NodeType.SWITCH_STATEMENT, NodeType.TRY_CATCH].includes(nodeType)) {
      return SignificanceLevel.MEDIUM;
    }

    // Low: Simple control flow
    if (
      [NodeType.IF_STATEMENT, NodeType.FOR_LOOP, NodeType.WHILE_LOOP].includes(
        nodeType,
      )
    ) {
    ...
    if (
      [NodeType.PROPERTY, NodeType.FIELD, NodeType.VARIABLE].includes(nodeType)
    ) {
      return SignificanceLevel.LOW;
    }
  ...
  private calculateComplexityFactor(
    ...
    if (
      [NodeType.FUNCTION, NodeType.METHOD, NodeType.CONSTRUCTOR].includes(
        node.type,
      )
    ) {
      const parameterCount = context.children.filter(
        (child) => child.type === NodeType.PARAMETER,
      ).length;
    ...
    if (node.type === NodeType.CLASS) {
      const memberCount = context.children.filter((child) =>
        [NodeType.METHOD, NodeType.PROPERTY, NodeType.FIELD].includes(
          child.type,
        ),
      ).length;
   ...
   */
  private calculateScopeFactor(
    node: ASTNode,
    _context: SignificanceContext,
  ): number {
  ...
  private calculateUsageFactor(
    ...
    if (node.name) {
      ...
      if (
        ["main", "index", "init", "start", "run"].some((keyword) =>
      ...
      if (
        ["api", "public", "export"].some((keyword) => name.includes(keyword))
      ) {
        usageFactor += 0.5;
      }
      ...
      // Core/base classes
      if (
        ["base", "core", "abstract", "interface"].some((keyword) =>
          name.includes(keyword),
        )
      ) {
        usageFactor += 0.5;
    ...
    if (node.name) {
      const name = node.name.toLowerCase();
      if (
        ["temp", "tmp", "test", "debug", "util", "helper"].some((keyword) =>
          name.includes(keyword),
        )
      ) {
        usageFactor -= 0.5;
  ...
  private generateExplanation(
    ...
    if (complexity !== 0) {
      parts.push(
        `Complexity: ${complexity > 0 ? "+" : ""}${complexity.toFixed(1)}`,
      );
    }
  ...
  private significanceLevelName(level: SignificanceLevel): string {
    switch (level) {
      case SignificanceLevel.CRITICAL:
        return "Critical";
      case SignificanceLevel.HIGH:
        return "High";
      case SignificanceLevel.MEDIUM:
        return "Medium";
      case SignificanceLevel.LOW:
        return "Low";
      case SignificanceLevel.MINIMAL:
        return "Minimal";
      default:
        return "Unknown";
    }
...
export class SignificanceUtils {
   ...
   */
  static compareSignificance(
    level1: SignificanceLevel,
    level2: SignificanceLevel,
  ): number {
   ...
   */
  static groupBySignificance(
    nodes: ASTNode[],
  ): Record<SignificanceLevel, ASTNode[]> {
