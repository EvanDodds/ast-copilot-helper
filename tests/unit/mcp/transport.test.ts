...
import { JSONRPCRequest, JSONRPCResponse } from '../../../packages/ast-mcp-server/src/mcp/protocol';
...
describe('Transport Layer', () => {
  ...
  describe('StdioTransport', () => {
    ...
    it('should fail to start if already running', async () => {
      ...
      await expect(transport.start()).rejects.toThrow('Stdio transport is already running');
    });
    ...
    it('should fail to send message when not running', async () => {
      ...
      await expect(transport.sendMessage(response)).rejects.toThrow('Stdio transport is not running');
    });
  ...
  describe('Transport Message Handling', () => {
    ...
    it('should serialize messages correctly', () => {
      ...
      // Access the protected method through reflection for testing
      const serializeMessage = (transport as any).serializeMessage.bind(transport);
