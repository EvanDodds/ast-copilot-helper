...
describe("NodeProcessor", () => {
  ...
  describe("Single File Processing", () => {
    ...
    it("should generate correct file hash", async () => {
      ...
      const result3 = await processor.processFile(
        mockFilePath,
        "typescript",
        "content1",
      );
  ...
  describe("Directory Processing", () => {
    it("should discover and process files in directory", async () => {
      ...
      expect(result.results.some((r) => r.filePath.includes("file1.ts"))).toBe(
        true,
      );
      expect(result.results.some((r) => r.filePath.includes("file2.js"))).toBe(
        true,
      );
    });
    ...
    it("should respect file extension filters", async () => {
      ...
      expect(
        result.results.every(
          (r) => r.filePath.endsWith(".py") || r.filePath.endsWith(".java"),
        ),
      ).toBe(true);
    });
  ...
  describe("Node Validation", () => {
    ...
    it("should detect missing required fields", () => {
      ...
      expect(validation.errors).toContain(
        'Node missing ID: {"type":"function","name":"testFunc","filePath":"/test.ts","start":{"line":1,"column":0},"end":{"line":5,"column":0},"children":[]}',
      );
      expect(
        validation.errors.some((e) => e.includes("missing metadata")),
      ).toBe(true);
      expect(
        validation.errors.some((e) => e.includes("missing significance")),
      ).toBe(true);
    });
    ...
    it("should detect invalid position ranges", () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(
        validation.errors.some((e) => e.includes("Invalid position range")),
      ).toBe(true);
    });
    ...
    it("should detect invalid child IDs", () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(
        validation.errors.some((e) => e.includes("Invalid child ID")),
      ).toBe(true);
    });
  });
  ...
  describe("Error Handling", () => {
    it("should create ProcessingError with correct properties", () => {
      ...
      expect(processingError.message).toContain(
        "Processing failed at validation for /test/file.ts",
      );
    ...
    it("should handle processing stage failures gracefully", async () => {
      ...
      const result = await invalidProcessor.processFile(
        mockFilePath,
        "typescript",
      );
...
describe("Integration Tests", () => {
  it("should handle complex processing workflow end-to-end", async () => {
    ...
    mockFs.readFileSync
      .mockReturnValueOnce(
        "function complexFunction(a: string, b: number): boolean { if (a.length > b) { return true; } return false; }",
      )
