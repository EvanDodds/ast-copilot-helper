...
export class FileVerifier {
  ...
  async verifyONNXFormat(filePath: string): Promise<ValidationResult> {
    try {
      ...
      const { bytesRead } = await fileHandle.read(buffer, 0, ONNX_MIN_HEADER_SIZE, 0);
      ...
      const magicMatch = buffer.subarray(0, ONNX_MAGIC.length).equals(ONNX_MAGIC);
      ...
      return {
        valid: false,
        errors: [`ONNX format verification failed: ${error instanceof Error ? error.message : String(error)}`],
        warnings: []
      };
    }
  }
...
break;
        ...
        switch (wireType) {
          case 0: // Varint
            offset++;
            while (offset < buffer.length && buffer[offset] !== undefined && (buffer[offset]! & 0x80)) {
              offset++;
            }
            offset++;
            break;
   ...
   * - ✅ Quarantine failed downloads
   */
  async quarantineFile(filePath: string, details: Partial<QuarantineEntry>): Promise<void> {
    try {
      ...
      logger.warn(`File quarantined: ${filePath} -> ${quarantinePath} (${entry.reason})`);

    } catch (error) {
      logger.error(`Failed to quarantine file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async cleanupQuarantine(maxAgeDays = 7): Promise<number> {
    try {
      ...
      for (const entry of entries) {
        ...
        if (entryDate < cutoffDate) {
          try {
            ...
            logger.warn(`Failed to delete quarantined file ${entry.filePath}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
      ...
      if (cleanedCount > 0) {
        ...
        await fs.writeFile(metadataPath, JSON.stringify(remainingEntries, null, 2));
      }

      logger.info(`Cleaned up ${cleanedCount} quarantined files older than ${maxAgeDays} days`);
      return cleanedCount;

    } catch (error) {
      logger.error(`Quarantine cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  async restoreQuarantinedFile(quarantinePath: string, targetPath: string): Promise<void> {
    try {
      ...
      logger.info(`Restored quarantined file: ${quarantinePath} -> ${targetPath}`);
    } catch (error) {
      logger.error(`Failed to restore quarantined file: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
}
