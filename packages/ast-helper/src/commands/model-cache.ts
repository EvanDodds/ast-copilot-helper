...
export class ModelCacheCommandHandler {
  ...
  private async listCachedModels(cache: ModelCache): Promise<void> {
    ...
    this.logger.info(
      `  Total cache size: ${this.formatBytes(stats.totalSize)}`,
    );
  }
  ...
  private async verifyCache(cache: ModelCache): Promise<void> {
    ...
    this.logger.info(
      `📊 Verification complete: ${stats.validModels} valid, ${stats.invalidModels} invalid`,
    );
  }
  ...
  private async showCacheStatus(cache: ModelCache): Promise<void> {
    ...
    this.logger.info(
      `  Cache utilization: ${utilizationPercent}% of 10GB limit`,
    );
  }
