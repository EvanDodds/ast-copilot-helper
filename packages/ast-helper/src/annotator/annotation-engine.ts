...
export class AnnotationEngine {
    ...
    public async generateAnnotation(
        ...
        try {
            ...
            const annotation: Annotation = {
                ...
                calls: dependencies?.filter(d => d.type === 'call').map(d => d.source) || [],
                ...
                completeness: this.calculateCompleteness(node, signature, parameters, summary),
            ...
            this.recordPerformance('total-annotations', this.annotationCache.size + 1);
            ...
            this.logError(error as Error, `generateAnnotation for ${node.type}:${node.name}`);
            const processingTime = performance.now() - startTime;
    ...
    public async generateAnnotationsBatch(
        ...
        options: { maxConcurrency?: number; progressCallback?: (completed: number, total: number) => void } = {}
    ): Promise<Annotation[]> {
        const { maxConcurrency = this.config.maxConcurrency, progressCallback } = options;
    ...
    private async extractSignature(node: ASTNode, sourceText?: string, filePath?: string): Promise<string | null> {
        try {
    ...
    private async extractParameters(node: ASTNode, sourceText?: string, filePath?: string): Promise<Parameter[]> {
        try {
            const language = this.detectLanguage(filePath, node);
            const extractor = this.extractors.get(language);
...
return 1;
    ...
    private async analyzeDependencies(node: ASTNode, _sourceText?: string, filePath?: string) {
        try {
    ...
    private async generateSummary(node: ASTNode, sourceText?: string, filePath?: string): Promise<string> {
        try {
            return await this.summaryGenerator.generateSummary(node, sourceText, filePath);
        } catch (error) {
    ...
    private async generateSemanticTags(node: ASTNode, sourceText?: string, filePath?: string): Promise<SemanticTag[]> {
        try {
            return await this.summaryGenerator.generateSemanticTags(node, sourceText, filePath);
        } catch (error) {
    ...
    private async determinePurposeCategory(node: ASTNode, sourceText?: string, _filePath?: string): Promise<PurposeCategory> {
        try {
            return await this.summaryGenerator.determinePurposeCategory(node, sourceText);
        } catch (error) {
...
return 0;
    ...
    private extractContextLines(node: ASTNode, sourceText?: string): { before: string[]; after: string[] } {
        ...
        const beforeStart = Math.max(0, currentLine - this.config.contextLinesBefore);
        const afterEnd = Math.min(lines.length, currentLine + 1 + this.config.contextLinesAfter);
    ...
    private extractCodeSnippet(node: ASTNode, sourceText?: string): string {
        ...
        try {
            const lines = sourceText.split('\n');
            const startLine = Math.max(0, (node.start.line || 1) - this.config.contextLinesBefore - 1);
            const endLine = Math.min(lines.length, (node.end.line || 1) + this.config.contextLinesAfter);
    ...
    private generateFallbackSignature(node: ASTNode): string {
        ...
        if (type.toLowerCase().includes('function') || type.toLowerCase().includes('method')) {
            return `${name}()`;
        }
    ...
    private calculateConfidence(node: ASTNode, signature?: string | null, summary?: string): number {
...
confidence += 0.1;
    ...
    private calculateCompleteness(node: ASTNode, signature?: string | null, parameters?: Parameter[], summary?: string): number {
...
completeness += 1 / totalAspects;
    ...
    private createFallbackAnnotation(node: ASTNode, filePath?: string, _error?: Error): Annotation {
        return {
            nodeId: node.id,
            filePath: filePath || '',
