...
describe("SignificanceCalculator", () => {
  ...
  describe("Basic Significance Calculation", () => {
    ...
    it("should assign CRITICAL significance to files and modules", () => {
      ...
      expect(calculator.calculateSignificance(fileNode)).toBe(
        SignificanceLevel.CRITICAL,
      );
      expect(calculator.calculateSignificance(moduleNode)).toBe(
        SignificanceLevel.CRITICAL,
      );
    });
    ...
    it("should assign CRITICAL significance to classes and interfaces", () => {
      ...
      const interfaceNode = createTestNode(
        NodeType.INTERFACE,
        "ITestInterface",
      );

      expect(calculator.calculateSignificance(classNode)).toBe(
        SignificanceLevel.CRITICAL,
      );
      expect(calculator.calculateSignificance(interfaceNode)).toBe(
        SignificanceLevel.CRITICAL,
      );
    });
    ...
    it("should assign HIGH significance to function-like constructs", () => {
      ...
      expect(calculator.calculateSignificance(functionNode)).toBe(
        SignificanceLevel.HIGH,
      );
      expect(calculator.calculateSignificance(methodNode)).toBe(
        SignificanceLevel.HIGH,
      );
      expect(calculator.calculateSignificance(constructorNode)).toBe(
        SignificanceLevel.HIGH,
      );
    });
    ...
    it("should assign MEDIUM significance to type definitions", () => {
      ...
      expect(calculator.calculateSignificance(enumNode)).toBe(
        SignificanceLevel.MEDIUM,
      );
      expect(calculator.calculateSignificance(typeAliasNode)).toBe(
        SignificanceLevel.MEDIUM,
      );
      expect(calculator.calculateSignificance(importNode)).toBe(
        SignificanceLevel.MEDIUM,
      );
    });
    ...
    it("should assign LOW significance to simple constructs", () => {
      ...
      expect(calculator.calculateSignificance(variableNode)).toBe(
        SignificanceLevel.LOW,
      );
      expect(calculator.calculateSignificance(propertyNode)).toBe(
        SignificanceLevel.LOW,
      );
      expect(calculator.calculateSignificance(ifNode)).toBe(
        SignificanceLevel.LOW,
      );
    });
    ...
    it("should assign MINIMAL significance to basic constructs", () => {
      ...
      expect(calculator.calculateSignificance(parameterNode)).toBe(
        SignificanceLevel.MINIMAL,
      );
      expect(calculator.calculateSignificance(commentNode)).toBe(
        SignificanceLevel.MINIMAL,
      );
      expect(calculator.calculateSignificance(stringNode)).toBe(
        SignificanceLevel.MINIMAL,
      );
    });
  });
  ...
  describe("Complexity Factor Adjustment", () => {
    it("should increase significance for nodes with many children", () => {
      const context: Partial<SignificanceContext> = {
        children: Array(15)
          .fill(null)
          .map((_, i) => createTestNode(NodeType.VARIABLE, `var${i}`)),
      };
      ...
      expect(factors.finalSignificance).toBeGreaterThanOrEqual(
        SignificanceLevel.HIGH,
      );
    });
    ...
    it("should increase significance for functions with many parameters", () => {
      const parameters = Array(6)
        .fill(null)
        .map((_, i) => createTestNode(NodeType.PARAMETER, `param${i}`));
    ...
    it("should increase significance for classes with many members", () => {
      const members = Array(8)
        .fill(null)
        .map((_, i) => createTestNode(NodeType.METHOD, `method${i}`));
  ...
  describe("Batch Processing", () => {
    ...
    it("should be consistent between single and batch calculations", () => {
      ...
      const singleResults = nodes.map((node) =>
        calculator.calculateSignificance(node),
      );
      calculator.resetStats();
  ...
  describe("Configuration", () => {
    it("should respect custom configuration", () => {
      ...
      const commentNode = createTestNode(NodeType.COMMENT);
      const commentSignificance =
        customCalculator.calculateSignificance(commentNode);
      expect(commentSignificance).toBeGreaterThanOrEqual(
        SignificanceLevel.MEDIUM,
      );

      // Test maximum significance
      const classNode = createTestNode(NodeType.CLASS, "TestClass");
      const classSignificance =
        customCalculator.calculateSignificance(classNode);
    ...
    it("should disable complexity adjustment when configured", () => {
      ...
      const factors = customCalculator.calculateSignificanceFactors(
        complexNode,
        context,
      );
...
describe("SignificanceUtils", () => {
  ...
  describe("isAboveThreshold", () => {
    it("should correctly identify levels above threshold", () => {
      expect(
        SignificanceUtils.isAboveThreshold(
          SignificanceLevel.HIGH,
          SignificanceLevel.MEDIUM,
        ),
      ).toBe(true);
  ...
  describe("filterBySignificance", () => {
    it("should filter nodes by significance threshold", () => {
      ...
      const filtered = SignificanceUtils.filterBySignificance(
        nodes,
        SignificanceLevel.MEDIUM,
      );
      ...
      expect(
        filtered.every(
          (node: ASTNode) => node.significance >= SignificanceLevel.MEDIUM,
        ),
      ).toBe(true);
    });
  });
