...
describe("File Locking System", () => {
  ...
  beforeEach(async () => {
    ...
    testWorkspace = join(
      tmpdir(),
      `ast-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    );
  ...
  describe("LockManager - Exclusive Locks", () => {
    ...
    it("should prevent multiple exclusive locks", async () => {
      ...
      await expect(
        lockManager2.acquireExclusiveLock("test-operation-2"),
  ...
  describe("LockManager - Shared Locks", () => {
    ...
    it("should prevent shared lock when exclusive lock exists", async () => {
      const exclusiveLock =
        await lockManager.acquireExclusiveLock("test-write");
      ...
      await expect(lockManager2.acquireSharedLock("test-read")).rejects.toThrow(
        LockTimeoutError,
      );
    });
  });
