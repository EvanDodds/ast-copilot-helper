...
export class ComplexityAnalyzer {
   ...
   */
  calculateDetailedMetrics(node: ASTNode, sourceText: string): ComplexityMetrics {
    ...
    const keywords = this.config.decisionKeywords[language] || this.config.decisionKeywords.javascript || [];
    ...
    const cognitive = this.calculateCognitiveComplexity(nodeContent, keywords, maxNesting);
  ...
  private calculateStandardComplexity(
    content: string,
    keywords: string[]
  ): { complexity: number; breakdown: Record<string, number>; decisionPoints: number; maxNesting: number } {
    let complexity = 1; // Base complexity
    const breakdown: Record<string, number> = {};
    ...
    const complexityTypes = {
      ...
      logical: ['&&', '||', 'and', 'or', '??'],
      ternary: ['?', ':'],
      functional: ['forEach', 'map', 'filter', 'reduce', 'find', 'some', 'every']
    };
...
continue;
      ...
      if (keywords.includes(lowerToken)) {
        ...
        if (lowerToken === 'else' && i > 0 && tokens[i-1]?.toLowerCase() === 'if') {
   ...
   * Calculate cognitive complexity (accounts for nesting)
   */
  private calculateCognitiveComplexity(content: string, keywords: string[], maxNesting: number): number {
    // Cognitive complexity gives higher weight to nested constructs
    const baseComplexity = this.calculateStandardComplexity(content, keywords).complexity;
   ...
   */
  private categorizeComplexity(complexity: number): 'low' | 'medium' | 'high' | 'very-high' {
