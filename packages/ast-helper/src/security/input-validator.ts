...
export class ComprehensiveInputValidator implements InputValidator {
    ...
    try {
      ...
      if (inputStr.length > this.config.general.maxInputSize) {
        ...
        return this.createValidationResult(
          false,
          `Input exceeds maximum size of ${this.config.general.maxInputSize} characters`,
        );
      }
      ...
      if (
        this.config.sqlInjection.enabled &&
        (context?.inputType === "sql" ||
          (!context?.inputType && this.containsSQLPattern(normalizedInput)))
      ) {
      ...
      if (
        this.config.xssProtection.enabled &&
        (context?.inputType === "html" ||
          (!context?.inputType && this.containsHTMLPattern(normalizedInput)))
      ) {
      ...
      if (
        this.config.commandInjection.enabled &&
        context?.inputType === "command"
      ) {
      ...
      if (isValid) {
        ...
        this.log("warn", "Input validation failed", {
          violations,
          input: this.truncateForLog(inputStr),
        });
      }
  ...
  async sanitizeInput(
    ...
    try {
      ...
      return this.createSanitizationResult(
        sanitized,
        wasModified,
        changes.join("; "),
      );
  ...
  private containsSQLPattern(input: string): boolean {
    const sqlKeywords =
      /\b(select|insert|update|delete|from|where|join|union|drop|alter|create|table|or|and)\b/i;
    ...
    return (
      sqlKeywords.test(input) ||
      sqlOperators.test(input) ||
      sqlComments.test(input)
    );
  }
  ...
  private async validateSQLInjection(
    input: string,
  ...
  private async validatePathTraversal(
    ...
    if (
      !this.config.pathTraversal.allowAbsolutePaths &&
      (input.startsWith("/") || /^[A-Z]:/.test(input))
    ) {
  ...
  private async validateCommandInjection(
    input: string,
  ): Promise<{ isValid: boolean; violations: string[] }> {
    const violations: string[] = [];
  ...
  private async validateDataTypes(
    ...
    if (inputType === "string") {
      ...
      if (
        num < this.config.dataTypes.numberRange.min ||
        num > this.config.dataTypes.numberRange.max
      ) {
  ...
  private createSanitizationResult(
    sanitized: string,
    wasModified: boolean,
    message: string,
  ): SanitizationResult {
...
export class InputValidationUtils {
  ...
  static generateValidationReport(results: InputValidationResult[]): {
    ...
    const validationRate =
      totalChecks > 0 ? (validInputs / totalChecks) * 100 : 0;
      ...
      express: (req: unknown, res: unknown, next: unknown) => {
        ...
        const validateData = async () => {
          try {
            ...
            const queryValidation = await validator.validateInput(
              request.query,
            );
