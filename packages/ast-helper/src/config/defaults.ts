...
export function validateConfigLegacy(config: PartialConfig): Config {
  ...
  if (config.parseGlob) {
    ...
    result.parseGlob = config.parseGlob.filter(
  ...
  if (config.watchGlob) {
    ...
    result.watchGlob = config.watchGlob.filter(
      (pattern) => typeof pattern === "string",
    );
  }
  ...
  if (config.modelHost !== undefined) {
    if (
      typeof config.modelHost !== "string" ||
      config.modelHost.trim().length === 0
    ) {
  ...
  if (config.fileWatching) {
    ...
    if (watchConfig.watchPaths !== undefined) {
      ...
      const validPaths = watchConfig.watchPaths.filter(
    ...
    if (watchConfig.enableRecursive !== undefined) {
      result.fileWatching!.enableRecursive = Boolean(
        watchConfig.enableRecursive,
      );
    }
