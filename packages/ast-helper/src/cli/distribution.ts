  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `📊 Validated ${Object.keys(result.results || {}).length} publishers`,
        );
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `📊 Distributed to ${Object.keys(result.results || {}).length} channels`,
        );
        ...
        if (result.results) {
          Object.entries(result.results).forEach(
            ([channel, channelResult]: [string, any]) => {
              const status = channelResult.success ? "✅" : "❌";
              console.log(
                `  ${status} ${channel}: ${channelResult.success ? "Success" : "Failed"}`,
              );
            },
          );
        }
      ...
      if (result.success) {
        ...
        if (result.results) {
          Object.entries(result.results).forEach(([channel, verification]) => {
            console.log(
              `  ✅ ${channel}: ${verification.checks?.length || 0} checks passed`,
            );
          });
        }
      } else {
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.error(
        `💥 Verification report generation failed: ${error.message}`,
      );
      process.exit(1);
    }
  });
