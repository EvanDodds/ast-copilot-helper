...
export class ParsingBenchmarkRunner {
   ...
   */
  async runParsingBenchmarks(
    config: ParsingBenchmarkConfig,
   ...
   */
  private async runParsingBenchmark(
    config: ParsingBenchmarkConfig,
  ...
  private generateTypeScriptTestData(nodeCount: NodeCount): string {
    ...
    while (currentNodes < targetNodes) {
      ...
      // Generate methods
      const methodsNeeded = Math.min(
        10,
        Math.ceil((targetNodes - currentNodes) / 50),
      );
      for (let i = 1; i <= methodsNeeded && currentNodes < targetNodes; i++) {
        lines.push(
          `  public method${i}(param: string): Promise<TestInterface[]> {`,
        );
  ...
  private generatePythonTestData(nodeCount: NodeCount): string {
    ...
    while (currentNodes < targetNodes) {
      ...
      const methodsNeeded = Math.min(
        8,
        Math.ceil((targetNodes - currentNodes) / 40),
      );
      for (let i = 1; i <= methodsNeeded && currentNodes < targetNodes; i++) {
        lines.push(
          `    async def method_${i}(self, param: str) -> List[TestData]:`,
        );
        lines.push("        items = []");
        ...
        lines.push(
          '                data={"index": j, "timestamp": asyncio.get_event_loop().time()}',
        );
        lines.push("            ))");
  ...
  private generateJavaScriptTestData(nodeCount: NodeCount): string {
    ...
    while (currentNodes < targetNodes) {
      ...
      const methodsNeeded = Math.min(
        8,
        Math.ceil((targetNodes - currentNodes) / 35),
      );
      for (let i = 1; i <= methodsNeeded && currentNodes < targetNodes; i++) {
  ...
  private generateJavaTestData(nodeCount: NodeCount): string {
    ...
    while (currentNodes < targetNodes) {
      ...
      const methodsNeeded = Math.min(
        6,
        Math.ceil((targetNodes - currentNodes) / 45),
      );
      for (let i = 1; i <= methodsNeeded && currentNodes < targetNodes; i++) {
        lines.push(
          `    public CompletableFuture<List<TestData>> method${i}(String param) {`,
        );
        ...
        lines.push(
          '                item.data = Map.of("index", j, "timestamp", Instant.now().toEpochMilli());',
        );
  ...
  private getNumericNodeCount(nodeCount: NodeCount): number {
    switch (nodeCount) {
      ...
      case "xlarge":
        return 500000;
      default:
        return typeof nodeCount === "number" ? nodeCount : 10000;
    }
  }
  ...
  private analyzeBenchmarkResults(
    ...
    if (successfulRuns.length === 0) {
      return {
        ...
        performanceScore: 0,
        recommendations: [
          "All benchmark runs failed - check system resources and test data",
        ],
      };
    }

    const durations = successfulRuns.map((run) => run.duration);
    const parseTimes = successfulRuns.map(
      (run) => run.metadata?.parseTime || run.duration,
    );
    ...
    if (targetNodes >= 100000 && avgParseTime > fiveMinutesMs) {
      warnings.push(
        `Parsing 100k+ nodes took ${avgParseTime}ms, exceeds 5 minute target`,
      );
    }
    ...
    if (avgThroughput < 300) {
      warnings.push(
        `Throughput ${avgThroughput.toFixed(0)} nodes/sec below 300 nodes/sec target`,
      );
    }
    ...
    const performanceScore = Math.min(
      100,
      Math.max(0, (avgThroughput / 300) * 100),
    );

    const recommendations: string[] = [];
    if (avgThroughput < 300) {
      recommendations.push(
        "Consider optimizing parsing algorithms for better throughput",
      );
    }
    if (avgMemory > 1000 * 1024 * 1024) {
      // > 1GB
      recommendations.push(
    ...
    return {
      ...
      totalNodesProcessed: successfulRuns.reduce(
        (total, run) => total + run.nodeCount,
        0,
      ),
      errors: failedRuns.map((run) => run.error!),
      warnings,
