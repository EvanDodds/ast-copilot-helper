...
describe('NodeVersionCompatibilityTester', () => {
  ...
  describe('Node.js version compatibility tests', () => {
    ...
    it('should test Node.js version parsing correctly', async () => {
      const result = await tester.runTests();
      const versionTest = result.testResults.find(test => 
        test.name === 'node_version_parsing'
      );
    ...
    it('should test ES Module support', async () => {
      const result = await tester.runTests();
      const esModuleTest = result.testResults.find(test => 
        test.name === 'es_module_support'
      );
    ...
    it('should test modern JavaScript features', async () => {
      ...
      jsFeatureTests.forEach(testName => {
        ...
        if (test!.name === 'optional_chaining' || test!.name === 'nullish_coalescing') {
    ...
    it('should test performance characteristics', async () => {
      const result = await tester.runTests();
      const perfTest = result.testResults.find(test => 
        test.name === 'performance_characteristics'
      );
    ...
    it('should test native module compatibility', async () => {
      const result = await tester.runTests();
      const nativeTest = result.testResults.find(test => 
        test.name === 'native_module_compatibility'
      );
    ...
    it('should test worker threads support', async () => {
      const result = await tester.runTests();
      const workerTest = result.testResults.find(test => 
        test.name === 'worker_threads_support'
      );
    ...
    it('should test stream API compatibility', async () => {
      const result = await tester.runTests();
      const streamTest = result.testResults.find(test => 
        test.name === 'stream_api_compatibility'
      );
    ...
    it('should test async features', async () => {
      const result = await tester.runTests();
      const asyncTest = result.testResults.find(test => 
        test.name === 'async_features'
      );
    ...
    it('should test error handling', async () => {
      const result = await tester.runTests();
      const errorTest = result.testResults.find(test => 
  ...
  describe('summary generation', () => {
    it('should generate accurate compatibility summary', async () => {
      ...
      expect(typeof result.summary.featureCompatibility.esModules).toBe('boolean');
      expect(typeof result.summary.featureCompatibility.asyncAwait).toBe('boolean');
      expect(typeof result.summary.featureCompatibility.optionalChaining).toBe('boolean');
      ...
      expect(typeof result.summary.apiCompatibility.worker_threads).toBe('boolean');
      
      // Performance metrics
      expect(typeof result.summary.performanceMetrics.memoryUsage).toBe('number');
      expect(typeof result.summary.performanceMetrics.v8HeapSize).toBe('number');
    });
