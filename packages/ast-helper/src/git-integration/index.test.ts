...
describe("GitRepositoryValidator", () => {
  ...
  describe("validateRepository", () => {
    it("should validate a Git repository successfully", async () => {
      ...
      await expect(
        validator.validateRepository(workspaceDir),
    ...
    it("should handle Git command errors gracefully", async () => {
      const workspaceDir = "/test/workspace";
      mockGitManager.isGitRepository.mockRejectedValue(
  ...
  describe("validateGitReference", () => {
    it("should validate existing Git reference", async () => {
      ...
      await expect(
        validator.validateGitReference(ref, workspaceDir),
      ).resolves.toBeUndefined();

      expect(mockGitManager.validateGitReference).toHaveBeenCalledWith(
        ref,
        workspaceDir,
      );
    });
    ...
    it("should throw error for invalid Git reference", async () => {
      ...
      await expect(
        validator.validateGitReference(ref, workspaceDir),
      ).rejects.toThrow('Git reference "nonexistent-branch" does not exist');
    });
  });
});
...
describe("GitFileSelector", () => {
  ...
  describe("selectFiles", () => {
    ...
    it("should select changed files successfully", async () => {
      ...
      expect(mockValidator.validateRepository).toHaveBeenCalledWith(
        workspaceDir,
      );
    });
    ...
    it("should validate Git reference when base is specified", async () => {
      ...
      expect(mockValidator.validateGitReference).toHaveBeenCalledWith(
        "develop",
    ...
    it("should handle Git validation errors", async () => {
      ...
      await expect(
        selector.selectFiles(mockOptions, mockConfig),
...
describe("ParseGitUtils", () => {
  ...
  describe("validateGitPreconditions", () => {
    it("should validate Git preconditions for changed files", async () => {
      ...
      await expect(
        gitUtils.validateGitPreconditions(options),
      ).resolves.toBeUndefined();
