...
import type { BatchProcessingResult } from "../parser/batch-processor.js";
...
export class FileProcessor {
  ...
  private constructor(
    ...
    outputManager: ASTOutputManager,
  ) {
  ...
  async processFiles(
    ...
    try {
      ...
      let finalProgressCallback: ProcessingProgressCallback | undefined =
        onProgress;
  ...
  private async processBatches(
    ...
    const orchestratorCallback = onProgress
    ...
    try {
      ...
      for (const [filePath, parseResult] of Array.from(
        ...
        const processingResult: FileProcessingResult = {
          ...
          error:
            parseResult.errors.length > 0 && parseResult.errors[0]
