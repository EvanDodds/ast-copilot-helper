...
import {
  DEFAULT_CONFIG,
  validateConfig,
} from "../../../packages/ast-helper/src/config/defaults.js";
import type {
  PartialConfig,
  Config,
} from "../../../packages/ast-helper/src/types.js";
...
describe("File Watching Configuration", () => {
  ...
  describe("validateConfig", () => {
    ...
    it("should preserve defaults for missing file watching properties", () => {
      ...
      expect(result.fileWatching!.includePatterns).toEqual(
        DEFAULT_CONFIG.fileWatching!.includePatterns,
      );
      expect(result.fileWatching!.excludePatterns).toEqual(
        DEFAULT_CONFIG.fileWatching!.excludePatterns,
      );
      expect(result.fileWatching!.debounceMs).toBe(
        DEFAULT_CONFIG.fileWatching!.debounceMs,
      );
    });
    ...
    describe("watchPaths validation", () => {
      ...
      it("should filter out invalid watchPaths", () => {
        ...
        expect(result.fileWatching!.watchPaths).toEqual([
          "valid/path",
          "another/valid",
        ]);
      });
  ...
  describe("integration with existing config", () => {
    it("should work alongside existing configuration options", () => {
      ...
      expect(result.fileWatching!.batchSize).toBe(
        DEFAULT_CONFIG.fileWatching!.batchSize,
      );
    });
