...
class QualityReporter {
  ...
  private getCoverageData(): QualityReport['coverage'] {
    ...
    try {
      ...
      const score = (total.lines.pct + total.statements.pct + total.functions.pct + total.branches.pct) / 4;
  ...
  private calculateOverallScore(report: QualityReport): QualityReport['overall'] {
    // Weighted scoring
    const weights = {
      coverage: 0.25,
      security: 0.30,
    ...
    const codeQualityScore = report.codeQuality.lintErrors === 0 && 
                           report.codeQuality.formatErrors === 0 && 
                           report.codeQuality.typeErrors === 0 ? 100 : 0;
  ...
  private displayReport(report: QualityReport): void {
    ...
    const gradeColor = report.overall.score >= 80 ? '🟢' : 
                      report.overall.score >= 60 ? '🟡' : '🔴';
    
    console.log(`${gradeColor} Overall Grade: ${report.overall.grade} (${report.overall.score}/100)`);
    console.log(`${report.overall.passed ? '✅' : '❌'} Quality Gate: ${report.overall.passed ? 'PASSED' : 'FAILED'}`);
    console.log('');
    ...
    console.log(`  Code Quality: ${report.codeQuality.lintErrors === 0 ? '100' : '0'}/100`);
    console.log('');
