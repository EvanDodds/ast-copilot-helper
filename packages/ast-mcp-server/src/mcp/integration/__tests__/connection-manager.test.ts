...
import { MCPConnectionManager, type ClientConnection } from '../connection-manager.js';
...
describe('MCPConnectionManager', () => {
  ...
  describe('Transport Management', () => {
    it('should add transport successfully', async () => {
      ...
      const connectionInfo = createConnectionInfo('test-connection-1', 'test-client');
    ...
    it('should handle multiple transports', async () => {
      ...
      mockTransport.emit('connection', createConnectionInfo('conn-1', 'client-1'));
  ...
  describe('Connection Management', () => {
    ...
    it('should reject connections when limit reached', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        mockTransport.emit('connection', createConnectionInfo(`conn-${i}`, `client-${i}`));
      }

      expect(connectionManager.getConnections()).toHaveLength(5);

      // This should be rejected
      mockTransport.emit('connection', createConnectionInfo('conn-overflow', 'overflow-client'));
  ...
  describe('Message Handling', () => {
    beforeEach(async () => {
      ...
      mockTransport.emit('connection', createConnectionInfo('test-connection-1', 'test-client'));
    });
    ...
    it('should broadcast messages to all connections', async () => {
      ...
      mockTransport.emit('connection', createConnectionInfo('test-connection-2', 'test-client-2'));
