...
class PerformanceScorer {
  ...
  analyzePerformance(): number {
    const performanceFile = join(process.cwd(), 'test-output', 'performance', 'benchmark-results.json');
    ...
    try {
      const results = JSON.parse(readFileSync(performanceFile, 'utf8')) as PerformanceMetrics;
  ...
  private displayMetrics(metrics: PerformanceMetrics): void {
    ...
    console.log(`    Throughput: ${metrics.parsing.throughput.toFixed(2)} files/sec`);
    
    console.log(`  Querying:`);
    console.log(`    Average Time: ${metrics.querying.avgTime.toFixed(2)}ms`);
    console.log(`    Throughput: ${metrics.querying.throughput.toFixed(2)} queries/sec`);
    
    console.log(`  Indexing:`);
    console.log(`    Average Time: ${metrics.indexing.avgTime.toFixed(2)}ms`);
    console.log(`    Throughput: ${metrics.indexing.throughput.toFixed(2)} items/sec`);
  ...
  private calculateScore(metrics: PerformanceMetrics): number {
    ...
    if (metrics.parsing.avgTime > this.thresholds.parsing.maxAvgTime) {
      const penalty = Math.min(30, (metrics.parsing.avgTime - this.thresholds.parsing.maxAvgTime) / 10);
      score -= penalty;
    }
    
    if (metrics.parsing.throughput < this.thresholds.parsing.minThroughput) {
      const penalty = Math.min(20, (this.thresholds.parsing.minThroughput - metrics.parsing.throughput) * 2);
      score -= penalty;
    }
    
    // Querying performance
    if (metrics.querying.avgTime > this.thresholds.querying.maxAvgTime) {
      const penalty = Math.min(25, (metrics.querying.avgTime - this.thresholds.querying.maxAvgTime) / 5);
      score -= penalty;
    }
    
    if (metrics.querying.throughput < this.thresholds.querying.minThroughput) {
      const penalty = Math.min(15, (this.thresholds.querying.minThroughput - metrics.querying.throughput));
      score -= penalty;
    }
    
    // Indexing performance
    if (metrics.indexing.avgTime > this.thresholds.indexing.maxAvgTime) {
      const penalty = Math.min(20, (metrics.indexing.avgTime - this.thresholds.indexing.maxAvgTime) / 20);
      score -= penalty;
    }
    
    // Memory usage
    if (metrics.memory.peakUsage > this.thresholds.memory.maxPeakUsage) {
      const penalty = Math.min(15, (metrics.memory.peakUsage - this.thresholds.memory.maxPeakUsage) / 50);
      score -= penalty;
    }
    
    if (metrics.memory.avgUsage > this.thresholds.memory.maxAvgUsage) {
      const penalty = Math.min(10, (metrics.memory.avgUsage - this.thresholds.memory.maxAvgUsage) / 25);
      score -= penalty;
    }
  ...
  private generateReport(metrics: PerformanceMetrics, score: number): void {
    ...
    const checks = [
      ...
      },
      {
        ...
        passed: metrics.parsing.throughput >= this.thresholds.parsing.minThroughput,
      ...
      },
      {
        ...
        passed: metrics.querying.throughput >= this.thresholds.querying.minThroughput,
    ...
    checks.forEach(check => {
      ...
      console.log(`  ${status} ${check.name}: ${check.actual.toFixed(2)} ${check.unit}`);
    ...
    console.log(`${passed ? '✅' : '❌'} Performance check ${passed ? 'passed' : 'failed'} (threshold: ${threshold}/100)`);
  ...
  private generateHtmlReport(metrics: PerformanceMetrics, score: number, checks: any[]): void {
    const html = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    console.log('📄 HTML performance report generated: performance-report.html');
  }
}
