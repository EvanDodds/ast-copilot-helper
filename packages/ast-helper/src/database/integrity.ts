...
export class IntegrityValidator {
  ...
  async validateIntegrity(): Promise<IntegrityReport> {
    ...
    try {
      ...
      report.performanceMetrics.structureCheckTime =
        performance.now() - structureCheckStart;
      ...
      report.performanceMetrics.embeddingCheckTime =
        performance.now() - embeddingCheckStart;
      ...
      report.performanceMetrics.metadataCheckTime =
        performance.now() - metadataCheckStart;
      ...
      report.isValid =
        report.databaseIntegrity.isValid &&
      ...
      logger.info(
        `Integrity validation completed in ${report.duration.toFixed(2)}ms. Status: ${report.isValid ? "VALID" : "CORRUPTED"}`,
      );
   ...
   */
  private async validateDatabaseStructure(): Promise<
    IntegrityReport["databaseIntegrity"]
  > {
    try {
   ...
   */
  private async validateEmbeddingFiles(): Promise<
    IntegrityReport["embeddingIntegrity"]
  > {
    try {
      ...
      try {
        ...
        const embeddingFiles = files.filter(
          (f) => f.endsWith(".json") || f.endsWith(".bin"),
        );
        ...
        for (const file of embeddingFiles) {
          try {
            ...
            logger.warn(
              `Failed to validate embedding file ${file}: ${fileError instanceof Error ? fileError.message : String(fileError)}`,
            );
            result.corruptedFiles.push(file);
            result.isValid = false;
          }
   ...
   */
  private async validateMetadataIntegrity(): Promise<
    IntegrityReport["metadataIntegrity"]
  > {
    try {
      const structure = this.dbManager.getDatabaseStructure();
      const result = {
      ...
      try {
        ...
        const metadataFiles = files.filter(
          (f) => f.includes("metadata") && f.endsWith(".json"),
        );
        ...
        for (const file of metadataFiles) {
          try {
            ...
            if (metadata.checksum) {
              const calculatedChecksum = this.calculateContentChecksum(
                JSON.stringify({
            ...
            logger.warn(
              `Failed to validate metadata file ${file}: ${fileError instanceof Error ? fileError.message : String(fileError)}`,
            );
            result.corruptedMetadata.push(file);
            result.isValid = false;
          }
   ...
   */
  private async validateFileConsistency(): Promise<
    IntegrityReport["fileConsistency"]
  > {
    try {
      const structure = this.dbManager.getDatabaseStructure();
      const result = {
      ...
      try {
        ...
        const embeddingFiles = new Set(
          files.filter((f) => f.endsWith(".json") && !f.includes("metadata")),
        );
        const metadataFiles = new Set(
          files.filter((f) => f.includes("metadata") && f.endsWith(".json")),
        );

        // Check for orphaned embedding files (without corresponding metadata)
        for (const embeddingFile of embeddingFiles) {
          const expectedMetadata = embeddingFile.replace(
            ".json",
            "_metadata.json",
          );
        ...
        for (const metadataFile of metadataFiles) {
          const expectedEmbedding = metadataFile.replace(
            "_metadata.json",
            ".json",
          );
  ...
  private generateRecoveryRecommendations(report: IntegrityReport): string[] {
    ...
    if (!report.embeddingIntegrity.isValid) {
      if (report.embeddingIntegrity.corruptedFiles.length > 0) {
        recommendations.push(
          `Regenerate corrupted embedding files: ${report.embeddingIntegrity.corruptedFiles.join(", ")}`,
        );
      }
    ...
    if (!report.metadataIntegrity.isValid) {
      if (report.metadataIntegrity.checksumMismatches > 0) {
        recommendations.push(
          `${report.metadataIntegrity.checksumMismatches} metadata checksum mismatches detected - recalculate checksums`,
        );
      }
    ...
    if (!report.fileConsistency.isValid) {
      if (report.fileConsistency.orphanedFiles > 0) {
        recommendations.push(
          `Clean up ${report.fileConsistency.orphanedFiles} orphaned embedding files`,
        );
      }
      if (report.fileConsistency.missingReferences > 0) {
        recommendations.push(
          `Fix ${report.fileConsistency.missingReferences} metadata files with missing embeddings`,
        );
      }
    }
    ...
    if (totalTime > 5000) {
      // More than 5 seconds
      recommendations.push(
        "Integrity check took longer than expected - consider file system optimization",
      );
    }
  ...
  async recoverFromCorruption(
    ...
    try {
      ...
      if (!report.metadataIntegrity.isValid && options.recalculateChecksums) {
        const checksumRecovery = await this.recoverCorruptedChecksums(
          report.metadataIntegrity.corruptedMetadata,
        );
        ...
        result.actionsPerformed.push(
          `Recalculated ${checksumRecovery.recovered} metadata checksums`,
        );
      }

      if (!report.embeddingIntegrity.isValid) {
        const embeddingRecovery = await this.recoverCorruptedEmbeddings(
          report.embeddingIntegrity.corruptedFiles,
        );
        result.recoveredItems += embeddingRecovery.recovered;
        result.unrecoverableItems += embeddingRecovery.unrecoverable;
        result.actionsPerformed.push(
          `Recovered ${embeddingRecovery.recovered} embedding files`,
        );
      }

      if (!report.fileConsistency.isValid) {
        const consistencyRecovery = await this.fixFileInconsistencies(
          report.fileConsistency,
        );
      ...
      logger.info(
        `Recovery completed in ${result.duration.toFixed(2)}ms. Success: ${result.success}`,
      );
      return result;
    } catch (error) {
      result.errors.push(
   ...
   */
  private async recoverCorruptedChecksums(
    corruptedMetadata: string[],
  ): Promise<{ recovered: number; unrecoverable: number }> {
    let recovered = 0;
    let unrecoverable = 0;
    ...
    for (const metadataFile of corruptedMetadata) {
      try {
        ...
        const newChecksum = this.calculateContentChecksum(
          JSON.stringify({
            model: metadata.model,
            dimensions: metadata.dimensions,
            count: metadata.count,
        ...
        logger.warn(
          `Failed to recover checksum for ${metadataFile}: ${error instanceof Error ? error.message : String(error)}`,
        );
        unrecoverable++;
      }
    }
   ...
   */
  private async recoverCorruptedEmbeddings(
    corruptedFiles: string[],
  ): Promise<{ recovered: number; unrecoverable: number }> {
    let recovered = 0;
    let unrecoverable = 0;
    ...
    for (const file of corruptedFiles) {
      try {
        ...
        logger.error(
          `Failed to recover embedding file ${file}: ${error instanceof Error ? error.message : String(error)}`,
        );
        unrecoverable++;
      }
    }
   ...
   */
  private async fixFileInconsistencies(
    consistency: IntegrityReport["fileConsistency"],
  ): Promise<{ recovered: number; unrecoverable: number }> {
    let recovered = 0;
    let unrecoverable = 0;
    ...
    try {
      ...
      logger.error(
        `Failed to fix file inconsistencies: ${error instanceof Error ? error.message : String(error)}`,
      );
 ...
 */
export function createIntegrityValidator(
