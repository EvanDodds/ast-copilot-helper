 ...
 */
export class PerformanceMonitorCommandHandler implements CommandHandler<PerformanceMonitorOptions> {
  private logger = createLogger();
  private isMonitoring = false;
  private monitoringData: any[] = [];

  async execute(options: PerformanceMonitorOptions, _config: Config): Promise<void> {
    const duration = options.duration || 60;
    const interval = options.interval || 1000;
    
    try {
      this.logger.info(`📈 Starting performance monitoring for ${duration} seconds...`);
      ...
      this.logger.info(`✅ Monitoring completed in ${(actualDuration / 1000).toFixed(1)}s`);
      this.logger.info(`📊 Collected ${this.monitoringData.length} samples`);
      
    } catch (error: any) {
      this.logger.error('Performance monitoring failed:', { error: error.message || error });
      throw error;
    }
  }
  
  private async startMonitoringLoop(interval: number, endTime: number): Promise<void> {
  ...
  private outputLiveSample(sample: any): void {
    ...
    console.log(`[${time}] Memory: ${sample.memory.heapUsed}MB heap, ${sample.memory.rss}MB RSS | CPU: ${(sample.cpu.user / 1000).toFixed(1)}ms user`);
  }
  
  private async outputResults(options: PerformanceMonitorOptions): Promise<void> {
    ...
    const summary = {
      ...
      duration: this.monitoringData.length > 0 ? 
        this.monitoringData[this.monitoringData.length - 1].timestamp - this.monitoringData[0].timestamp : 0,
    };
    ...
    console.log(`Memory usage: ${summary.memory.min}-${summary.memory.max}MB (avg: ${summary.memory.avg}MB)`);
