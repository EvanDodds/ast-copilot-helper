...
class QueryPreprocessor {
  ...
  static normalizeQueryText(text: string): string {
    ...
    const filteredWords = words.filter(
      (word) => word.length >= 2 && !codeStopWords.includes(word),
    );
...
export class MainQueryProcessor implements QueryProcessor {
  ...
  async processQuery(query: MCPQuery): Promise<QueryResponse> {
    ...
    try {
      ...
      const enhancedText = QueryPreprocessor.enhanceWithContext(
        preprocessedText,
        query.context,
      );
      ...
      switch (query.type) {
        case "semantic":
          response = await this.processSemanticQuery(
            enhancedText,
          ...
          break;
        case "signature":
          response = await this.processSignatureQuery(
            enhancedText,
            query.options as SignatureQueryOptions,
            query.maxResults,
          );
          break;
        case "file":
          response = await this.processFileQuery(
            enhancedText,
            query.options as FileQueryOptions,
            query.maxResults,
          );
          break;
        case "contextual":
      ...
      this.stats.errorRate =
        (this.stats.errorRate * (this.stats.totalQueries - 1) + 1) /
        this.stats.totalQueries;
  ...
  async processFileQuery(
    ...
    try {
      ...
      const { matches, performance } =
        await this.fileProcessor.processQuery(fileQuery);
      ...
      await this.responseAssembler.assembleResponse(queryResponse, mcpQuery);
  ...
  async processQueryWithFormatting(
    ...
    return this.responseAssembler.assembleResponse(
      response,
      query,
      requestId,
      page,
  ...
  private updatePerformanceMetrics(queryTime: number): void {
    ...
    this.stats.performanceMetrics.p50QueryTime =
      sortedTimes[Math.floor(count * 0.5)] ?? 0;
    this.stats.performanceMetrics.p95QueryTime =
      sortedTimes[Math.floor(count * 0.95)] ?? 0;
    this.stats.performanceMetrics.p99QueryTime =
      sortedTimes[Math.floor(count * 0.99)] ?? 0;
  }
