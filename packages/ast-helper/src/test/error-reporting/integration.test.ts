...
describe('Error Reporting System Integration', () => {
  ...
  describe('Complete System Workflow', () => {
    it('should handle complete error reporting workflow with privacy controls', async () => {
      ...
      const testErrors = [
        new Error('Parse error in user@example.com file with sensitive data 555-123-4567'),
      ...
      const transmissionResult = await errorManager.sendSecureErrorReport(reports[0], userId);
    ...
    it('should handle privacy consent changes', async () => {
      ...
      await errorManager.setUserConsent(userId, consentData);
      expect(errorManager.getUserConsent(userId)?.level).toBe(ConsentLevel.BASIC);
      ...
      await errorManager.setUserConsent(userId, consentData);
      expect(errorManager.getUserConsent(userId)?.level).toBe(ConsentLevel.ANALYTICS);
    ...
    it('should handle diagnostic data collection', async () => {
      ...
      const diagnosticReport = await errorManager.generateErrorReport(diagnosticError, {
        userId,
      ...
      if (diagnosticReport.context.parameters) {
        ...
        expect(diagnosticReport.context.parameters.operation).toBe('diagnostic-test');
      }
    });
    ...
    it('should handle concurrent error reporting', async () => {
      ...
      console.log(`✅ Concurrent success rate: ${(successful.length / results.length * 100).toFixed(1)}%`);
    });
    ...
    it('should handle error export and diagnostics', async () => {
      ...
      for (const error of errors) {
        const report = await errorManager.generateErrorReport(error, { userId });
