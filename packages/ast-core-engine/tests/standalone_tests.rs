...
async fn test_storage_layer_creation() {
    ...
    assert!(storage_result.is_ok(), "Should create storage layer successfully");
...
async fn test_vector_database_creation() {
    ...
    assert!(result.is_ok(), "Should initialize vector database successfully");
    ...
    assert_eq!(search_results[0].node_id, "test_node", "Should find the correct node");
...
async fn test_batch_processor() {
    ...
    let storage_layer = Arc::new(ast_core_engine::storage::StorageLayer::new(storage_config).await.unwrap());
    
    // Create batch processor with required arguments
    let processor = ast_core_engine::batch_processor::BatchProcessor::new(ast_processor, storage_layer, batch_config);
...
async fn test_error_handling() {
    ...
    let vector_error = ast_core_engine::error::EngineError::VectorDbError("Test vector error".to_string());
    assert!(matches!(vector_error, ast_core_engine::error::EngineError::VectorDbError(_)));
    
    let config_error = ast_core_engine::error::EngineError::Configuration("Test config error".to_string());
    assert!(matches!(config_error, ast_core_engine::error::EngineError::Configuration(_)));
