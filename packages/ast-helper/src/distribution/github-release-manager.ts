...
export class GitHubReleaseManager implements Publisher {
  ...
  private loadCredentials(): void {
    ...
    this.credentials.token =
      process.env.GITHUB_TOKEN || this.findReleaseToken();
  ...
  private async verifyGitHubAccess(): Promise<void> {
    try {
      const response = await this.makeGitHubApiRequest(
        `/repos/${this.credentials.owner}/${this.credentials.repo}`,
      );

      if (!response.permissions?.push) {
        throw new Error(
          "GitHub token does not have push permissions to repository",
        );
      }

      console.log(
        `✅ GitHub API access verified for ${this.credentials.owner}/${this.credentials.repo}`,
      );
    } catch (error) {
  ...
  async validate(): Promise<ValidationResult> {
    ...
    if (!this.config) {
      addError(
        ...
        "GitHub Release Manager not initialized with config",
      );
  ...
  private validateCredentials(
    ...
    if (!this.credentials.token) {
      addError(
        ...
        "GitHub personal access token is required (GITHUB_TOKEN environment variable)",
      );
    }
    ...
    if (!this.credentials.owner) {
      addError(
        "MISSING_REPO_OWNER",
        "Repository owner could not be determined from git remote",
      );
    }
    ...
    if (!this.credentials.repo) {
      addError(
        "MISSING_REPO_NAME",
        "Repository name could not be determined from git remote",
      );
    }
  }
  ...
  private async validateRepositoryAccess(
    ...
    try {
      const repo = await this.makeGitHubApiRequest(
        `/repos/${this.credentials.owner}/${this.credentials.repo}`,
      );
      ...
      if (!repo.permissions?.push) {
        addError(
          ...
          "GitHub token lacks push permissions for releases",
        );
      }
      ...
      if (repo.archived) {
        addError(
          "REPO_ARCHIVED",
          "Cannot create releases for archived repository",
        );
      }
  ...
  private validateReleaseConfig(
    ...
    if (this.config!.version && !this.isValidSemVer(this.config!.version)) {
      addWarning(
        "INVALID_SEMVER",
        `Version "${this.config!.version}" is not valid semantic versioning`,
      );
    }
    ...
    try {
      ...
      if (tagExists) {
        addError(
          "TAG_EXISTS",
          `Git tag v${this.config!.version} already exists`,
        );
      }
    } catch (error) {
      addWarning(
        "TAG_CHECK_FAILED",
        `Could not verify tag existence: ${error}`,
      );
    }
  }
  ...
  private async validateReleaseAssets(
    ...
    const commonAssets = ["CHANGELOG.md", "README.md", "LICENSE"];
    ...
    for (const assetName of commonAssets) {
      try {
        ...
        addWarning(
          "MISSING_COMMON_ASSET",
          `Common release asset missing: ${assetName}`,
        );
      }
    }
  }
  ...
  private async extractFromChangelog(
    version: string,
  ...
  private async publishReleaseIfReady(
    release: GitHubRelease,
  ): Promise<GitHubRelease> {
    const releaseConfig = this.getReleaseConfig();
  ...
  async verify(result: GitHubReleaseResult): Promise<VerificationResult> {
    ...
    if (result.assets.length > 0 && result.releaseId !== null) {
      const assetCheck = await this.verifyReleaseAssets(
        result.releaseId,
        result.assets,
      );
  ...
  private async verifyReleaseAssets(
    releaseId: number,
    expectedAssets: Array<any>,
  ): Promise<VerificationCheck> {
    const startTime = Date.now();
  ...
  private async makeGitHubApiRequest(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `GitHub API request failed: ${response.statusText} - ${error}`,
      );
    }
