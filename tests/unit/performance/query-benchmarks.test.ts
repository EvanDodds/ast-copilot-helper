  ...
  describe("QueryBenchmarkRunner", () => {
    ...
    describe("Error Handling", () => {
      ...
      it("should provide meaningful error messages", async () => {
        ...
        runner["simulateQuery"] = vi
          .fn()
          .mockRejectedValue(new Error("Query simulation failed"));
      ...
      it("should calculate performance scores accurately", async () => {
        ...
        if (result.successfulRuns > 0) {
          ...
          const expectedMinScore =
            (result.successfulRuns / result.totalRuns) * 100 * 0.3; // At least 30% of success contribution
          expect(result.performanceScore).toBeGreaterThanOrEqual(
            expectedMinScore,
          );
        }
      }, 8000);
    ...
    describe("Test Data Generation", () => {
      ...
      it("should scale test data based on node count", async () => {
        ...
        expect(largeResult.totalNodesProcessed).toBeGreaterThan(
          smallResult.totalNodesProcessed,
        );
      }, 10000);
