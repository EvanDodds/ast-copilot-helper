...
class VSCodeExtensionIntegrationTestSuite {
  ...
  async setup(): Promise<void> {
    ...
    (
      this.mockVSCode.commands.registerCommand as MockedFunction<any>
  ...
  async testExtensionActivation(): Promise<void> {
    ...
    expect(this.mockVSCode.window.createOutputChannel).toHaveBeenCalledWith(
      "AST MCP Helper",
    );
  ...
  async testCommandExecution(): Promise<void> {
    ...
    const startServerHandler = this.registeredCommands.get(
      "astCopilotHelper.startServer",
    );
    ...
    const showStatusHandler = this.registeredCommands.get(
      "astCopilotHelper.showStatus",
    );
    ...
    const openSettingsHandler = this.registeredCommands.get(
      "astCopilotHelper.openSettings",
    );
  ...
  async testConfigurationManagement(): Promise<void> {
    ...
    const config =
      this.mockVSCode.workspace.getConfiguration("astCopilotHelper");
    ...
    expect(
      this.mockVSCode.workspace.onDidChangeConfiguration,
    ).toHaveBeenCalled();
  ...
  async testWorkspaceIntegration(): Promise<void> {
    ...
    expect(Array.isArray(this.mockVSCode.workspace.workspaceFolders)).toBe(
      true,
    );

    const parseWorkspaceHandler = this.registeredCommands.get(
      "astCopilotHelper.parseWorkspace",
    );
    ...
    const testErrors = [
      {
        ...
        errorType: "Server start failure",
      },
      {
        command: "astCopilotHelper.parseWorkspace",
        errorType: "Workspace parsing failure",
      },
      {
        command: "astCopilotHelper.analyzeIssue",
        errorType: "GitHub API failure",
      },
    ];
