...
describe('TreeSitterGrammarManager', () => {
  ...
  describe('downloadGrammar', () => {
    it('should download and cache a grammar successfully', async () => {
      ...
      // Verify file exists
      const fileExists = await fs.access(grammarPath).then(() => true).catch(() => false);
      expect(fileExists).toBe(true);

      // Verify metadata exists
      const metadataPath = path.join(path.dirname(grammarPath), 'metadata.json');
  ...
  describe('getCachedGrammarPath', () => {
    ...
    it('should download grammar if not cached', async () => {
      const grammarPath = await grammarManager.getCachedGrammarPath('typescript');
  ...
  describe('loadParser', () => {
    it('should ensure grammar is available but not yet implement parser loading', async () => {
      ...
      ).rejects.toThrow('Parser loading not yet implemented');
      
      // But should have downloaded the grammar
      const grammarPath = path.join(testBaseDir, 'grammars', 'typescript', 'tree-sitter-typescript.wasm');
      const fileExists = await fs.access(grammarPath).then(() => true).catch(() => false);
      expect(fileExists).toBe(true);
    });
  });
