...
describe("ComprehensiveErrorReportingManager", () => {
  ...
  describe("initialization", () => {
    test("should initialize with default configuration", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "🔧 Initializing comprehensive error reporting system...",
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        "✅ Error reporting system initialized successfully",
      );
  ...
  describe("error report generation", () => {
    ...
    test("should generate error report with context", async () => {
      ...
      const errorReport = await errorManager.generateErrorReport(
        testError,
        context,
      );
    ...
    test("should categorize errors correctly", async () => {
      ...
      const networkReport =
        await errorManager.generateErrorReport(networkError);
      const fileReport = await errorManager.generateErrorReport(fileError);
      const permissionReport =
        await errorManager.generateErrorReport(permissionError);
    ...
    test("should determine error severity correctly", async () => {
      ...
      const criticalReport =
        await errorManager.generateErrorReport(criticalError);
      ...
      const warningReport =
        await errorManager.generateErrorReport(warningError);
  ...
  describe("error reporting", () => {
    ...
    test("should report error successfully", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "📊 Reporting error: error - test-error",
      );
  ...
  describe("diagnostic collection", () => {
    ...
    test("should collect diagnostic data", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "🔍 Collecting comprehensive diagnostic data...",
      );
  ...
  describe("suggestions", () => {
    ...
    test("should provide suggestions for error", async () => {
      ...
      const suggestions =
        await errorManager.provideSuggestions(mockErrorReport);

      expect(Array.isArray(suggestions)).toBe(true);
      expect(consoleSpy).toHaveBeenCalledWith(
        "🧠 Generating suggestions for error: test-error",
      );
