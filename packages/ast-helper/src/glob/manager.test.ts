...
describe("GlobManager", () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      join(tempDir, "node_modules", "package", "index.js"),
      "module.exports = {}",
    );
  });
  ...
  describe("pattern matching", () => {
    ...
    it("should match wildcard patterns", () => {
      ...
      expect(globManager.match(["src/*.ts"], "tests/example.test.js")).toBe(
        false,
      );
    });
    ...
    it("should handle brace expansion", () => {
      ...
      expect(
        globManager.match(["**/*.{ts,js}"], "src/components/Button.tsx"),
      ).toBe(false);
    });

    it("should handle negation patterns", () => {
      expect(
        globManager.match(["**/*", "!node_modules/**"], "src/index.ts"),
      ).toBe(true);
      expect(
        globManager.match(
          ["**/*", "!node_modules/**"],
  ...
  describe("pattern expansion", () => {
    ...
    it("should handle brace expansion in file expansion", async () => {
      const result = await globManager.expandPatterns(
        ["src/*.{ts,js}"],
        tempDir,
      );
  ...
  describe("options and configuration", () => {
    ...
    it("should create new instance with different options", () => {
      ...
      const newManager = globManager.withOptions({
        caseSensitive: !originalCaseSensitive,
      });

      expect(newManager.getOptions().caseSensitive).toBe(
        !originalCaseSensitive,
      );
      expect(globManager.getOptions().caseSensitive).toBe(
        originalCaseSensitive,
      ); // Original unchanged
    });
  });
