...
import { WebSocketTransport, WebSocketTransportConfig } from '../../../packages/ast-mcp-server/src/mcp/transport/websocket';
...
class TransportUtils {
  static validateConfig(config: any): { valid: boolean; error?: string } {
    ...
    if ('input' in config && 'url' in config) {
      return { valid: false, error: 'Cannot specify both input/output streams and WebSocket URL' };
    }
...
describe('Transport Integration Tests', () => {
  ...
  describe('TransportFactory', () => {
    ...
    it('should create STDIO transport with custom config', () => {
      ...
      expect(transport).toBeInstanceOf(StdioTransport);
      expect((transport as StdioTransport).supportsMultipleConnections()).toBe(false);
    });
    ...
    it('should create WebSocket transport with custom config', () => {
      ...
      expect(transport).toBeInstanceOf(WebSocketTransport);
      expect((transport as WebSocketTransport).supportsMultipleConnections()).toBe(true);
    });
  ...
  describe('TransportUtils', () => {
    describe('validateConfig', () => {
      ...
      it('should reject mixed stream and URL config', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.error).toBe('Cannot specify both input/output streams and WebSocket URL');
      });
    });
    ...
    describe('getTransportType', () => {
      ...
      it('should detect websocket transport by URL', () => {
        const result = TransportUtils.getTransportType({ url: 'ws://localhost:8080' });
  ...
  describe('Transport Cross-compatibility', () => {
    it('should have consistent API across transport types', () => {
      ...
      expect((stdioTransport as StdioTransport).supportsMultipleConnections).toBeTypeOf('function');
      expect((websocketTransport as WebSocketTransport).supportsMultipleConnections).toBeTypeOf('function');
    });
