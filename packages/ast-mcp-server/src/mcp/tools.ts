...
import type { DatabaseReader, QueryOptions } from "../types.js";
...
export class IntentQueryHandler extends BaseHandler {
  ...
  async handle(request: JSONRPCRequest): Promise<JSONRPCResponse> {
    try {
      ...
      const matches = await this.db.queryByIntent(
        processedParams.intent,
        queryOptions,
      );
            ...
            text: JSON.stringify(
              {
                ...
                matches: matches.map((match) => ({
                  ...
                  snippet:
                    match.sourceSnippet?.substring(0, 200) +
    ...
    try {
      ...
      const responseData: any = {
        node: {
          ...
          sourceSnippet: processedParams.includeText
            ? node.sourceSnippet
            : undefined,
...
export class FileQueryHandler extends BaseHandler {
    ...
    try {
      ...
      if (processedParams.nodeTypes && processedParams.nodeTypes.length > 0) {
        filteredNodes = nodes.filter((node) =>
          processedParams.nodeTypes.includes(node.nodeType),
        );
      }
  ...
  static getDefinition(): MCPToolDefinition {
    return {
      name: "ast_file_query",
      description:
        "Retrieve all AST nodes for a specific file, with optional filtering by node type and depth.",
      inputSchema: {
        type: "object",
        properties: {
          filePath: {
          ...
          nodeTypes: {
            ...
            items: { type: "string" },
            description:
              "Optional array of node types to filter by (e.g., ['FunctionDeclaration', 'ClassDeclaration'])",
            default: [],
          },
          maxDepth: {
            type: "number",
            description:
              "Maximum depth level of nodes to include (-1 for no limit, default: -1)",
            minimum: -1,
            default: -1,
          },
          includeText: {
            type: "boolean",
            description:
              "Include the full text content of nodes (default: false)",
            default: false,
          },
        },
        required: ["filePath"],
...
export class TextSearchHandler extends BaseHandler {
  ...
  async handle(request: JSONRPCRequest): Promise<JSONRPCResponse> {
    try {
      ...
      const matches = await this.db.searchNodes(
            ...
            text: JSON.stringify(
              {
                ...
                matches: matches.map((match) => ({
                  ...
                  matchReason: match.matchReason,
                  matchedText:
                    match.sourceSnippet?.substring(0, 300) +
                    (match.sourceSnippet && match.sourceSnippet.length > 300
                      ? "..."
                      : ""),
                })),
              },
  ...
  static getDefinition(): MCPToolDefinition {
    return {
      ...
      description:
        "Search for text patterns across all AST nodes, with support for case sensitivity and file filtering.",
      inputSchema: {
        type: "object",
        properties: {
          query: {
          ...
          includeContext: {
            type: "boolean",
            description:
              "Include surrounding context in results (default: true)",
            default: true,
          },
          filePattern: {
            type: "string",
            description:
              "Optional glob pattern to filter files (e.g., '**/*.ts', 'src/**')",
            default: null,
          },
        },
        required: ["query"],
...
export class IndexStatusHandler extends BaseHandler {
  ...
  static getDefinition(): MCPToolDefinition {
    return {
      ...
      description:
        "Get information about the AST database index status, including node count, file count, and last update time.",
      inputSchema: {
        type: "object",
        properties: {},
        required: [],
...
export class RecentChangesHandler extends BaseHandler {
  ...
  async handle(request: JSONRPCRequest): Promise<JSONRPCResponse> {
    try {
      ...
      const recentNodes = await this.db.getRecentChanges(
        processedParams.since,
        queryOptions,
      );
