...
import { PrivacyManager, ConsentLevel } from '../../error-reporting/privacy/privacy-manager.js';
...
import type { ConsentData, SecurityConfig } from '../../error-reporting/types.js';
...
describe('Privacy and Security Controls', () => {
  ...
  describe('Privacy Manager', () => {
    ...
    it('should check consent correctly', async () => {
      ...
      expect(privacyManager.hasConsent(userId, 'error', ConsentLevel.BASIC)).toBe(true);
      
      // Should not have consent for analytics level
      expect(privacyManager.hasConsent(userId, 'error', ConsentLevel.ANALYTICS)).toBe(false);
      
      // Should not have consent for uncategorized data
      expect(privacyManager.hasConsent(userId, 'performance', ConsentLevel.BASIC)).toBe(false);
    });
    ...
    it('should filter simple error data with PII scrubbing', async () => {
      const mockError = {
        ...
        severity: 'medium',
        category: 'error',
        message: 'Test error with email test@example.com and phone 555-123-4567',
        operation: 'test',
        suggestions: [],
        userProvided: false,
  ...
  describe('Secure Transmission Manager', () => {
    ...
    it('should test connection to endpoint', async () => {
      const result = await transmissionManager.testConnection('https://api.example.com/errors');
    ...
    it('should respect rate limiting', async () => {
      ...
      const promises = Array(15).fill(null).map(() => 
        transmissionManager.sendErrorReport(mockError, 'https://api.example.com/errors')
      );
  ...
  describe('Privacy System Integration', () => {
    it('should work together for complete privacy protection', async () => {
      ...
      const filteredReport = await privacyManager.filterErrorReport(mockError, userId);
