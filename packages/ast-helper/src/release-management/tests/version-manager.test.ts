...
describe('VersionManagerImpl', () => {
  ...
  describe('version calculation', () => {
    test('should calculate next patch version', async () => {
      const nextVersion = await versionManager.calculateNextVersion('1.0.0', ReleaseType.PATCH);
    ...
    test('should calculate next major version', async () => {
      const nextVersion = await versionManager.calculateNextVersion('1.0.0', ReleaseType.MAJOR);
      expect(nextVersion).toBe('2.0.0');
    });

    test('should calculate prerelease version', async () => {
      const nextVersion = await versionManager.calculateNextVersion('1.0.0', ReleaseType.PRERELEASE);
  ...
  describe('version validation', () => {
    test('should validate semantic version format', async () => {
      const isValid = await versionManager.validateVersion('1.2.3', ReleaseType.PATCH);
      expect(isValid).toBe(true);
    });

    test('should validate prerelease version format', async () => {
      const isValid = await versionManager.validateVersion('1.2.3-beta.1', ReleaseType.PRERELEASE);
      expect(isValid).toBe(true);
    });

    test('should reject invalid version format', async () => {
      const isValid = await versionManager.validateVersion('1.2', ReleaseType.PATCH);
      expect(isValid).toBe(false);
    });

    test('should reject version type mismatch', async () => {
      const isValid = await versionManager.validateVersion('1.2.3-beta.1', ReleaseType.PATCH);
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('version comparison', () => {
    ...
    test('should handle prerelease comparison', () => {
      expect(versionManager.compareVersions('1.0.0-alpha.1', '1.0.0-alpha.2')).toBe(-1);
  ...
  describe('configuration validation', () => {
    test('should handle invalid scheme', async () => {
      ...
      await expect(versionManager.initialize(invalidConfig))
        .rejects.toThrow('Unsupported versioning scheme');
    });
    ...
    test('should handle malformed initial version', async () => {
      ...
      await expect(versionManager.initialize(invalidConfig))
        .rejects.toThrow('Invalid initial version format');
    });
  });
});