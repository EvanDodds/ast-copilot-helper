...
import { DEFAULT_PERFORMANCE_CONFIG, DEFAULT_PERFORMANCE_BENCHMARKS } from '../../packages/ast-helper/src/performance-validation/config.js';
import { validatePerformance, quickBenchmark } from '../../packages/ast-helper/src/performance-validation/index.js';
import type { PerformanceBenchmark, PerformanceConfig } from '../../packages/ast-helper/src/performance-validation/types.js';
...
describe('Performance Validation System', () => {
  ...
  describe('Performance Configuration', () => {
    it('should have default benchmarks', () => {
      ...
      for (const benchmark of DEFAULT_PERFORMANCE_BENCHMARKS) {
        ...
        expect(['cli', 'mcp-server', 'vscode-extension', 'memory', 'disk-io', 'network']).toContain(benchmark.category);
    ...
    it('should have default configuration', () => {
      ...
      expect(Array.isArray(DEFAULT_PERFORMANCE_CONFIG.scalabilityTests)).toBe(true);
  ...
  describe('Benchmark Categories', () => {
    it('should test CLI performance benchmarks', async () => {
      const cliBenchmarks = DEFAULT_PERFORMANCE_BENCHMARKS.filter(b => b.category === 'cli');
    ...
    it('should test MCP server performance benchmarks', async () => {
      const mcpBenchmarks = DEFAULT_PERFORMANCE_BENCHMARKS.filter(b => b.category === 'mcp-server');
      ...
      for (const result of results) {
        expect(result.measurement.value).toBeGreaterThan(0);
        expect(['ms', 'connections', 'mb'].includes(result.measurement.unit)).toBe(true);
      }
    });

    it('should test memory benchmarks', async () => {
      const memoryBenchmarks = DEFAULT_PERFORMANCE_BENCHMARKS.filter(b => b.category === 'memory');
  ...
  describe('Benchmark Evaluation', () => {
    it('should correctly evaluate passing benchmarks', async () => {
      ...
      expect(Math.abs(result.deviation)).toBeLessThanOrEqual(passingBenchmark.tolerance);
    });
  ...
  describe('Event Emission', () => {
    it('should emit events during benchmark execution', async () => {
      ...
      runner.on('benchmarks-complete', () => events.push('benchmarks-complete'));
  ...
  describe('Performance Targets', () => {
    it('should have reasonable production targets', () => {
      ...
      const cliStartupBenchmark = cliBenchmarks.find(b => b.name === 'cli-startup-time');
      ...
      const mcpQueryBenchmark = mcpBenchmarks.find(b => b.name === 'mcp-query-response');
