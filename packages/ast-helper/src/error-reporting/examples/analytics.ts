  ...
  try {
    ...
    const errorScenarios = [
      {
        ...
        error: () => {
          const error = new Error(
            `API request timeout after ${5000 + Math.random() * 3000}ms`,
          );
      ...
      },
      {
        ...
        error: () => {
          const error = new Error(
            `Unexpected token at line ${Math.floor(Math.random() * 100) + 1}`,
          );
    ...
    Object.entries(timeRanges).forEach(([label, milliseconds]) => {
      ...
      const recentErrors = errorHistory.filter(
        (entry) => entry.error.timestamp > cutoff,
      );
    ...
    const performanceErrors = errorHistory.filter(
      (entry) =>
    ...
    if (performanceErrors.length > 0) {
      ...
      const patterns = performanceErrors.reduce(
        (acc, entry) => {
    ...
    if (diagnosticsData.environment) {
      ...
      if (diagnosticsData.environment.uptime) {
        const uptimeHours = Math.round(
          diagnosticsData.environment.uptime / 3600,
        );
    ...
    const recentErrors = errorHistory
      ...
      .sort(
        (a, b) => a.error.timestamp.getTime() - b.error.timestamp.getTime(),
      );

    if (recentErrors.length >= 10) {
      const firstHalf = recentErrors.slice(
        0,
        Math.floor(recentErrors.length / 2),
      );
      const secondHalf = recentErrors.slice(
        Math.floor(recentErrors.length / 2),
      );
...
async function customMetricsExample() {
  ...
  for (const errorData of customMetricErrors) {
    ...
    console.log(
      `   Metrics: ${Object.keys(errorData.context.customMetrics).join(", ")}`,
    );
  }
