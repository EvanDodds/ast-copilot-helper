...
describe("MarketplacePublisher", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(fsMocks.readFile).mockResolvedValue(
      JSON.stringify(
        {
  ...
  describe("initialization", () => {
    ...
    it("should throw error if vsce is missing", async () => {
      ...
      await expect(publisher.initialize(testConfig)).rejects.toThrow(
        "vsce (Visual Studio Code Extension Manager) is not installed",
      );
    });
  });
  ...
  describe("validation", () => {
    ...
    it("should report errors for missing required fields", async () => {
      ...
      vi.mocked(fsMocks.readFile).mockResolvedValueOnce(
      ...
      expect(
        result.errors.some((e: any) => e.code === "MISSING_DISPLAY_NAME"),
      ).toBe(true);
      expect(
        result.errors.some((e: any) => e.code === "MISSING_PUBLISHER"),
      ).toBe(true);
      expect(
        result.errors.some((e: any) => e.code === "MISSING_VSCODE_ENGINE"),
      ).toBe(true);
    });
    ...
    it("should report warnings for missing recommended fields", async () => {
      ...
      expect(
        result.warnings.some((w: any) => w.code === "MISSING_CATEGORIES"),
      ).toBe(true);
      expect(
        result.warnings.some((w: any) => w.code === "MISSING_KEYWORDS"),
      ).toBe(true);
      expect(result.warnings.some((w: any) => w.code === "MISSING_ICON")).toBe(
        true,
      );
    });
    ...
    it("should report errors for missing files", async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e: any) => e.code === "MAIN_FILE_NOT_FOUND"),
      ).toBe(true);
    });
    ...
    it("should report errors for missing credentials", async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e: any) => e.code === "MISSING_VSCODE_TOKEN"),
      ).toBe(true);
      expect(
        result.errors.some((e: any) => e.code === "MISSING_VSCODE_PUBLISHER"),
      ).toBe(true);
    });
    ...
    it("should handle no VS Code packages", async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors.some((e: any) => e.code === "NO_VSCODE_PACKAGES"),
      ).toBe(true);
    });
  });
  ...
  describe("publishing", () => {
    ...
    it("should publish successfully to VS Code Marketplace", async () => {
      ...
      mockExecSync
        ...
        .mockReturnValueOnce(
          "Published test-publisher.test-vscode-extension@2.0.0",
        ); // vsce publish
      ...
      try {
        ...
        expect(result.packages.some((p: any) => p.registry === "vscode")).toBe(
          true,
        );
      } finally {
    ...
    it("should publish to both marketplaces when credentials available", async () => {
      ...
      try {
        ...
        expect(result.packages.some((p: any) => p.registry === "openvsx")).toBe(
          true,
        );
      } finally {
        // Restore environment variables
      ...
      try {
        ...
        expect(
          result.packages.some(
            (p: any) =>
              !p.success &&
              p.error?.includes("VS Code Marketplace publish failed"),
          ),
        ).toBe(true);
      } finally {
        // Restore environment variables
    ...
    it("should handle publish failures", async () => {
      ...
      try {
        ...
        expect(result.packages[0].error).toContain(
  ...
  describe("verification", () => {
    ...
    it("should verify successful VS Code Marketplace publications", async () => {
      ...
      mockExecSync.mockReturnValue(
        "Extension: test-publisher.test-vscode-extension\nVersion: 2.0.0\nPublisher: test-publisher",
      );
