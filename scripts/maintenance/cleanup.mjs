...
class RepositoryCleanup {
  ...
  async runCleanup() {
    ...
    try {
      const tasks = [
        {
          name: "build",
          fn: this.cleanBuildArtifacts.bind(this),
          description: "Remove build artifacts and dist files",
        },
        {
          name: "deps",
          fn: this.cleanDependencies.bind(this),
          description: "Clean and optimize dependencies",
        },
        {
          name: "cache",
          fn: this.cleanCaches.bind(this),
          description: "Clear various cache directories",
        },
        {
          name: "logs",
          fn: this.cleanLogs.bind(this),
          description: "Remove old log files",
        },
        {
          name: "git",
          fn: this.cleanGitHistory.bind(this),
          description: "Optimize git repository",
        },
        {
          name: "temp",
          fn: this.cleanTempFiles.bind(this),
          description: "Remove temporary files",
        },
        {
          name: "optimize",
          fn: this.optimizeFiles.bind(this),
          description: "Optimize file formats and remove duplicates",
        },
      ];
  ...
  async cleanBuildArtifacts() {
    ...
    console.log(
      `  ✅ Removed ${result.filesRemoved} build artifacts (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  async cleanDependencies() {
    ...
    console.log(
      `  ✅ Cleaned ${result.filesRemoved} dependency items (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  async cleanCaches() {
    ...
    for (const pattern of cachePatterns) {
      try {
        ...
        if (this.verbose) {
          console.log(
            `Warning: Could not process cache pattern ${pattern}:`,
            error.message,
          );
        }
      }
    ...
    const osCachePatterns = [".DS_Store", "Thumbs.db", "desktop.ini"];
    ...
    console.log(
      `  ✅ Removed ${result.filesRemoved} cache items (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  async cleanLogs() {
    ...
    for (const pattern of logPatterns) {
      try {
        ...
        for (const file of files) {
          ...
          if (
            this.isFileOld(file, 7) &&
            this.shouldDelete(file, "old log file")
          ) {
            const size = this.getFileSize(file);
            this.deleteFileOrDir(file);
            result.filesRemoved++;
        ...
        if (this.verbose) {
          console.log(
            `Warning: Could not process log pattern ${pattern}:`,
            error.message,
          );
        }
      }
    ...
    console.log(
      `  ✅ Removed ${result.filesRemoved} old log files (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  async cleanGitHistory() {
    ...
    try {
        ...
        if (currentBranch === "main" || currentBranch === "master") {
          ...
          if (!this.dryRun) {
            ...
            const mergedBranches = execSync(
              `git branch --merged | grep -v "\\*\\|main\\|master\\|develop" | tr -d ' '`,
              {
                cwd: ROOT_DIR,
                encoding: "utf-8",
              },
            ...
            for (const branch of mergedBranches) {
              if (
                branch.trim() &&
                this.shouldDelete(`branch: ${branch}`, "merged branch")
              ) {
      ...
      console.log(
        `  ⚠️  Git cleanup warning: Not in a git repository or git not available`,
      );
    }

    console.log(
      `  ✅ Git optimization completed (${result.filesRemoved} items cleaned)`,
    );
    return result;
  }
  ...
  async cleanTempFiles() {
    ...
    console.log(
      `  ✅ Removed ${result.filesRemoved} temporary files (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  async optimizeFiles() {
    ...
    console.log(
      `  ✅ Optimized ${result.filesRemoved} files/directories (${this.formatBytes(result.bytesFreed)} freed)`,
    );
    return result;
  }
  ...
  findDuplicateFiles() {
    ...
    try {
      const command =
        'find . -type f -not -path "./.git/*" -not -path "./node_modules/*" -exec ls -la {} \\;';
      const output = execSync(command, { cwd: ROOT_DIR, encoding: "utf-8" });
...
async function main() {
  try {
    ...
    if (!options.force && !options.dryRun && !options.interactive) {
      console.log(
        "⚠️  Running cleanup without --dry-run. This will delete files!",
      );
