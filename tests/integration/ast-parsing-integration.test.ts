...
import {
  ParserFactory,
  createParser,
} from "../../packages/ast-helper/src/parser/parsers/factory.js";
...
import {
  ...
  detectLanguage,
  getLanguageConfig,
} from "../../packages/ast-helper/src/parser/languages.js";
...
class ASTParsingTestUtils {
  ...
  async setupTestEnvironment(): Promise<void> {
    this.tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "ast-parsing-test-"),
    );
  }
  ...
  async createCodeSamples(): Promise<
    Map<string, { path: string; content: string; language: string }>
  > {
...
`;
    ...
    const pyPath = await this.createTestFile(
      "user-repository.py",
      pythonContent,
    );

    samples.set("typescript", {
      path: tsPath,
      content: tsContent,
      language: "typescript",
    });
    samples.set("javascript", {
      path: jsPath,
      content: jsContent,
      language: "javascript",
    });
  ...
  generateMalformedCodeSamples(): Map<
    string,
    { content: string; language: string; expectedErrorType: string }
  > {
    const samples = new Map();
  ...
  validateASTNode(node: ASTNode): boolean {
    ...
    if (typeof node.start.line !== "number") {
      return false;
    }
    if (typeof node.start.column !== "number") {
      return false;
    }
    if (typeof node.end.line !== "number") {
      return false;
    }
    if (typeof node.end.column !== "number") {
      return false;
    }
    ...
    if (
      ...
      node.start.column > node.end.column
    ) {
      return false;
    }
...
class ComprehensiveASTParsingIntegrationTestSuite {
  ...
  async testMultiLanguageParsing(): Promise<void> {
    ...
    for (const [language, sample] of this.testSamples) {
      ...
      console.log(
        `  ✓ ${language}: ${result.nodes.length} nodes, ${result.errors.length} errors`,
      );
    }
  ...
  async testSyntaxTreeValidation(): Promise<void> {
    ...
    const tsResult = await parser.parseFile(
      this.testSamples.get("typescript")!.path,
    );
    ...
    if (tsResult.nodes.length > 0) {
      ...
      console.warn(
        "TypeScript parsing returned no nodes - may indicate parser setup issue",
      );
    }
    ...
    if (tsResult.nodes.length > 0) {
      ...
      console.log(
        `  TypeScript complexity: ${tsAnalysis.totalNodes} nodes, depth ${tsAnalysis.maxDepth}`,
      );
    ...
    const jsResult = await parser.parseFile(
      this.testSamples.get("javascript")!.path,
    );
    ...
    if (jsResult.nodes.length > 0) {
      ...
      console.log(
        `  JavaScript complexity: ${jsAnalysis.totalNodes} nodes, depth ${jsAnalysis.maxDepth}`,
      );
    } else {
      console.warn(
        "JavaScript parsing returned no nodes - may indicate parser setup issue",
      );
    }

    // Test Python parsing accuracy
    const pyResult = await parser.parseFile(
      this.testSamples.get("python")!.path,
    );
    ...
    if (pyResult.nodes.length > 0) {
      ...
      console.log(
        `  Python complexity: ${pyAnalysis.totalNodes} nodes, depth ${pyAnalysis.maxDepth}`,
      );
      ...
      console.warn(
        "Python parsing returned no nodes - may indicate parser setup issue",
      );
    }
  ...
  async testParserPerformance(): Promise<void> {
    ...
    for (const [language, sample] of this.testSamples) {
      ...
      console.log(
        `  ${language}: ${parseTime.toFixed(2)}ms, ${result.nodes.length} nodes`,
      );
    }
    ...
    const batchResults = await parser.batchParseFiles(batchFiles, {
      ...
      onProgress: (completed, total, currentFile) => {
        console.log(
          `  Batch progress: ${completed}/${total} (${path.basename(currentFile)})`,
        );
      },
    });
    ...
    console.log(
      `  Batch parsing: ${batchTime.toFixed(2)}ms for ${batchFiles.length} files`,
    );
  ...
  async testErrorHandling(): Promise<void> {
    ...
    for (const [testName, sample] of malformedSamples) {
      ...
      const hasExpectedError = result.errors.some(
        (error) => error.type === "syntax" || error.type === "runtime",
      );
    ...
    const unsupportedResult = await parser.parseCode(
      "some code",
  ...
  async testCrossPlatformCompatibility(): Promise<void> {
    ...
    const windowsResult = await parser.parseCode(
      windowsLineEndings,
      "javascript",
    );
    ...
    if (
      windowsResult.errors.length > 0 &&
      windowsResult.errors[0].type === "runtime"
    ) {
      console.warn(
        "Runtime error in Windows line ending test:",
        windowsResult.errors[0],
      );
    } else {
    ...
    if (
      unixResult.errors.length > 0 &&
      unixResult.errors[0].type === "runtime"
    ) {
      console.warn(
        "Runtime error in Unix line ending test:",
        unixResult.errors[0],
      );
    } else {
    ...
    if (windowsResult.nodes.length > 0 && unixResult.nodes.length > 0) {
      ...
      windowsResult.nodes.length === 0 &&
      unixResult.nodes.length === 0
    ) {
      console.warn(
        "Both line ending tests returned no nodes - may indicate parser setup issue",
      );
    }
...
`;
    ...
    const largeFileResult = await parser.parseCode(
      largeTypeScriptCode,
      "typescript",
    );
    ...
    if (
      largeFileResult.errors.length > 0 &&
      largeFileResult.errors[0].type === "runtime"
    ) {
      console.warn(
        "Runtime error in large file parsing:",
        largeFileResult.errors[0],
      );
    } else {
    ...
    if (largeFileResult.nodes.length > 0) {
      ...
      console.warn(
        "Large file parsing returned no nodes - may indicate parser setup issue",
      );
    }
...
`;
    ...
    const nestedResult = await parser.parseCode(
      nestedJavaScriptCode,
      "javascript",
    );

    // Handle runtime errors gracefully for nested code parsing
    if (
      nestedResult.errors.length > 0 &&
      nestedResult.errors[0].type === "runtime"
    ) {
      console.warn(
        "Runtime error in nested code parsing:",
        nestedResult.errors[0],
      );
    } else {
    ...
    console.log(
      `  Large file: ${largeFileResult.nodes.length} nodes in ${largeFileResult.parseTime.toFixed(2)}ms`,
    );
    console.log(
      `  Nested code: depth ${complexity.maxDepth}, ${nestedResult.nodes.length} nodes`,
    );
