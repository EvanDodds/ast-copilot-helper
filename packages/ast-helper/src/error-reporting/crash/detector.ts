...
export class CrashDetector {
  ...
  private checkMemoryThresholds(systemState: SystemStateSnapshot): void {
    const heapUsagePercent =
      (systemState.heap.used / systemState.heap.total) * 100;

    if (heapUsagePercent > this.config.memoryThreshold) {
      const error = new Error(
        `Memory usage exceeded threshold: ${heapUsagePercent.toFixed(2)}%`,
      );
      ...
      this.reportCrash(error, {
        ...
        console.error(
          "❌ Failed to report memory threshold crash:",
          reportError,
        );
      });
    }
  ...
  private checkEventLoopLag(systemState: SystemStateSnapshot): void {
    if (systemState.eventLoop.lag > this.config.eventLoopLagThreshold) {
      const error = new Error(
        `Event loop lag exceeded threshold: ${systemState.eventLoop.lag.toFixed(2)}ms`,
      );
  ...
  private checkResourceLimits(systemState: SystemStateSnapshot): void {
    ...
    if (activeHandles > this.config.fileDescriptorThreshold) {
      const error = new Error(
        `Active handles exceeded threshold: ${activeHandles}`,
      );
  ...
  private async attemptRecovery(
    ...
    for (const action of applicableActions) {
      if (recovery.actions.length >= this.config.maxRecoveryAttempts) {
        console.log(
          `⚠️ Maximum recovery attempts (${this.config.maxRecoveryAttempts}) reached`,
        );
        break;
      }
      ...
      try {
        console.log(
          `🔧 Executing recovery action: ${action.id} (${action.type})`,
        );
        ...
        console.error(
          `❌ Error executing recovery action ${action.id}:`,
          error,
        );
    ...
    recovery.recoveryDuration = Date.now() - startTime;
    console.log(
      `🏁 Recovery completed in ${recovery.recoveryDuration}ms, final state: ${recovery.finalState}`,
    );
   ...
   */
  private isRecoveryActionApplicable(
    ...
    if (
      conditions.crashType &&
      !conditions.crashType.includes(crashReport.type)
    ) {
      return false;
    }
    ...
    if (
      conditions.severity &&
      !conditions.severity.includes(crashReport.severity)
    ) {
      return false;
    }
    ...
    if (conditions.resourceThreshold) {
      const memoryUsage =
        crashReport.systemState.heap.used / crashReport.systemState.heap.total;
  ...
  private async executeRecoveryAction(
    ...
    try {
      ...
      const result = await Promise.race([action.execute(), timeoutPromise]);
  ...
  private storeCrashReport(crashReport: CrashReport): void {
    ...
    if (this.crashHistory.length > this.config.maxCrashReports) {
      this.crashHistory = this.crashHistory.slice(
        0,
        this.config.maxCrashReports,
      );
    }
  ...
  private countSimilarCrashes(error: Error): number {
    return this.crashHistory.filter(
      (crash) =>
