...
export const NORMALIZATION_CONFIGS: Record<string, NormalizationConfig> = {
  ...
  python: {
    ...
    identifierTypes: new Set(["identifier", "attribute", "dotted_name"]),
    literalTypes: new Set([
      "string",
      "integer",
      "float",
...
export class ASTNormalizer {
    ...
    if (customConfigs) {
      for (const [language, customConfig] of Object.entries(customConfigs)) {
        if (this.configs[language]) {
          this.configs[language] = this.mergeConfigs(
  ...
  private normalizeNode(
    ...
    if (depth > options.maxDepth) {
      throw new Error(
        `Maximum normalization depth (${options.maxDepth}) exceeded`,
      );
    }
  ...
  private categorizeNode(
    nodeType: string,
    config: NormalizationConfig,
  ): NormalizedASTNode["metadata"]["category"] {
  ...
  private mergeConfigs(
    ...
    return {
      nodeTypeMappings: {
        ...base.nodeTypeMappings,
        ...custom.nodeTypeMappings,
      },
      ignoredNodeTypes: new Set([
        ...base.ignoredNodeTypes,
        ...(custom.ignoredNodeTypes || []),
      ]),
      identifierTypes: new Set([
        ...base.identifierTypes,
        ...(custom.identifierTypes || []),
      ]),
      literalTypes: new Set([
        ...base.literalTypes,
        ...(custom.literalTypes || []),
      ...
      declarationTypes: new Set([
        ...
        ...(custom.declarationTypes || []),
      ]),
      statementTypes: new Set([
        ...base.statementTypes,
        ...(custom.statementTypes || []),
      ]),
      expressionTypes: new Set([
        ...base.expressionTypes,
        ...(custom.expressionTypes || []),
      ]),
      scopeBoundaries: new Set([
        ...base.scopeBoundaries,
        ...(custom.scopeBoundaries || []),
      ]),
