...
export async function ensureLockDirectory(lockFilePath: string): Promise<void> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "EEXIST") {
      throw new Error(
        `Failed to create lock directory ${dir}: ${(error as Error).message}`,
      );
    }
  }
}
...
export async function readLockFile(lockFilePath: string): Promise<Lock | null> {
  try {
    ...
    throw new Error(
      `Failed to read lock file ${lockFilePath}: ${(error as Error).message}`,
    );
  }
}
...
export async function writeLockFile(
  ...
  try {
    ...
    throw new Error(
      `Failed to write lock file ${lockFilePath}: ${(error as Error).message}`,
    );
  }
}
