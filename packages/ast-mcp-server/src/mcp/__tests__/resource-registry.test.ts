...
describe("MCPResourceRegistry", () => {
  ...
  describe("Resource Registration", () => {
    ...
    it("should throw error for duplicate URI registration", async () => {
      ...
      await expect(registry.registerResource(mockResource)).rejects.toThrow(
        `Resource with URI '${mockResource.uri}' is already registered`,
      );
    });
  ...
  describe("Resource Discovery", () => {
    ...
    it("should exclude disabled resources by default", async () => {
      ...
      const disabledUri = allResources.find(
        (r) => r.uri === "disabled://resource",
      );
    ...
    it("should include disabled resources when requested", async () => {
      ...
      const disabledResource = allResources.find(
        (r) => r.uri === "disabled://resource",
      );
  ...
  describe("Resource Access", () => {
    ...
    it("should timeout on slow resources", async () => {
      ...
      await expect(registry.accessResource(slowResource.uri)).rejects.toThrow(
    ...
    it("should handle resource access errors", async () => {
      ...
      await expect(registry.accessResource(errorResource.uri)).rejects.toThrow(
        "Access error",
      );
    ...
    it("should throw error for non-existent resource", async () => {
      await expect(registry.accessResource("non-existent")).rejects.toThrow(
    ...
    it("should throw error for disabled resource", async () => {
      ...
      await expect(
        ...
        `Resource '${mockResourceWithHandler.uri}' is disabled`,
      );
    });
  ...
  describe("Cache Management", () => {
    it("should enforce cache size limits", async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await smallCacheRegistry.registerResource(resource, {
          cacheable: true,
        });
      }
