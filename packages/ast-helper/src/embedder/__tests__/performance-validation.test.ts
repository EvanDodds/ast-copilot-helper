...
describe("Embedding System Performance Validation", () => {
  ...
  describe("Batch Processing Performance", () => {
    it("should meet batch processing time requirements", async () => {
      ...
      for (const [size, result] of Object.entries(results)) {
        console.log(
          `  ${size} annotations: ${result.timeMs.toFixed(2)}ms (${result.throughput.toFixed(2)} annotations/sec)`,
        );
      }
    });
    ...
    it("should handle concurrent batch processing efficiently", async () => {
      ...
      for (const concurrency of concurrencyLevels) {
        ...
        console.log(
          `Concurrency ${concurrency}: ${totalTime.toFixed(2)}ms (${throughput.toFixed(2)} annotations/sec)`,
        );
  ...
  describe("Memory Management Performance", () => {
    it("should maintain memory usage within limits", async () => {
      ...
      performanceMetrics.memoryUsage.before =
      ...
      console.log(
        `  Initial: ${performanceMetrics.memoryUsage.before.toFixed(2)}MB`,
      );
      console.log(
        `  Peak: ${performanceMetrics.memoryUsage.peak.toFixed(2)}MB`,
      );
      console.log(
        `  Final: ${performanceMetrics.memoryUsage.after.toFixed(2)}MB`,
      );
      console.log(
        `  Growth: ${(performanceMetrics.memoryUsage.peak - performanceMetrics.memoryUsage.before).toFixed(2)}MB`,
      );
    ...
    it("should cleanup memory efficiently between batches", async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        const avgMemoryPerBatch =
          memorySnapshots[batchSize].reduce((sum, mem) => sum + mem, 0) /
          memorySnapshots[batchSize].length;
        console.log(
          `Batch size ${batchSize}: Average memory per batch: ${avgMemoryPerBatch.toFixed(2)}MB`,
        );
  ...
  describe("Text Processing Performance", () => {
    it("should meet text processing speed requirements", async () => {
      ...
      for (const size of textSizes) {
        ...
        console.log(
          `Text size ${size} chars: ${totalTime.toFixed(2)}ms (${throughput.toFixed(2)} texts/sec)`,
        );
    ...
    it("should handle edge cases efficiently", async () => {
      ...
      const edgeCases = [
        ...
        },
        {
          ...
          annotation: {
            ...
            sourceSnippet:
              'const test = "string with \\"quotes\\" and \\n newlines";',
          },
        },
      ];
      ...
      for (const testCase of edgeCases) {
        ...
        console.log(
          `Edge case ${testCase.name}: ${processingTime.toFixed(2)}ms`,
        );
  ...
  describe("Overall System Performance", () => {
    it("should meet end-to-end performance requirements", async () => {
      ...
      for (const size of workloadSizes) {
        ...
        const results = await simulateEndToEndProcessing(
          annotations,
          mockConfig,
        );
        ...
        console.log(
          `  Annotation throughput: ${annotationThroughput.toFixed(2)} annotations/sec`,
        );
        console.log(
          `  Embedding throughput: ${embeddingThroughput.toFixed(2)} embeddings/sec`,
        );
        console.log(
          `  Average processing time: ${avgProcessingTime.toFixed(2)}ms per annotation`,
        );
      ...
      const throughputRatio =
        large.annotationThroughput / small.annotationThroughput;
...
async function simulateEndToEndProcessing(
  ...
  for (const batch of batches) {
    const batchResults = await simulateBatchProcessing(
      batch,
      config.embeddings!,
    );
