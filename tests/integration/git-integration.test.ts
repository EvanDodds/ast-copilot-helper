...
class ComprehensiveGitIntegrationTestSuite {
  ...
  async setupTestEnvironment(): Promise<void> {
    ...
    await this.gitTestUtils.createFile(
      join(this.nonRepoDir, "test.txt"),
  ...
  async testStatusAndChangeDetection(): Promise<void> {
    ...
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "src", "modified.ts"),
      "export const modified = true;",
    );
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "untracked.ts"),
      "export const untracked = true;",
    );

    // Modify existing file
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "src", "index.ts"),
    ...
    expect(
      statusWithChanges.modifiedFiles +
        statusWithChanges.untrackedFiles +
        statusWithChanges.stagedFiles,
    ).toBeGreaterThan(0);
  ...
  async testChangedFilesRetrieval(): Promise<void> {
    ...
    // Create some changes
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "new-file.ts"),
      "export const newFile = true;",
    );
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "src", "updated.ts"),
      "export const updated = true;",
    );

    // Modify existing file
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "README.md"),
  ...
  async testGitReferenceValidation(): Promise<void> {
    ...
    const headValid = await this.gitManager.validateGitReference(
      "HEAD",
      this.testRepoDir,
    );
    ...
    const branchValid = await this.gitManager.validateGitReference(
      currentBranch,
      this.testRepoDir,
    );
    ...
    const invalidRef = await this.gitManager.validateGitReference(
      "non-existent-branch",
      this.testRepoDir,
    );
    ...
    const { stdout } = await this.gitTestUtils.execGit(
      ["rev-parse", "HEAD"],
      this.testRepoDir,
    );
    const commitHash = stdout.trim();
    const commitValid = await this.gitManager.validateGitReference(
      commitHash,
      this.testRepoDir,
    );
    ...
    const partialValid = await this.gitManager.validateGitReference(
      partialHash,
      this.testRepoDir,
    );
  ...
  async testBranchOperationsWorkflow(): Promise<void> {
    ...
    // Create new branch
    await this.gitTestUtils.execGit(
    ...
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "feature.ts"),
  ...
  async testErrorHandlingEdgeCases(): Promise<void> {
    ...
    await expect(
      this.gitManager.getRepositoryRoot(this.nonRepoDir),
    ).rejects.toThrow();
    await expect(
      this.gitManager.getChangedFiles({ cwd: this.nonRepoDir }),
    ).rejects.toThrow();
    await expect(
      this.gitManager.getStagedFiles(this.nonRepoDir),
    ).rejects.toThrow();
    ...
    const invalidInEmpty = await emptyGitManager.validateGitReference(
      "non-existent-ref",
      emptyRepoDir,
    );
  ...
  async testCrossPlatformCompatibility(): Promise<void> {
    ...
    await this.gitTestUtils.createDirectory(
      join(this.testRepoDir, "deep", "nested", "path"),
    );
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "deep", "nested", "path", "file.ts"),
  ...
  async testPerformanceScalability(): Promise<void> {
    ...
    for (let i = 0; i < 50; i++) {
      filePromises.push(
        this.gitTestUtils.createFile(
          join(this.testRepoDir, `bulk-file-${i}.ts`),
  ...
  async testGitWorkflowIntegration(): Promise<void> {
    ...
    await this.gitTestUtils.execGit(
      ["checkout", "-b", "workflow-test"],
      this.testRepoDir,
    );

    // 3. Make changes
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "workflow.ts"),
      'export const workflow = "test";',
    );
    await this.gitTestUtils.createFile(
      join(this.testRepoDir, "src", "feature.ts"),
    ...
    const diffWithDefault = await this.gitManager.getChangedFiles({
      base: defaultBranch,
    });
  ...
  async testMockIntegrationScenarios(): Promise<void> {
    ...
    mockGitManager.setMockResponse("getChangedFiles", [
      "mocked-file1.ts",
      "mocked-file2.ts",
    ]);
