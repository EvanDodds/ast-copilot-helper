...
export class GlobFileSelector implements FileSelector {
    ...
    try {
      if (!options.glob) {
        throw ValidationErrors.invalidValue(
          "glob pattern",
          "undefined",
          "Glob pattern is required for glob-based file selection",
        );
      }
      ...
      const globResult = await this.globManager.expandPatterns(
        patterns,
        workspacePath,
      );

      // Convert to full paths and get metadata
      const candidateFiles = globResult.files.map((file) =>
        resolve(workspacePath, file),
      );
      const filesWithMetadata = await this.getFileMetadata(candidateFiles);

      // Apply filtering
      const { files, skipped, errors } =
        await this.filterFiles(filesWithMetadata);
  ...
  private buildGlobPatterns(globPattern: string): string[] {
    ...
    const ignorePatterns = DEFAULT_IGNORE_PATTERNS.map(
      (pattern) => `!${pattern}`,
    );
  ...
  async selectFiles(
    ...
    if (!selector) {
      throw ValidationErrors.invalidValue(
        ...
        strategy,
        "Unknown file selection strategy",
      );
    }
