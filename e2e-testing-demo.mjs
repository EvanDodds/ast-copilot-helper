...
async function runE2ETestingDemo() {
  ...
  runner.on('scenario:complete', ({ scenario, result }) => {
    ...
    console.log(`${status} Scenario ${scenario}: ${score}% (${passedSteps}/${steps} steps, ${duration}ms)`);
  ...
  try {
    ...
    console.log(`  Reliability Score: ${result.summary.reliabilityScore.toFixed(1)}%`);
    console.log(`  Resource Efficiency: ${result.summary.resourceEfficiency}%\n`);

    // Performance metrics
    console.log('⚡ Performance Metrics:');
    console.log(`  Average Response Time: ${result.performance.averageResponseTime.toFixed(0)}ms`);
    console.log(`  Peak Response Time: ${result.performance.peakResponseTime.toFixed(0)}ms`);
    ...
    console.log(`  Availability: ${result.performance.availability.toFixed(1)}%`);
    console.log(`  Scalability Score: ${result.performance.scalabilityScore}%\n`);

    // Resource metrics
    console.log('💾 Resource Usage:');
    console.log(`  Peak Memory: ${result.resources.peakMemoryMB.toFixed(1)}MB`);
    console.log(`  Average Memory: ${result.resources.averageMemoryMB.toFixed(1)}MB`);
    console.log(`  Peak CPU: ${result.resources.peakCpuPercent.toFixed(1)}%`);
    console.log(`  Average CPU: ${result.resources.averageCpuPercent.toFixed(1)}%`);
    ...
    if (result.resources.resourceLeaks.length > 0) {
      console.log(`  ⚠️  ${result.resources.resourceLeaks.length} resource leaks detected`);
    }
    console.log('');
    ...
    console.log(`  System Uptime: ${(result.environment.system.uptime / 3600).toFixed(1)}h`);
