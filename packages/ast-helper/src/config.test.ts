...
describe("Configuration System", () => {
  ...
  describe("Default Configuration", () => {
    it("should provide valid default configuration", () => {
      ...
      expect(DEFAULT_CONFIG.parseGlob).toEqual([
        "src/**/*.ts",
        "src/**/*.js",
        "src/**/*.py",
      ]);
  ...
  describe("Configuration Validation", () => {
    it("should throw error for invalid topK", () => {
      expect(() => validateConfig({ topK: 0 })).toThrow(
        "Configuration validation failed",
      );
      expect(() => validateConfig({ topK: 1001 })).toThrow(
        "Configuration validation failed",
      );
    });

    it("should throw error for invalid snippetLines", () => {
      expect(() => validateConfig({ snippetLines: 0 })).toThrow(
        "Configuration validation failed",
      );
      expect(() => validateConfig({ snippetLines: 201 })).toThrow(
        "Configuration validation failed",
      );
    });
    ...
    it("should throw error for invalid concurrency", () => {
      ...
      expect(() => validateConfig({ concurrency: 33 })).toThrow(
        "Configuration validation failed",
      );
    });
    ...
    it("should throw error for invalid index parameters", () => {
      expect(() =>
        validateConfig({ indexParams: { efConstruction: 15 } }),
      ).toThrow("Configuration validation failed");
      expect(() => validateConfig({ indexParams: { M: 3 } })).toThrow(
        "Configuration validation failed",
      );
    });

    it("should throw error for invalid modelHost", () => {
      expect(() => validateConfig({ modelHost: "" })).toThrow(
        "Configuration validation failed",
      );
      expect(() => validateConfig({ modelHost: "   " })).toThrow(
        "Configuration validation failed",
  ...
  describe("Environment Variable Parsing", () => {
    ...
    it("should parse array environment variables", () => {
      process.env.AST_COPILOT_PARSE_GLOB =
        "src/**/*.ts,lib/**/*.js,test/**/*.py";
      const config = parseEnvironmentConfig();
      expect(config.parseGlob).toEqual([
        "src/**/*.ts",
        "lib/**/*.js",
        "test/**/*.py",
      ]);
  ...
  describe("ConfigManager Integration", () => {
    ...
    it("should handle configuration loading errors gracefully", async () => {
      ...
      await expect(
        configManager.loadConfig("/tmp/test-workspace", invalidArgs),
