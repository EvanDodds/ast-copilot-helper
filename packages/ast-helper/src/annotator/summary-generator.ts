...
export class SummaryGenerator {
  ...
  private defaultTemplates = new Map<string, SummaryTemplate>([
    ['function', {
      ...
      placeholders: {
        ...
        'async': ['async ', ''],
        'purpose': ['that handles', 'that processes', 'that validates', 'that transforms', 'that creates']
      }
    } as SummaryTemplate],
    ['class', {
      ...
      placeholders: {
        ...
        'purpose': ['for managing', 'for handling', 'for processing', 'for representing']
      }
    } as SummaryTemplate],
    ['interface', {
    ...
    ['method', {
      pattern: '{visibility} {static}{async}method {name}{signature} {purpose}',
      placeholders: {
        'visibility': ['public', 'private', 'protected'],
        'static': ['static ', ''],
        'async': ['async ', ''],
        'purpose': ['that handles', 'that processes', 'that validates', 'that returns']
      }
    } as SummaryTemplate],
    ['variable', {
  ...
  private patterns: SummaryPattern[] = [
    ...
    // Factory functions
    {
      ...
      template: 'Factory function that creates and returns {returnType} instances',
    ...
    // Validators
    {
      ...
      template: 'Validator function that checks {validation} and returns boolean result',
  ...
  public async generateSummary(
    ...
    try {
      ...
      if (this.config.usePatternMatching) {
        const patternSummary = await this.generatePatternBasedSummary(node, sourceText, filePath);
        if (patternSummary) {
      ...
      const templateSummary = await this.generateTemplateBasedSummary(node, sourceText, filePath);
      
      // Enhance with additional information
      const enhancedSummary = await this.enhanceSummary(templateSummary, node, sourceText, filePath);
  ...
  public async generateSemanticTags(
    ...
    try {
      ...
      if (this.config.includeComplexity) {
        const complexityTags = await this.getComplexityBasedTags(node, sourceText);
        complexityTags.forEach(tag => tags.add(tag));
      }

      // Add dependency-based tags
      if (this.config.includeDependencies) {
        const dependencyTags = await this.getDependencyBasedTags(node, filePath);
  ...
  public async determinePurposeCategory(
    ...
    if (nodeType.includes('test') || nodeName.includes('test') || nodeName.includes('spec')) {
  ...
  private async generateTemplateBasedSummary(
    ...
    const template = this.defaultTemplates.get(nodeType) || this.defaultTemplates.get('function')!;
  ...
  private async enhanceSummary(
    ...
    if (this.config.includeComplexity && sourceText) {
      const complexity = await this.complexityAnalyzer.calculateCyclomaticComplexity(node, sourceText);
      if (complexity > 5) {
    ...
    if (this.config.includeDependencies && filePath) {
      const dependencies = await this.dependencyAnalyzer.analyzeDependencies(node, filePath);
    ...
    if (enhanced.length > this.config.maxSummaryLength) {
      enhanced = enhanced.substring(0, this.config.maxSummaryLength - 3) + '...';
    }
  ...
  private isApiEndpoint(node: ASTNode): boolean {
    ...
    const hasRouteDecorator = nodeAny.decorators?.some((d: any) => 
      d.name && ['get', 'post', 'put', 'delete', 'patch'].includes(d.name.toLowerCase())
    );
    
    return hasRouteDecorator || 
  ...
  private extractSignature(node: ASTNode, _sourceText?: string): string {
    ...
    if (nodeAny.parameters) {
      const params = nodeAny.parameters.map((p: any) => p.name || 'param').join(', ');
      return `(${params})`;
    }
  ...
  private async getComplexityBasedTags(node: ASTNode, sourceText?: string): Promise<SemanticTag[]> {
    const tags: SemanticTag[] = [];
  ...
  private async getDependencyBasedTags(node: ASTNode, filePath?: string): Promise<SemanticTag[]> {
    const tags: SemanticTag[] = [];
