...
import { EnhancedConfigValidator, type ValidationError } from '../enhanced-validator.js';
...
describe('Enhanced Configuration Validation', () => {
  ...
  describe('Basic Validation', () => {
    ...
    it('should clean and normalize string values', () => {
      ...
      expect(result.cleanedConfig?.modelHost).toBe('https://example.com/models');
    });
  });
  ...
  describe('Error Reporting', () => {
    it('should provide detailed error messages for invalid types', () => {
      ...
      const telemetryError = result.errors.find(e => e.path === 'enableTelemetry');
    ...
    it('should validate numeric ranges with helpful messages', () => {
      ...
      const efError = result.errors.find(e => e.path === 'indexParams.efConstruction');
    ...
    it('should validate array contents with specific error messages', () => {
      ...
      const watchPathsError = result.errors.find(e => e.path === 'fileWatching.watchPaths');
      expect(watchPathsError?.message).toContain('must contain at least 1 valid');
    });
  });
  ...
  describe('Nested Object Validation', () => {
    it('should validate deeply nested configuration objects', () => {
      ...
      const snippetError = result.errors.find(e => e.path === 'embeddings.textProcessing.maxSnippetLength');
    ...
    it('should validate file watching configuration comprehensively', () => {
      ...
      const debounceError = result.errors.find(e => e.path === 'fileWatching.debounceMs');
      expect(debounceError?.message).toContain('must be between 0 and 10000');
      
      const batchError = result.errors.find(e => e.path === 'fileWatching.batchSize');
      ...
      expect(validResult.cleanedConfig?.fileWatching?.enableRecursive).toBe(true);
      expect(validResult.cleanedConfig?.fileWatching?.followSymlinks).toBe(false);
    });
  });
