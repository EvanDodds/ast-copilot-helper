...
import type { InitOptions, VersionInfo } from "./types.js";
...
export class DatabaseVersionManager {
  ...
  async validateVersionCompatibility(astdbPath: string): Promise<void> {
    ...
    if (
      !this.isVersionCompatible(versionInfo.schemaVersion, currentSchemaVersion)
    ) {
      // Database is newer than current tool
      if (
        this.compareVersions(versionInfo.schemaVersion, currentSchemaVersion) >
        0
      ) {
        throw DatabaseErrors.versionMismatch(
  ...
  async updateVersionInfo(
    ...
    if (
      migrationId &&
      !versionInfo.compatibility.migrations.includes(migrationId)
    ) {
  ...
  async planMigration(astdbPath: string): Promise<string[]> {
    ...
    if (comparison < 0) {
      ...
      migrationPlan.push(
        `upgrade-from-${versionInfo.schemaVersion}-to-${currentSchemaVersion}`,
      );
    } else if (comparison > 0) {
      // Database is newer - downgrade needed (rarely supported)
      migrationPlan.push(
        `downgrade-from-${versionInfo.schemaVersion}-to-${currentSchemaVersion}`,
      );
    }
  ...
  async getDatabaseAge(astdbPath: string): Promise<{
    ...
    const lastMigrated = versionInfo.lastMigrated
      ? new Date(versionInfo.lastMigrated)
      : undefined;
    const now = new Date();
  ...
  private validateVersionInfo(versionInfo: any): void {
    ...
    for (const field of requiredStringFields) {
      if (
        typeof versionInfo[field] !== "string" ||
        versionInfo[field].trim() === ""
      ) {
    ...
    if (
      versionInfo.schemaVersion &&
      !this.isValidSemver(versionInfo.schemaVersion)
    ) {
    ...
    if (
      ...
      !this.isValidSemver(versionInfo.toolVersion)
    ) {
    ...
    if (
      versionInfo.lastMigrated &&
      isNaN(new Date(versionInfo.lastMigrated).getTime())
    ) {
    ...
    if (
      typeof versionInfo.compatibility !== "object" ||
      versionInfo.compatibility === null
    ) {
      ...
      if (
        typeof comp.minToolVersion !== "string" ||
        comp.minToolVersion.trim() === ""
      ) {
