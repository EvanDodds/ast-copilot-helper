...
describe("Error Handling Framework", () => {
  ...
  describe("Error Formatter", () => {
    ...
    it("should format error for debugging", () => {
      const error = new DatabaseError("Database corruption", {
        dbPath: "/data/ast.db",
        version: "1.0",
      });
  ...
  describe("Error Factory Functions", () => {
    describe("ConfigurationErrors", () => {
      it("should create invalid value error", () => {
        const error = ConfigurationErrors.invalidValue(
          "topK",
          "invalid",
          "number",
        );
        ...
        expect(error.message).toContain(
          "Invalid configuration value for 'topK'",
        );
      ...
      it("should create missing required error", () => {
        const error = ConfigurationErrors.missingRequired(
          "apiKey",
      ...
      it("should create file not accessible error", () => {
        const error = ConfigurationErrors.fileNotAccessible(
          "/config.json",
          "Permission denied",
        );

        expect(error.message).toContain(
          "Configuration file not accessible: /config.json",
        );
    ...
    describe("FileSystemErrors", () => {
      it("should create not found error", () => {
        ...
        expect(error.message).toContain(
          "File or directory not found: /missing/file.txt",
        );
      ...
      it("should create permission denied error", () => {
        const error = FileSystemErrors.permissionDenied(
          "/protected/file.txt",
          "write",
        );

        expect(error.message).toContain(
          "Permission denied: cannot write /protected/file.txt",
        );
        expect(
          error.suggestions.some((s) => s.includes("file permissions")),
        ).toBe(true);
      });

      it("should create disk space exceeded error", () => {
        const error = FileSystemErrors.diskSpaceExceeded(
          "/tmp",
          1024 * 1024 * 100,
        );

        expect(error.message).toContain(
          "Insufficient disk space for operation at: /tmp",
        );
    ...
    describe("ValidationErrors", () => {
      ...
      it("should create out of range error", () => {
        ...
        expect(error.message).toContain(
          "Value out of range for topK: 150 (must be between 1 and 100)",
        );
    ...
    describe("ParseErrors", () => {
      it("should create syntax error", () => {
        const error = ParseErrors.syntaxError(
          "/src/file.ts",
          42,
          15,
          "Missing semicolon",
        ...
        expect(error.message).toContain(
          "Syntax error in /src/file.ts:42:15 - Missing semicolon",
        );
      ...
      it("should create unsupported language error", () => {
        ...
        expect(error.message).toContain(
          "Unsupported language or file type: Ruby for /src/file.rb",
        );
    ...
    describe("DatabaseErrors", () => {
      it("should create corruption error", () => {
        const error = DatabaseErrors.corruption(
          "/data/ast.db",
        ...
        expect(error.message).toContain(
          "Database corruption detected in /data/ast.db: Invalid magic number",
        );
      ...
      it("should create version mismatch error", () => {
        const error = DatabaseErrors.versionMismatch(
          "1.0",
          "2.0",
          "/data/ast.db",
        );
        ...
        expect(error.message).toContain(
          "Database version mismatch: found 1.0, expected 2.0",
        );
    ...
    describe("NetworkErrors", () => {
      it("should create timeout error", () => {
        ...
        expect(error.message).toContain(
          "Network timeout connecting to https://api.example.com after 5000ms",
        );
      ...
      it("should create service unavailable error", () => {
        ...
        expect(error.message).toContain(
          "Service unavailable: API Service (HTTP 503)",
        );
    ...
    describe("TimeoutErrors", () => {
      it("should create operation timeout error", () => {
        ...
        expect(error.message).toContain(
          "Operation timeout: database query exceeded 30000ms",
        );
