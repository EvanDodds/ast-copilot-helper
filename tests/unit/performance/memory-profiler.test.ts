...
} from "../../../packages/ast-helper/src/performance/memory-profiler";
import {
  MemoryProfile,
  PhaseMemoryProfile,
  MemoryLeak,
...
describe("MemoryProfiler", () => {
  ...
  describe("Workload Simulation", () => {
    ...
    it("should throw error for unknown workload type", async () => {
      ...
      await expect(profiler.runMemoryProfiling(invalidConfig)).rejects.toThrow(
  ...
  describe("Memory Leak Detection", () => {
    ...
    it("should detect potential leaks with high retention", async () => {
      ...
      const mockHighRetention = () => {
        ...
        const baseMemory = 10000000;
        const growth =
          callCount < 5 ? callCount * 5000000 : callCount * 4800000; // High retention
    ...
    it("should categorize leak severity correctly", async () => {
      ...
      const mockExtremeRetention = () => {
        ...
        const baseMemory = 10000000;
        const growth =
          callCount < 5 ? callCount * 10000000 : callCount * 9800000; // 98% retention
      ...
      const highSeverityLeaks = result.memoryLeaks.filter(
        (leak: MemoryLeak) => leak.severity === "high",
      );
