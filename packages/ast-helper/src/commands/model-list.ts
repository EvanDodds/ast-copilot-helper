...
export class ModelListCommandHandler {
    ...
    async execute(options: ModelListOptions, config: Config): Promise<void> {
        try {
            ...
            this.logger.error(`❌ List operation failed: ${(error as Error).message}`);
            throw error;
        }
    }
    ...
    private async listCachedModels(config: Config): Promise<void> {
        ...
        this.logger.info(`  Total cached: ${stats.totalModels} models (${this.formatBytes(stats.totalSize)})`);
        this.logger.info(`  Valid: ${stats.validModels}, Invalid: ${stats.invalidModels}`);
        ...
        for (const modelName of ModelRegistry.listModels()) {
            ...
            if (cacheResult.hit) {
                ...
                this.logger.info(`  ${status} ${model.name} v${model.version} - ${model.format.toUpperCase()}`);
            }
        }
    }
    ...
    private displayModels(models: any[], detailed: boolean): void {
        for (const model of models) {
            ...
            if (detailed) {
                ...
                this.logger.info(`    Description: ${model.description || 'No description'}`);
                ...
                if (model.requirements) {
                    ...
                    if (model.requirements.architecture) {
                        this.logger.info(`      Architecture: ${model.requirements.architecture.join(', ')}`);
                    }
                    if (model.requirements.platforms) {
                        this.logger.info(`      Platforms: ${model.requirements.platforms.join(', ')}`);
                    }
                }
            }
