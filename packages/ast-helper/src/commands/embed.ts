...
export class EmbedCommand {
  ...
  async execute(options: EmbedOptions = {}): Promise<void> {
    ...
    try {
      ...
      const annotations = await this.loadAnnotations(options.input, this.config);
      ...
      if (annotationsToEmbed.length === 0) {
        this.logger.info('✅ All annotations already have embeddings (use --force to re-embed)');
        return;
      }
      
      this.logger.info(`🔄 Embedding ${annotationsToEmbed.length} new annotations`);
      
      // Generate embeddings
      const results = await this.generateEmbeddings(annotationsToEmbed, embeddingConfig);
      
      // Store results
      await this.storeResults(results, options.output);
      
      this.logger.info(`✅ Embedding generation completed: ${results.length} embeddings generated`);
   ...
   */
  private buildEmbeddingConfig(config: any, options: EmbedOptions): EmbeddingConfig {
    const embeddingDefaults = config.embeddings || {};
    
    return {
      modelPath: options.model || embeddingDefaults.modelPath || '.astdb/models/codebert-base',
      ...
      maxConcurrency: options.maxConcurrency || embeddingDefaults.maxConcurrency || 2,
      enableProgressReporting: options.verbose ?? embeddingDefaults.showProgress ?? true,
      enableConfidenceScoring: embeddingDefaults.enableConfidenceScoring ?? true,
    };
  }
   ...
   */
  private validateConfiguration(config: EmbeddingConfig, options: EmbedOptions): void {
    // Validate model path
    if (!config.modelPath) {
    ...
    this.logger.debug('✅ Configuration validation passed', { config, options });
  }
   ...
   * Load annotations from input source
   */
  private async loadAnnotations(inputPath?: string, config?: Config): Promise<Annotation[]> {
   ...
   */
  private async loadAnnotationsFromDatabase(config?: Config, dbPath?: string): Promise<Annotation[]> {
    try {
      ...
      this.logger.warn('⚠️  Annotation database integration not yet implemented, using mock data');
      ...
      throw new Error(`Failed to load annotations from database: ${error.message}`);
    }
  }
   ...
   */
  private async loadAnnotationsFromFile(filePath: string): Promise<Annotation[]> {
    try {
   ...
   */
  private async filterUnembeddedAnnotations(annotations: Annotation[]): Promise<Annotation[]> {
    if (!this.embeddingDb) {
      this.logger.debug('📝 Embedding database not initialized - processing all annotations');
      return annotations;
    }
    ...
    try {
      ...
      const existingEmbeddings = await this.embeddingDb.getExistingEmbeddings(nodeIds);
      ...
      if (skippedCount > 0) {
        this.logger.info(`📝 Found ${skippedCount} existing embeddings, processing ${unembeddedAnnotations.length} new annotations`);
      }
      
      return unembeddedAnnotations;
    } catch (error: any) {
      this.logger.warn(`⚠️  Error checking existing embeddings: ${error.message} - processing all annotations`);
      return annotations;
    }
  }
    ...
    const batchOptions = {
      ...
      progressCallback: config.enableProgressReporting 
        ? (processed: number, total: number) => {
            ...
            this.logger.info(`📊 Progress: ${processed}/${total} (${percent}%)`);
          }
        : undefined,
    };
   ...
   */
  private async storeResultsToFile(results: EmbeddingResult[], filePath: string): Promise<void> {
    try {
      const fs = await import('fs/promises');
   ...
   */
  private async storeResultsToDatabase(results: EmbeddingResult[]): Promise<void> {
    if (!this.embeddingDb) {
    ...
    try {
      ...
      await this.embeddingDb.storeEmbeddings(results, modelInfo, this.config.embeddings);
      
      this.logger.info(`� Stored ${results.length} embeddings to database`);
    } catch (error: any) {
      throw new Error(`Failed to store embeddings to database: ${error.message}`);
    }
  }
