...
export const CONFIG_VALIDATION_SCHEMA: ValidationSchema = {
  ...
  indexParams: {
    efConstruction: ValidationUtils.numericRange(
      16,
      800,
  ...
  model: {
    ...
    downloadTimeout: ValidationUtils.numericRange(
      1000,
      300000,
      "model.downloadTimeout",
    ),
    maxConcurrentDownloads: ValidationUtils.numericRange(
      1,
      10,
  ...
  embeddings: {
    ...
    maxConcurrency: ValidationUtils.numericRange(
      1,
      8,
      "embeddings.maxConcurrency",
    ),
    ...
    enableConfidenceScoring: ValidationUtils.boolean(
      "embeddings.enableConfidenceScoring",
    ),

    textProcessing: {
      maxTokenLength: ValidationUtils.numericRange(
        100,
        100000,
        "embeddings.textProcessing.maxTokenLength",
      ),
      preserveCodeStructure: ValidationUtils.boolean(
        "embeddings.textProcessing.preserveCodeStructure",
      ),
      normalizeWhitespace: ValidationUtils.boolean(
        "embeddings.textProcessing.normalizeWhitespace",
      ),
      preserveComments: ValidationUtils.boolean(
        "embeddings.textProcessing.preserveComments",
      ),
      maxSnippetLength: ValidationUtils.numericRange(
        50,
        10000,
        "embeddings.textProcessing.maxSnippetLength",
      ),
    },
  },
  ...
  fileWatching: {
    ...
    includePatterns: ValidationUtils.stringArray(
      "fileWatching.includePatterns",
      0,
    ),
    excludePatterns: ValidationUtils.stringArray(
      "fileWatching.excludePatterns",
      0,
    ),
    debounceMs: ValidationUtils.numericRange(
      0,
      10000,
      "fileWatching.debounceMs",
    ),
