...
import { ModelInitializationError, EmbeddingGenerationError } from "./types.js";
...
export class XenovaEmbeddingGenerator implements EmbeddingGenerator {
  ...
  async initialize(modelPath: string): Promise<void> {
    ...
    try {
      ...
      env.backends.onnx.wasm.wasmPaths = path.join(
      ...
      console.log(
        `✅ Model "${this.modelName}" loaded successfully in ${this.initializationTime}ms`,
      );
  ...
  async generateEmbeddings(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      console.log(
        `Generated ${embeddings.length} embeddings in ${processingTime}ms (${(processingTime / texts.length).toFixed(2)}ms per embedding)`,
      );
  ...
  async batchProcess(
    ...
    if (config.maxConcurrency < 1 || config.maxConcurrency > 8) {
      throw new EmbeddingGenerationError(
        "Max concurrency must be between 1 and 8",
      );
    }

    console.log(
      `🚀 Processing ${annotations.length} annotations in batches of ${config.batchSize} (concurrency: ${config.maxConcurrency})`,
    );
    ...
    for (let i = 0; i < batches.length; i += config.maxConcurrency) {
      ...
      const progressPercent = (
        (results.length / annotations.length) *
        100
      ).toFixed(1);
      console.log(
        `📊 Progress: ${results.length}/${annotations.length} (${progressPercent}%)`,
      );
    }

    console.log(
      `✅ Batch processing completed: ${results.length} embeddings generated`,
    );
    return results;
  }
  ...
  private async validateModelOperation(): Promise<void> {
    try {
      ...
      if (
        ...
        !testEmbedding[0] ||
        testEmbedding[0].length === 0
      ) {
        throw new Error("Model validation failed: no embedding generated");
      }

      console.log(
        `✅ Model validation passed (embedding dimensions: ${testEmbedding[0].length})`,
      );
    } catch (error: any) {
      throw new ModelInitializationError(
  ...
  private validateInputTexts(texts: string[]): void {
    for (let i = 0; i < texts.length; i++) {
      ...
      if (typeof text !== "string") {
        throw new EmbeddingGenerationError(
          `Invalid input at index ${i}: expected string, got ${typeof text}`,
        );
      }
    ...
    try {
      ...
      for (let i = 0; i < annotations.length; i++) {
        ...
        try {
          ...
          const errorMsg =
            error.message || "Unknown error during embedding generation";
          errors.push({ nodeId: annotation.nodeId, error: errorMsg });
          console.warn(
            `⚠️  Failed to process annotation ${annotation.nodeId}: ${errorMsg}`,
          );
        }
      }
      ...
      if (errors.length > 0) {
        console.log(
          `📦 Batch ${batchNumber}/${totalBatches} completed with ${errors.length} errors in ${processingTime}ms (${avgTimePerItem.toFixed(2)}ms per item)`,
        );
        console.log(
          `❌ Failed annotations: ${errors.map((e) => e.nodeId).join(", ")}`,
        );
      } else {
        console.log(
          `📦 Batch ${batchNumber}/${totalBatches} completed successfully in ${processingTime}ms (${avgTimePerItem.toFixed(2)}ms per item)`,
        );
      }
  ...
  private prepareTextForEmbedding(annotation: Annotation): string {
    ...
    const processedText =
      this.textProcessor.prepareTextForEmbedding(annotation);

    // Validate the processed text
    const validation = this.textProcessor.validateInputText(processedText);
    if (!validation.isValid) {
      console.warn(
        `⚠️  Text validation issues for ${annotation.nodeId}:`,
        validation.issues,
      );
  ...
  private validateEmbeddings(
    ...
    for (let i = 0; i < embeddings.length; i++) {
      ...
      if (embedding.length === 0) {
        throw new EmbeddingGenerationError(
          `Empty embedding vector at index ${i}`,
        );
      }
      ...
      if (i === 0) {
        if (embedding.length !== DEFAULT_CONFIG.modelDimensions) {
          console.warn(
            `⚠️  Expected ${DEFAULT_CONFIG.modelDimensions} dimensions, got ${embedding.length}`,
          );
        }
      ...
      if (magnitude > 10 || magnitude < 0.01) {
        console.warn(
          `⚠️  Unusual embedding magnitude ${magnitude.toFixed(4)} at index ${i}`,
        );
      }
    }

    console.log(
      `✅ Validated ${embeddings.length} embeddings (${embeddings[0]?.length || 0} dimensions each)`,
    );
  }
