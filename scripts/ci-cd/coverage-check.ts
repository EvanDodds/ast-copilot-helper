  ...
  checkCoverage(): boolean {
    ...
    try {
      ...
      for (const check of checks) {
        ...
        const status = passed ? "✅" : "❌";
        console.log(
          `  ${status} ${check.name}: ${check.actual.toFixed(2)}% (required: ${check.threshold}%)`,
        );
      ...
      console.log(
        `  ${globalStatus} Global: ${globalCoverage.toFixed(2)}% (required: ${this.thresholds.global}%)`,
      );
...
const globalThreshold = thresholdArg
