...
describe("ModelRegistry", () => {
  ...
  it("should get models by format", () => {
    ...
    expect(
      onnxModels.every((model: ModelConfig) => model.format === "onnx"),
    ).toBe(true);
  });
});
...
describe("validateModelConfig", () => {
  ...
  it("should reject invalid format", () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain(
      "Model format must be one of: onnx, pytorch, tensorflow, json",
    );
  });
  ...
  it("should reject invalid tokenizer configuration", () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some((e: string) => e.includes("Tokenizer:"))).toBe(
      true,
    );
  });
});
...
describe("validateUrl", () => {
  ...
  it("should warn about unknown hosts", () => {
    ...
    expect(result.warnings).toContain(
      "URL host 'unknown-host.com' is not in the list of known safe sources",
    );
  });
  ...
  it("should reject incompatible architecture", () => {
    ...
    expect(
      result.errors.some((e: string) =>
        e.includes("Model requires architecture"),
      ),
    ).toBe(true);
  });
  ...
  it("should warn about excessive memory requirements", () => {
    ...
    expect(
      result.warnings.some(
        (w: string) => w.includes("Model requires") && w.includes("memory"),
      ),
    ).toBe(true);
  });
});
