...
export class EmbeddingDatabaseManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.debug("Embeddings directory initialized", {
  ...
  async storeEmbeddings(
    ...
    try {
      ...
      const storedEmbeddings: StoredEmbedding[] = results.map(
        (result, index) => ({
          ...result,
          index,
          storedAt: timestamp,
  ...
  async loadEmbeddings(): Promise<{
    ...
    try {
      ...
      throw new DatabaseError(`Failed to load embeddings: ${error.message}`, {
  ...
  async getExistingEmbeddings(
    nodeIds: string[],
  ): Promise<Record<string, StoredEmbedding>> {
        ...
        try {
          await Promise.all([
            access(join(this.embeddingsPath, metadataFile), constants.F_OK)
              .then(() =>
                writeFile(join(this.embeddingsPath, metadataFile), ""),
              )
              .catch(() => {}),
            access(join(this.embeddingsPath, embeddingFile), constants.F_OK)
              .then(() =>
                writeFile(join(this.embeddingsPath, embeddingFile), ""),
              )
              .catch(() => {}),
          ]);
        } catch (error: any) {
