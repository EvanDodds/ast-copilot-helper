...
describe("Performance Benchmarks", () => {
  ...
  describe("Large Repository Parsing Performance", () => {
    it(
      "should parse 15k+ nodes in under 10 minutes",
      async () => {
        ...
        console.log(
          `Generated ${filesNeeded} files with ~${totalNodes} nodes in ${generationTime}ms`,
        );
        ...
        const actualNodeCount = parseResults.reduce(
        ...
        console.log(
          `✅ Parsed ${actualNodeCount} nodes in ${totalParseTime}ms (${(totalParseTime / 1000).toFixed(2)}s)`,
        );
        console.log(
          `   Throughput: ${(actualNodeCount / (totalParseTime / 1000)).toFixed(0)} nodes/second`,
        );
    ...
    it("should handle concurrent parsing efficiently", async () => {
      ...
      for (let i = 0; i < fileCount; i++) {
        ...
        filePromises.push(
          testRepo.createFile(`src/concurrent/file${i}.ts`, content),
        );
      }
  ...
  describe("Query Performance Benchmarks", () => {
    it("should handle MCP queries under 200ms", async () => {
      ...
      const avgMCPTime =
        mcpQueryTimes.reduce((a, b) => a + b, 0) / mcpQueryTimes.length;
      ...
      console.log(
        `  All queries: ${mcpQueryTimes.map((t) => t.toFixed(0)).join(", ")}ms`,
      );
    ...
    it("should handle CLI queries under 500ms", async () => {
      ...
      const cliQueries = [
        ...
        {
          query: "search for functions with auth",
          expectedResults: symbolCount * 0.1,
        },
        {
          query: "list interfaces in api module",
          expectedResults: symbolCount * 0.05,
        },
        {
          query: "show error handling code",
          expectedResults: symbolCount * 0.08,
        },
        {
          query: "database related utilities",
          expectedResults: symbolCount * 0.12,
        },
      ];
      ...
      const avgCLITime =
        cliQueryTimes.reduce((a, b) => a + b, 0) / cliQueryTimes.length;
      ...
      console.log(
        `  All queries: ${cliQueryTimes.map((t) => t.toFixed(0)).join(", ")}ms`,
      );
    ...
    it("should maintain performance under concurrent query load", async () => {
      ...
      const queryPromises = queries.map((query) =>
        simulateMCPQuery(mockDatabase, query),
      );
      ...
      console.log(
        `Concurrent query performance: ${totalTime}ms total, ${avgTimePerQuery.toFixed(2)}ms average`,
      );
  ...
  describe("Memory and Resource Performance", () => {
    it("should maintain reasonable memory usage during large operations", async () => {
      ...
      console.log(
        `  After generation: ${(afterGenerationMemory.heapUsed / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
...
async function simulateMCPQuery(
  database: any[],
  query: string,
): Promise<{ success: boolean; results: any[]; processingTime: number }> {
  const startTime = Date.now();
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, baseProcessingTime + vectorSearchTime),
  );
...
async function simulateCLIQuery(
  database: any[],
  query: string,
): Promise<{ success: boolean; results: any[]; processingTime: number }> {
  const startTime = Date.now();
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, baseProcessingTime + additionalProcessing),
  );
