...
describe('Telemetry Configuration', () => {
  ...
  it('should validate configuration correctly', () => {
    ...
    // Invalid batch size
    expect(validateTelemetryConfig({ batchSize: 0 })).toContain('batchSize must be between 1 and 1000');
    expect(validateTelemetryConfig({ batchSize: 1001 })).toContain('batchSize must be between 1 and 1000');

    // Invalid flush interval
    expect(validateTelemetryConfig({ flushInterval: 29000 })).toContain('flushInterval must be between 30 seconds and 24 hours');

    // Invalid endpoint
    expect(validateTelemetryConfig({ endpoint: 'not-a-url' })).toContain('endpoint must be a valid URL');
    expect(validateTelemetryConfig({ endpoint: 'http://api.example.com' })).toContain('endpoint must use HTTPS or be localhost for development');

    // Invalid privacy level
    expect(validateTelemetryConfig({ privacyLevel: 'invalid' as PrivacyLevel })).toContain('privacyLevel must be "strict", "balanced", or "permissive"');
  });
  ...
  it('should check if telemetry is enabled correctly', () => {
    ...
    // Disabled config
    expect(isTelemetryEnabled({ ...enabledConfig, enabled: false })).toBe(false);
...
describe('PrivacyRespectingTelemetryManager', () => {
  ...
  it('should initialize successfully', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Initializing telemetry system...');
  });
  ...
  it('should reject invalid configuration', async () => {
    ...
    await expect(testManager.initialize(invalidConfig as TelemetryConfig)).rejects.toThrow('Invalid telemetry configuration');
  ...
  it('should configure telemetry settings', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Configuring telemetry settings...');
  });
  ...
  it('should reject invalid telemetry settings', async () => {
    ...
    await expect(manager.configureTelemetry(invalidSettings)).rejects.toThrow('Invalid telemetry settings');
  });
  ...
  it('should shutdown gracefully', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Shutting down telemetry system...');
    expect(consoleLogSpy).toHaveBeenCalledWith('Telemetry system shutdown completed');
  });

  it('should prevent double initialization', async () => {
    await manager.initialize();
    
    await expect(manager.initialize()).rejects.toThrow('Telemetry manager is already initialized');
  });

  it('should prevent initialization after shutdown', async () => {
    // Use a separate test manager to avoid interference
    const testManager = new PrivacyRespectingTelemetryManager({ enabled: false });
    await testManager.initialize();
    await testManager.shutdown();
    
    await expect(testManager.initialize()).rejects.toThrow('Telemetry manager has been shutdown');
  });
});
...
describe('Edge Cases and Error Handling', () => {
  ...
  it('should handle initialization failures gracefully', async () => {
    ...
    await expect(testManager.initialize({
      ...DEFAULT_TELEMETRY_CONFIG,
