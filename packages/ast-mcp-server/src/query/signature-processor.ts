...
export class SignatureQueryProcessor {
  ...
  async processQuery(
    ...
    try {
      ...
      if (options.exactMatch) {
        ...
        candidates = await this.searchExactMatches(
          parsedSignature,
          options,
          maxResults,
        );
        ...
        candidates = await this.searchFuzzyMatches(
          parsedSignature,
          options,
          maxResults,
        );
      ...
      const convertedResults = finalResults.map((match) =>
        this.convertToAnnotationMatch(match),
      );
      ...
      const response: QueryResponse = {
        ...
        queryTime: totalTime,
        searchStrategy: options.exactMatch
          ? "signature_exact_match"
          : "signature_fuzzy_match",
        metadata: {
          vectorSearchTime: 0,
          rankingTime,
          ...
          searchParameters: {
            ...
            fuzzyThreshold:
              options.fuzzyThreshold || this.matchingConfig.fuzzyThreshold,
  ...
  private scoreAndRankMatches(
    ...
    const scoredMatches = candidates.map((match) => {
      const score = this.calculateSignatureScore(
        match,
        targetSignature,
        options,
      );
  ...
  private calculateSignatureScore(
    ...
    const parameterSimilarity = this.estimateParameterMatch(
      match,
      targetSignature,
    );
    ...
    if (options.includeReturnType && targetSignature.returnType) {
      const returnTypeSimilarity = this.estimateReturnTypeMatch(
        match,
        targetSignature,
      );
   ...
   */
  private estimateParameterMatch(
    match: ASTNodeMatch,
    targetSignature: SignatureAnalysis,
  ): number {
    // Count parentheses and commas to estimate parameter count
    const paramCount =
      (match.sourceSnippet.match(/,/g) || []).length +
      (match.sourceSnippet.includes("(") ? 1 : 0);
   ...
   */
  private estimateReturnTypeMatch(
    match: ASTNodeMatch,
    targetSignature: SignatureAnalysis,
  ): number {
    ...
    const hasReturnType =
      match.sourceSnippet.includes(":") || match.sourceSnippet.includes("=>");
    ...
    if (hasReturnType) {
      ...
      if (returnTypeMatch && returnTypeMatch[1]) {
        ...
        return this.calculateNameSimilarity(
          extractedType,
