...
import { DEFAULT_ANNOTATION_CONFIG, ExportType } from "./annotation-types.js";
...
export class AnnotationGenerator {
   ...
   */
  private extractLanguageFeatures(
    node: ASTNode,
    context: AnnotationContext,
  ): LanguageFeatures {
  ...
  private extractTypeScriptFeatures(
    ...
    if (node.metadata.languageSpecific?.typeAnnotation) {
      features.typeAnnotations = [
        node.metadata.languageSpecific.typeAnnotation,
      ];
    }
    ...
    features.isGenerator =
      features.modifiers.includes("generator") || node.name?.includes("*");
   ...
   */
  private extractJavaScriptFeatures(
    node: ASTNode,
    features: LanguageFeatures,
  ): void {
    ...
    if (node.type === NodeType.CLASS) {
      ...
      const extendsMatch = features.modifiers.find((m) =>
        m.startsWith("extends:"),
      );
      if (extendsMatch) {
  ...
  private extractPythonFeatures(
    ...
    // Async detection
    features.isAsync =
      features.modifiers.includes("async") || node.name?.startsWith("async ");
  }
  ...
  private generateSignature(
    ...
    // Add modifiers
    if (
      this.config.signature.includeModifiers &&
      features.modifiers.length > 0
    ) {
      const relevantModifiers = features.modifiers.filter(
        (m) => !m.startsWith("export") && !m.startsWith("extends:"),
      );
    ...
    if (signature.length > this.config.signature.maxLength) {
      signature =
        signature.substring(0, this.config.signature.maxLength - 3) + "...";
    }
  ...
  private generateFunctionSignature(
    ...
    if (this.config.signature.includeReturnType && features.typeAnnotations) {
      const returnType = features.typeAnnotations.find(
   ...
   */
  private generateClassSignature(
    node: ASTNode,
    features: LanguageFeatures,
  ): string {
   ...
   */
  private generateInterfaceSignature(
    node: ASTNode,
    features: LanguageFeatures,
  ): string {
   ...
   */
  private generateVariableSignature(
    node: ASTNode,
    features: LanguageFeatures,
  ): string {
  ...
  private calculateComplexity(
    ...
    if (langRules) {
      complexity = this.applyLanguageComplexityRules(
        complexity,
        node,
        context,
        langRules,
   ...
   */
  private applyLanguageComplexityRules(
    complexity: number,
    _node: ASTNode,
   ...
   */
  private extractNamespacedCalls(
    node: ASTNode,
    context: AnnotationContext,
  ): string[] {
    const calls: string[] = [];
    ...
    for (let i = startLine; i < endLine; i++) {
      if (i < sourceLines.length) {
        ...
        if (line) {
          ...
          const namespacedMatches = line.match(
  ...
  private generateSummary(
    ...
    const template =
      this.config.summary.templates[node.type] || "Code element {name}";
   ...
   */
  private extractSummaryVariables(
    node: ASTNode,
    features: LanguageFeatures,
    context: AnnotationContext,
    ...
    if (
      [NodeType.FUNCTION, NodeType.METHOD, NodeType.ARROW_FUNCTION].includes(
        node.type,
      )
    ) {
      const params = this.extractParameters(node, context);
  ...
  private inferPurpose(node: ASTNode, _context: AnnotationContext): string {
    ...
    switch (node.type) {
      ...
      case NodeType.GETTER:
        return "gets property value";
      case NodeType.SETTER:
        return "sets property value";
      case NodeType.METHOD:
        return "performs class operation";
      default:
  ...
  private extractSourceSnippet(
    ...
    const startLine = Math.max(
      0,
      node.start.line - 1 - this.config.snippet.contextBefore,
    );
    const endLine = Math.min(
      sourceLines.length - 1,
      node.end.line - 1 + this.config.snippet.contextAfter,
    );
    ...
    return {
      ...
      metadata: {
        ...
        quality: {
          ...
          isComplete: false,
          issues: [
            `Annotation generation failed: ${error instanceof Error ? error.message : String(error)}`,
          ],
        },
        characterCount: 0,
        lineCount: 0,
